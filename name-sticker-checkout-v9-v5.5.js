...
(function installNameStickerCheckoutAdapter(){
  ...
  /*
   * 結帳清單已清空時，只保留最後一組尚待製作的方案。
   * 例如舊紀錄 [1,1] 不可在新清單中繼續累計成 2 款，
   * 但仍保留使用者最後選擇的 1／2／4 入方案。
   */
  function collapseEmptyPackageHistory(plan, rows){
    const normalizedPlan = normalizePackagePlan(plan);
    if((Array.isArray(rows) ? rows.length : 0) > 0 || normalizedPlan.length <= 1){
      return normalizedPlan;
    }
    return [clampCount(normalizedPlan[normalizedPlan.length - 1] || 1)];
  }
  ...
  function loadPackagePlan(type, items){
    ...
    plan = collapseEmptyPackageHistory(plan, rows);
  ...
  if(baseDeleteSaved){
    deleteSavedDesign = function(index){
      ...
      TYPE_ORDER.forEach(type=>{ loadPackagePlan(type, remainingItems); });
      repriceNameStickerCart();
      renderCheckoutSummary();
      return result;
    };
