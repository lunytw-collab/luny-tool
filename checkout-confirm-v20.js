...
const LUNY_V17_VERSION = "2026-08-10.20";
...
const LUNY_OFFICIAL_LAUNCH_TIMEOUT_MS = 30000;
...
function beginCheckoutLaunchMonitor(){
  ...
  checkoutLaunchMonitorTimer = setInterval(function(){
    ...
    const routeActive = isOfficialCheckoutFlowActive();
    const addCartButton = routeActive ? findOfficialAddCartButton() : null;
    ...
    const ready = !!(
      addCartButton &&
      isLunyElementInteractable(addCartButton, { requireViewport:false })
    );
    ...
    // 新版自動同步有 expectedRewriteKey 時，必須等到購物車實際核對成功；
    // 只看到「加入購物車」按鈕不能視為交接完成。
    if(routeActive && ready && !expectedRewriteKey){
      if(!checkoutLaunchReadyAt) checkoutLaunchReadyAt = Date.now();

      // 要求按鈕持續處於「可見、未 disabled、pointer-events 可用」狀態一小段時間。
...
async function goToProductCheckout(){
  ...
  try{
    ...
    startCheckoutCountdown(10, "opening");
    ...
    setCheckoutActionButtonsText("LOADING 中｜同步 1SHOP（約 10 秒）");
