/*
  LUNY Catalog Patch v9.1 UI Only - no flash
  GitHub filename:
  luny-catalog-patch-v9.1-no-flash.js

  Required load order in 1SHOP:
  1) 原本標籤貼紙模板
  2) catalog pricing API 2026-09-07 or newer
  3) luny-catalog-patch-v9.1-no-flash.js
  ...
  - Runs legacy option rebuilds once and yields later display updates to the final page UI
*/
...
(function LunyCatalogFromLabelTemplateV8(){
  ...
  // ✅ 圖鑑貼紙價格表：luny-catalog-pricing-v7-20plus-x2.js
  // 本頁只透過 window.LUNY_CATALOG_PRICING.getPrice(...) 取得價格。



  const CATALOG_PRICING_SRC = "https://cdn.jsdelivr.net/gh/lunytw-collab/luny-tool@main/luny-catalog-pricing-v7-20plus-x2.js?v=20260907-1";
  ...
  function forceCatalogQuantityOptions(){
    const qty = $("quantity");
    if(!qty) return;
    const values = ["20","30","40","50","60","70","80","90","100","200","300","400","500"];
  ...
  function validateCatalogQuote(){
    ...
    if(!window.LUNY_CATALOG_PRICING){ alert("圖鑑報價資料尚未載入，請確認 luny-catalog-pricing-v7-20plus-x2.js 已正確引用。"); return false; }
    ...
    if(!window.LUNY_CATALOG_PRICING || typeof window.LUNY_CATALOG_PRICING.getPrice !== "function"){
      ...
      ensureCatalogPricingLoaded(function(ok){
        if(ok){
          ...
          if(hint2) hint2.textContent = "圖鑑報價資料載入失敗，請確認 luny-catalog-pricing-v7-20plus-x2.js 已上傳 GitHub，且引用順序在 patch 前面。";
        }
      });
      return null;
    }
  ...
  function forceCatalogLaminateOptions(){
    ...
    const material = normalizeCatalogMaterialValue($("material") ? $("material").value : "pearlescent");
    const options = material === "shtte"
      ? [{ value:"none", text:"無上膜" }]
      : [{ value:"gloss", text:"亮膜" }, { value:"matte", text:"霧膜" }];
    const oldValue = normalizeCatalogLaminateValue(laminate.value);
    laminate.innerHTML = options.map(o => '<option value="'+o.value+'">'+o.text+'</option>').join('');
    laminate.value = options.some(o => o.value === oldValue) ? oldValue : options[0].value;

    const group = $("laminateCardGroup");
    if(group){
      group.innerHTML = options.map(o => {
        const subtitle = o.value === "none"
          ? "可書寫紙材｜不另上膜"
          : (o.value === "matte" ? "柔和低反光｜亮霧同價" : "色彩較鮮明｜亮霧同價");
        return '<button type="button" class="laminate-card '+(laminate.value === o.value ? 'is-active' : '')+'" data-value="'+o.value+'">'+
          '<div class="laminate-card-title">'+o.text+'</div>'+
          '<div class="laminate-card-subtitle">'+subtitle+'</div>'+
          '</button>';
      }).join('');
  ...
  function forceCatalogSizeOptions(){
    ...
    `;
    target.value = ["A4","A5","A6"].includes(current) ? current : "A4";
  }

  function addCatalogMaterialImages(){
    document.querySelectorAll(".material-card[data-value='pearlescent'], .material-card[data-value='shtte'], .material-card[data-value='transparent']").forEach(card => {
      if(card.querySelector(".luny-catalog-material-img")) return;
      const img = document.createElement("img");
      img.src = CATALOG_MATERIAL_IMAGE_URL;
      img.alt = materialText(card.getAttribute("data-value")) + "示意圖";
      img.loading = "lazy";
  ...
  function buildCatalogUIFromLabelTemplate(){
    ...
    if(sizeRow){
      ...
      sizeRow.innerHTML = '<div style="width:100%;"><label for="catalogSize">尺寸：</label><select id="catalogSize"><option value="A4" selected>A4</option><option value="A5">A5</option><option value="A6">A6</option></select><div class="catalog-cloud-note">圖鑑貼紙為整張排版商品，請依需求選擇 A4／A5／A6。</div></div><input type="hidden" id="widthCm" value="21"><input type="hidden" id="heightCm" value="29.7">';
    }
    if($("shape")) $("shape").value = "roundrect";

    // 材質：保留標籤貼紙卡片 UI class，使用圖鑑三種正式材質
    const materialSelect = $("material");
    setSelectOptions(materialSelect, [
      {value:"pearlescent", text:"防水合成紙"},
      {value:"shtte", text:"模造貼紙"},
      {value:"transparent", text:"透明貼紙（加白墨）"}
    ]);
...
<button type="button" class="material-card is-active" data-value="pearlescent"><div class="material-card-title">防水合成紙</div><div class="material-card-subtitle">合成紙＋上膜｜亮霧同價</div><div class="material-card-desc">防水耐用，可選亮膜或霧膜。</div></button>\
<button type="button" class="material-card" data-value="shtte"><div class="material-card-title">模造貼紙</div><div class="material-card-subtitle">無上膜｜可書寫</div><div class="material-card-desc">紙張表面適合書寫，不另上膜。</div></button>\
<button type="button" class="material-card" data-value="transparent"><div class="material-card-title">透明貼紙（加白墨）</div><div class="material-card-subtitle">透明貼＋白墨＋上膜｜亮霧同價</div><div class="material-card-desc">可選亮膜或霧膜，白墨可呈現較清楚的圖案。</div></button>\
</div>';
    }
    ...
    // 數量：20~500；急件可選，急件計價規則由 pricing v6 統一處理
    forceCatalogQuantityOptions();
    const urgent = $("urgent");
    if(urgent){
...
(function(){
    ...
    section.innerHTML = `
      <div class="material-card-group material-card-group-inner" data-material-group="catalog" style="display:grid;grid-template-columns:1fr;gap:10px;">
        <button type="button" class="material-card ${current === "pearlescent" ? "is-active" : ""}" data-value="pearlescent">
          <img src="${MATERIAL_IMG}" class="material-card-img luny-catalog-material-img" alt="防水合成紙示意圖" loading="lazy">
          <div class="material-card-title">防水合成紙</div>
          <div class="material-card-subtitle">合成紙＋上膜｜亮霧同價</div>
          <div class="material-card-desc">防水耐用，可選亮膜或霧膜。</div>
        </button>

        <button type="button" class="material-card ${current === "shtte" ? "is-active" : ""}" data-value="shtte">
          <img src="${MATERIAL_IMG}" class="material-card-img luny-catalog-material-img" alt="模造貼紙示意圖" loading="lazy">
          <div class="material-card-title">模造貼紙</div>
          <div class="material-card-subtitle">無上膜｜可書寫</div>
          <div class="material-card-desc">紙張表面適合書寫，不另上膜。</div>
        </button>

        <button type="button" class="material-card ${current === "transparent" ? "is-active" : ""}" data-value="transparent">
          <img src="${MATERIAL_IMG}" class="material-card-img luny-catalog-material-img" alt="透明貼紙加白墨示意圖" loading="lazy">
          <div class="material-card-title">透明貼紙（加白墨）</div>
          <div class="material-card-subtitle">透明貼＋白墨＋上膜｜亮霧同價</div>
          <div class="material-card-desc">可選亮膜或霧膜，白墨可呈現較清楚的圖案。</div>
        </button>
      </div>
    `;
  ...
  function forceLaminateCardsV6(){
    ...
    var material = String($("material") && $("material").value || "pearlescent");
    var options = material === "shtte"
      ? [{ value:"none", label:"無上膜" }]
      : [{ value:"gloss", label:"亮膜" }, { value:"matte", label:"霧膜" }];
    setSelectOptions(select, options, material === "shtte" ? "none" : "gloss");
    ...
    group.innerHTML = options.map(function(opt){
      var subtitle = opt.value === "none"
        ? "可書寫紙材｜不另上膜"
        : (opt.value === "matte" ? "柔和低反光｜亮霧同價" : "色彩較鮮明｜亮霧同價");
      return '<button type="button" class="laminate-card '+(current === opt.value ? 'is-active' : '')+'" data-value="'+opt.value+'">'+
        '<div class="laminate-card-title">'+opt.label+'</div>'+
        '<div class="laminate-card-subtitle">'+subtitle+'</div>'+
        '</button>';
    }).join('');
  ...
  function forceSizeLabelsV6(){
    ...
    if(["A4","A5","A6"].indexOf(current) >= 0) size.value = current;
  }
  ...
  function scheduleV7(){
    if(window.__LUNY_CATALOG_FINAL_UI_OWNER__) return;
