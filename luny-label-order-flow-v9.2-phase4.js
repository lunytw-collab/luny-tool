/* LUNY label order flow v9.2-phase3-blob-queue
...
let json=null;
try{json=JSON.parse(text);}catch(e){throw new Error("GAS 回傳不是 JSON："+String(text||"").slice(0,800));}
return json;
}finally{
...
async function saveDesignToGAS(){
  ...
  try{
    ...
    const designId=window.__LUNY_CURRENT_SAVE_DESIGN_ID__||
      (window.__LUNY_CURRENT_SAVE_DESIGN_ID__=newDesignId());
    ...
    const json=await postJsonToGASWithRetry(
      ...
      stripLargeImageDataForSheet(bodyObj),
      {
        ...
        onRetry:function(attempt,waitMs){
          setSaveStatus(`伺服器忙碌，${Math.ceil(waitMs/1000)} 秒後進行第 ${attempt} 次資料寫入重試…`);
        }
      }
    );
