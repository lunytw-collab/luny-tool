...
(function () {
  ...
  // v38：100 / 300 張依市場基準重新校正。
  // - 88～15 模：若原價低於市場價，售價為市場價減 100 元；其餘回到市場價。
  // - 12～3 模：一律回到市場價，並將個位數無條件捨去。
  // 12 模同時落在使用者描述的兩段範圍內，採較嚴格的第二段規則。
      ...
      Object.keys(materialReferenceMultiplier[materialKey]).forEach(function (finishKey) {
        ...
        table.forEach(function (item) {
          ...
          ["100", "300"].forEach(function (qty) {
            ...
            var marketPrice = roundNicePrice(
              Math.round(Number(referenceItem.price[qty]) * referenceMultiplier)
            );
