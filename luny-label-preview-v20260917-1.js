/* LUNY preview 20260917-v1: opaque-white coverage, explicit edge choices, bounded display. */

/* LUNY v16.8.5-background-safe-ring: 背景色鎖定安全範圍外，邊緣色優先。 */
/* LUNY v16.8.4-edge-color-state-sync: 邊緣色、頁面選項及輸出模式同步。 */
/* LUNY v16.7.9-preflight-release-rule：正式產檔與畫面印前檢查共用 canProceed 放行規則。 */
/* LUNY v16.7.8-fit-checkout-state：裁掉白邊並放大後，以設計狀態鍵保留正式產檔放行。 */
/* LUNY v16.7.7-preflight-confirm-save-v9-material-clean-source：材質效果強制使用無刀線、無尺寸標示的統一成品圖。 */
/* LUNY v16.7.7-backend-preview-match-v5：黃色確認狀態仍顯示安全距離危險區覆蓋層。 */
/* LUNY v16.7.7-backend-preview-match-v4：合成原圖安全區判定改為警告確認，避免背景紋理誤擋送單。 */
/* LUNY v16.7.7-backend-preview-match-v3：建議邊緣色統一填滿綠色安全線以外區域。 */
/* LUNY v16.7.7 preflight：明確安全距離或出血未鋪滿時，可加購整圖縮放、置中與單色補出血整理。 */
...
/* v7.9.49：畫布顯示尺寸維持原本大小；結帳清單縮圖仍固定最高 360px／JPEG 0.76。 */
/* v7.9.48：預覽區新增說明，明確告知印刷使用上傳原始檔，不會拿畫面預覽縮圖印刷。 */
/* LUNY v7.9.49：前端預覽固定使用輕量圖；正式 print/cut 才使用原始完整圖片。 */
 ...
 * Phase 3：getPrintAndCutBlobs 送出前檢查 print/cut Blob 是否完全相同，相同則停止儲存。
 * v7.9.15 歷史版曾靠 print/cut 角色標記攔截輸出；Phase 3 已改由 render/draw 流程直接決定內容。
...
const LUNY_PREVIEW_MAX_SIDE_PX=1800,LUNY_PREVIEW_MAX_TOTAL_PIXELS=3200000,LUNY_PREVIEW_MIN_PPI=96;
...
function lunyGetTargetPreviewPpi(widthCm,heightCm){
  ...
  const rawW=Math.max(1,totalWcm*targetPpi/2.54);
  const rawH=Math.max(1,totalHcm*targetPpi/2.54);
  let limitScale=1;
  const maxSide=Math.max(rawW,rawH);
  if(maxSide>LUNY_PREVIEW_MAX_SIDE_PX)limitScale=Math.min(limitScale,LUNY_PREVIEW_MAX_SIDE_PX/maxSide);
  const totalPixels=rawW*rawH;
  if(totalPixels>LUNY_PREVIEW_MAX_TOTAL_PIXELS)limitScale=Math.min(limitScale,Math.sqrt(LUNY_PREVIEW_MAX_TOTAL_PIXELS/totalPixels));
  return Math.min(targetPpi,Math.max(LUNY_PREVIEW_MIN_PPI,targetPpi*limitScale));
}
...
function lunyGetCheckerboardTile(cell,c1,c2){
  ...
  __lunyCheckerboardTileCache.set(key,tile);
  return tile;
}
drawCheckerboard=function(ctx,x,y,w,h,size,c1,c2){
  // On-screen preview reveals one continuous CSS checkerboard behind both areas.
  // Off-screen exports retain their existing raster background.
  if(ctx.canvas===cG&&document.getElementById('lunyPreviewCanvasFrame')){
    ctx.clearRect(x,y,w,h);
    return;
  }

  const cell=Math.max(6,Math.round(size||16));
  const color1=c1||'#f7f7f7',color2=c2||'#ececec';
  let patterns=__lunyCheckerboardPatternCache.get(ctx);
  if(!patterns){
    patterns=new Map();
    __lunyCheckerboardPatternCache.set(ctx,patterns);
  }
  const key=cell+'|'+color1+'|'+color2;
  let pattern=patterns.get(key);
  if(!pattern){
    pattern=ctx.createPattern(lunyGetCheckerboardTile(cell,color1,color2),'repeat');
...
const LUNY_CUSTOM_CONTOUR_OFFSETS_MM=Object.freeze([-0.5,0,1,2,4,5]);
...
function lunyCustomCleanMask(mask,w,h){
  ...
  /* 只移除真正的像素雜點；上限可避免句點、小字或 QR 定位塊被大主圖的相對面積門檻誤刪。 */
  const minArea=Math.max(3,Math.min(18,Math.round(components[0].area*.00015)));const out=new Uint8Array(mask.length);
...
function lunyCustomErode(mask,w,h,radius){
  ...
  /* 畫布外也視為背景：多加一圈背景種子，再對背景膨脹。 */
  const pw=w+2,ph=h+2,inverse=new Uint8Array(pw*ph);
  for(let x=0;x<pw;x++){inverse[x]=1;inverse[(ph-1)*pw+x]=1;}
  for(let y=1;y<ph-1;y++){
    inverse[y*pw]=1;inverse[y*pw+pw-1]=1;
    for(let x=1;x<pw-1;x++)inverse[y*pw+x]=mask[(y-1)*w+x-1]?0:1;
  }
  const expandedBackground=lunyCustomDilate(inverse,pw,ph,radius),out=new Uint8Array(mask.length);
  for(let y=0;y<h;y++)for(let x=0;x<w;x++)out[y*w+x]=expandedBackground[(y+1)*pw+x+1]?0:1;
  return out;
}
function lunyCustomCloseMask(mask,w,h,radius){
  if(radius<=0)return mask.slice();
  return lunyCustomErode(lunyCustomDilate(mask,w,h,radius),w,h,radius);
}
function lunyCustomComponentCount(mask,w,h,stopAfter){
  const limit=Math.max(1,Number(stopAfter)||Number.MAX_SAFE_INTEGER);
  const visited=new Uint8Array(mask.length),queue=new Int32Array(mask.length);let count=0;
  for(let start=0;start<mask.length;start++){
    if(!mask[start]||visited[start])continue;
...
function lunyCustomPrepareOuterMask(mask,w,h,pxPerMm){
  const componentCountBefore=lunyCustomComponentCount(mask,w,h);
  const closeRadius=Math.max(.5,LUNY_CUSTOM_CLOSE_GAP_MM*pxPerMm/2);
  let prepared=lunyCustomCloseMask(mask,w,h,closeRadius);
  let componentCountAfterClose=lunyCustomComponentCount(prepared,w,h);
  let helperRadiusPx=0,retainedExpansionPx=0;

  if(componentCountAfterClose>1){
    /*
     * callas HelpShape 同類做法：對所有物件加粗，自動找到剛好能讓所有區塊連在一起的最小半徑。
     * 連接後再往內收，只保留足以維持封閉的頸部，避免直接使用凸包吞掉凹槽。
     */
    const maxRadius=Math.max(closeRadius,Math.hypot(w,h)*.55);
    let low=0,high=Math.max(closeRadius,.5*pxPerMm),expanded=null;
    while(high<maxRadius){
      expanded=lunyCustomDilate(prepared,w,h,high);
      if(lunyCustomComponentCount(expanded,w,h,2)===1)break;
      low=high;high=Math.min(maxRadius,high*1.65);
    }
    expanded=lunyCustomDilate(prepared,w,h,high);
    if(lunyCustomComponentCount(expanded,w,h,2)===1){
      for(let i=0;i<7;i++){
        const middle=(low+high)/2,candidate=lunyCustomDilate(prepared,w,h,middle);
        if(lunyCustomComponentCount(candidate,w,h,2)===1){high=middle;expanded=candidate;}else low=middle;
      }
      helperRadiusPx=high;
      const retainMm=[.5,.75,1,1.5,2,3,4,6];
      let selected=expanded;
      for(const mm of retainMm){
        const erodeBy=Math.max(0,helperRadiusPx-mm*pxPerMm),candidate=lunyCustomErode(expanded,w,h,erodeBy);
        if(lunyCustomCountMask(candidate)&&lunyCustomComponentCount(candidate,w,h,2)===1){selected=candidate;retainedExpansionPx=helperRadiusPx-erodeBy;break;}
      }
      prepared=selected;
    }
  }

  /* 刀線只取最外層邊界：中間透孔與字型內孔不另外產生內刀線。 */
  prepared=lunyCustomFillHoles(prepared,w,h);
  return{
    mask:prepared,
    componentCountBefore,
    componentCountAfter:lunyCustomComponentCount(prepared,w,h),
    closedGapMm:LUNY_CUSTOM_CLOSE_GAP_MM,
    helperStrokeMm:helperRadiusPx>0?helperRadiusPx/pxPerMm*2:0,
    retainedExpansionMm:retainedExpansionPx>0?retainedExpansionPx/pxPerMm:0,
    bridged:helperRadiusPx>0
  };
}
...
function lunyCustomLargestOuterLoop(mask,w,h){
  const loops=lunyCustomTraceLoops(mask,w,h);
  loops.sort((a,b)=>Math.abs(lunyCustomPolygonArea(b))-Math.abs(lunyCustomPolygonArea(a)));
  return loops[0]||[];
}
function lunyCustomContourFromRequiredMask(required,w,h,pxPerMm){
  let best=null;
  /* 平滑會讓曲線微幅內縮；逐步加入很小的安全外擴，每次都重新驗證必須包住原等距遮罩。 */
  const extraOffsetsMm=[0,.12,.25,.45,.7,1,1.4,2];
  for(const extraMm of extraOffsetsMm){
    ...
    if(contains&&lunyCustomMinAngle(points)>=LUNY_CUSTOM_MIN_ANGLE_DEG-.25)return points;
  }
...
function lunyCustomBuildOutline(baseMask,w,h,pxPerMm,offsetMm){
  const required=offsetMm>0
    ?lunyCustomDilate(baseMask,w,h,offsetMm*pxPerMm)
    :(offsetMm<0?lunyCustomErode(baseMask,w,h,Math.abs(offsetMm)*pxPerMm):baseMask);
  return lunyCustomContourFromRequiredMask(required,w,h,pxPerMm);
}
...
function lunyCustomGenerateAndLockCutline(force){
  ...
  let base=lunyCustomRenderContentMask(w,h,ratio);base=lunyCustomCleanMask(base,w,h);
  if(!lunyCustomCountMask(base))return null;
  const pxPerMm=CM2PX*ratio/10,preparedInfo=lunyCustomPrepareOuterMask(base,w,h,pxPerMm);
  base=preparedInfo.mask;
  if(!lunyCustomCountMask(base))return null;
  const offsets=LUNY_CUSTOM_CONTOUR_OFFSETS_MM,absoluteByOffset={};
  ...
  const result={
    key:'locked-'+(++__lunyCustomCutlineVersion),pointsByOffset,minAngle,aspect,
    analysisWidth:w,analysisHeight:h,warnings:[],generatedAt:Date.now(),locked:true,
    topology:'single-outer-contour',outerOnly:true,
    contentOffsetMm:0,cutOffsetMm:2,safeOffsetMm:-0.5,safeInsideCutMm:2.5,
    bleedOffsetMm:4,bleedOutsideCutMm:2,technicalLineOffsetMm:5,technicalLineOutsideCutMm:3,
    sourceComponentCount:preparedInfo.componentCountBefore,
    finalComponentCount:preparedInfo.componentCountAfter,
    closedGapMm:preparedInfo.closedGapMm,
    helperStrokeMm:preparedInfo.helperStrokeMm,
    retainedExpansionMm:preparedInfo.retainedExpansionMm,
    usedHelpShape:preparedInfo.bridged
  };
  if(minAngle<LUNY_CUSTOM_MIN_ANGLE_DEG-.25)result.warnings.push(`刀線仍有 ${minAngle.toFixed(1)}° 的銳角，請改用較簡單圖案。`);
  if(preparedInfo.helperStrokeMm>LUNY_CUSTOM_BRIDGE_WARN_MM)result.warnings.push(`圖案區塊間距較大，系統已用 ${preparedInfo.helperStrokeMm.toFixed(1)}mm HelpShape 連接為單一刀線。`);
  if(preparedInfo.componentCountAfter>1)result.warnings.push('部分圖案距離過遠，無法安全連成單一封閉刀線。');
...
function lunyCustomNearestOffset(offsetMm){const values=[-0.5,0,1,2,4,5];let best=values[0],distance=Infinity;for(const v of values){const d=Math.abs(v-offsetMm);if(d<distance){distance=d;best=v;}}return best;}
...
function lunyCustomDrawGuides(ctx,cx,cy,dims,b,gap,cm2px){
  ...
  const colorMode=['color','edgecolor','fullcolor'].includes(String(edgeOption||'').toLowerCase());
  const previewW=colorMode?cutW:bleedW,previewH=colorMode?cutH:bleedH;
  const path=(w,h)=>{ctx.beginPath();return lunyCustomAddPath(ctx,w,h,cm2px);};
  ctx.save();ctx.beginPath();ctx.rect(0,0,ctx.canvas.width,ctx.canvas.height);lunyCustomAddPath(ctx,previewW,previewH,cm2px);ctx.clip('evenodd');drawCheckerboard(ctx,0,0,ctx.canvas.width,ctx.canvas.height,16,'#f7f7f7','#e9e9e9');if(ctx.canvas!==cG||!document.getElementById('lunyPreviewCanvasFrame')){ctx.fillStyle='rgba(255,255,255,.28)';ctx.fillRect(0,0,ctx.canvas.width,ctx.canvas.height);}ctx.restore();
  ...
  if(!colorMode)stroke(bleedW,bleedH,'#888888',4,[8,8]);stroke(cutW,cutH,'#D3162D',4,[]);stroke(safeW,safeH,'#32CD32',4,[8,8]);return true;
}
...
function applyLegacyMirrorBleedFromCutLine(ctx,canvas,cm2px,forceForPrint){
...
function lunyMirrorCreateShapeMask(W,H,shapeValue,cx,cy,w,h,cm2px,scale){
  ...
  mctx.beginPath();
  addMirrorBleedShapePath(mctx,shapeValue,cx*scale,cy*scale,w*scale,h*scale,cm2px*scale);
  mctx.fill();
  return{
    width:mask.width,height:mask.height,scale,
    data:mctx.getImageData(0,0,mask.width,mask.height).data
  };
}
function lunyMirrorMaskContains(mask,x,y){
  const mx=Math.max(0,Math.min(mask.width-1,Math.round(x*mask.scale)));
  const my=Math.max(0,Math.min(mask.height-1,Math.round(y*mask.scale)));
...
function lunyMirrorPointInStandardShape(shapeValue,x,y,w,h,cm2px){
  ...
  if(shapeValue==='arch'){
    ...
    const top=-halfH,bottom=halfH,capY=top+capRadius;
  ...
  const outside=Math.sqrt(Math.max(qx,0)*Math.max(qx,0)+Math.max(qy,0)*Math.max(qy,0));
...
function lunyMirrorStandardRadius(shapeValue,ux,uy,w,h,cm2px){
  ...
  let low=0,high=Math.sqrt(w*w+h*h);
  for(let i=0;i<18;i++){
    const middle=(low+high)/2;
    if(lunyMirrorPointInStandardShape(shapeValue,ux*middle,uy*middle,w,h,cm2px))low=middle;
    else high=middle;
  }
  return low;
}
function lunyMirrorFindOuterRadius(mask,cx,cy,ux,uy,maxRadius){
  ...
  for(let radius=maxRadius;radius>=0;radius-=step){
    if(lunyMirrorMaskContains(mask,cx+ux*radius,cy+uy*radius))return radius;
  }
  return -1;
}
function lunyMirrorReflectDistance(distance,band){
  const width=Math.max(1,band),period=width*2;
...
function lunyMirrorBuildProfiles(sourceData,W,H,shapeValue,cx,cy,cutW,cutH,bleedW,bleedH,cm2px,band){
  ...
  const cutMask=useShapeMask?lunyMirrorCreateShapeMask(W,H,shapeValue,cx,cy,cutW,cutH,cm2px,maskScale):null;
  const bleedMask=useShapeMask?lunyMirrorCreateShapeMask(W,H,shapeValue,cx,cy,bleedW,bleedH,cm2px,maskScale):null;
...
function applyMirrorBleedFromCutLine(ctx,canvas,cm2px,forceForPrint){
  ...
  }catch(e){console.warn('[LUNY] 自動補邊邊界分析失敗：',e);return;}
  if(!Number.isFinite(profiles.minColorRadius)||profiles.maxBleedRadius<=0)return;

  const bleedMask=profiles.bleedMask,count=profiles.count;
  ...
  for(let y=minY;y<=maxY;y++){
    ...
    const innerDx=Math.abs(dy)<innerLimit?Math.sqrt(innerLimit*innerLimit-dy*dy):0;
    for(let x=minX;x<=maxX;x++){
      if(innerDx>0&&x>cx-innerDx&&x<cx+innerDx){
        x=Math.max(x,Math.ceil(cx+innerDx));
        if(x>maxX)break;
      }
      const dx=x-cx,radius=Math.sqrt(dx*dx+dy*dy);
...
function drawGuides(ctx,shapeValue,cx,cy,dims,b,gap,cm2px){
  ...
  function addShapeSubPath(w,h){
    if(s==='circle'){ctx.arc(cx,cy,Math.min(w,h)/2,0,Math.PI*2);}
    else if(s==='roundrect'){roundedRectPath(ctx,cx-w/2,cy-h/2,w,h,rpx);ctx.closePath();}
    else if(s==='ellipse'){ctx.ellipse(cx,cy,w/2,h/2,0,0,Math.PI*2);}
    else if(s==='arch'){archPath(ctx,cx-w/2,cy-h/2,w,h,cm2px);ctx.closePath();}
  }
  function shapePath(w,h){ctx.beginPath();addShapeSubPath(w,h);}
  function strokeShape(w,h,color,lineWidth,dash){ctx.save();ctx.strokeStyle=color;ctx.lineWidth=lineWidth;ctx.setLineDash(dash||[]);shapePath(w,h);ctx.stroke();ctx.restore();}
  ctx.save();ctx.beginPath();ctx.rect(0,0,ctx.canvas.width,ctx.canvas.height);addShapeSubPath(previewW,previewH);ctx.clip('evenodd');drawCheckerboard(ctx,0,0,ctx.canvas.width,ctx.canvas.height,16,'#f7f7f7','#e9e9e9');if(ctx.canvas!==cG||!document.getElementById('lunyPreviewCanvasFrame')){ctx.fillStyle='rgba(255,255,255,0.28)';ctx.fillRect(0,0,ctx.canvas.width,ctx.canvas.height);}ctx.restore();
  if(edgeOption==='on'){ctx.save();ctx.fillStyle='#ffffff';ctx.beginPath();addShapeSubPath(bleedW,bleedH);addShapeSubPath(safeW,safeH);ctx.fill('evenodd');ctx.restore();}
  if(!colorMode)strokeShape(bleedW,bleedH,'#888888',4,[8,8]);
...
/* v16.8.5: lock explicit background color outside the safe area after artwork. */
function applySelectedBackgroundColor(ctx,canvas,cm2px){
  if(!backgroundColor)return;
  lunyFillOutsideSafeArea(ctx,canvas,cm2px,backgroundColor);
}
function applySelectedEdgeColor(ctx,canvas,cm2px){
  ...
  lunyFillOutsideSafeArea(ctx,canvas,cm2px,color);
}
function lunyFillOutsideSafeArea(ctx,canvas,cm2px,color){
...
function photoCoversPreviewShape(w,h){if(!img)return false;const pts=sampleShapeBoundaryPoints(w,h,160);let miss=0;for(const p of pts){if(!photoCoversPreviewPoint(p.x,p.y)){miss++;if(miss>Math.max(2,pts.length*0.03))return false;}}return true;}function previewHasVisibleContentInShape(w,h,samples){const pts=sampleShapeBoundaryPoints(w,h,samples||140);let hits=0;for(const p of pts){if(photoContainsPreviewPoint(p.x,p.y)){hits++;if(hits>=3)return true;}}if(iconImg){const icx=cG.width/2+iconOffsetX,icy=cG.height/2+iconOffsetY;if(pointInRotRect(icx,icy,cG.width/2,cG.height/2,w,h,0)) return true;}if(textStr){const tcx=cG.width/2+textOffsetX,tcy=cG.height/2+textOffsetY;if(pointInRotRect(tcx,tcy,cG.width/2,cG.height/2,w,h,0)) return true;}return false;}function pointInPreviewShape(x,y,w,h){const cx=cG.width/2,cy=cG.height/2;if(shape.value==='custom'){const pts=lunyCustomGetPoints(ctxG,w,h,CM2PX);if(pts&&pts.length)return lunyCustomPointInPolygon(x,y,pts);}const s=shape.value;if(s==='circle'){return Math.hypot(x-cx,y-cy)<=Math.min(w,h)/2;}if(s==='ellipse'){const rx=Math.max(1,w/2),ry=Math.max(1,h/2);const nx=(x-cx)/rx,ny=(y-cy)/ry;return nx*nx+ny*ny<=1;}if(s==='arch'){const left=cx-w/2,right=cx+w/2,top=cy-h/2,bottom=cy+h/2;const r=Math.min(w/2,h);const archCy=top+r;if(y>=archCy&&y<=bottom&&x>=left&&x<=right)return true;const dx=x-cx,dy=y-archCy;return dy<=0 && dx*dx+dy*dy<=r*r;}return Math.abs(x-cx)<=w/2 && Math.abs(y-cy)<=h/2;}function pointInDangerRing(x,y,safeW,safeH,cutW,cutH){return pointInPreviewShape(x,y,cutW,cutH) && !pointInPreviewShape(x,y,safeW,safeH);}function rectTouchesDangerZone(cx,cy,w,h,a,safeW,safeH,cutW,cutH){const pts=corners(cx,cy,w,h,a);const mids=[mid(pts[0],pts[1]),mid(pts[1],pts[2]),mid(pts[2],pts[3]),mid(pts[3],pts[0]),{x:cx,y:cy}];for(const p of pts.concat(mids)){if(pointInDangerRing(p.x,p.y,safeW,safeH,cutW,cutH)) return true;}return false;}window.__lunyCheckDangerZoneHit=function(){const edge=String(getEdgeOption()||'off').toLowerCase();if(['on','white','whiteedge','white-edge','border','edge','color','edgecolor','fullcolor'].includes(edge)) return false;const W=cG.width,H=cG.height,b=BLEED_CM*CM2PX,gap=GAP_CM*CM2PX;const cutW=W-2*b,cutH=H-2*b;const safeW=Math.max(1,cutW-2*gap),safeH=Math.max(1,cutH-2*gap);if(iconImg){const w=iconImg.width*iconScale,h=iconImg.height*iconScale;if(rectTouchesDangerZone(cG.width/2+iconOffsetX,cG.height/2+iconOffsetY,w,h,iconAngle,safeW,safeH,cutW,cutH)) return true;}if(textStr){const fontPx=textSizeCM*CM2PX*textScale;const m=measureTextBox(textStr,fontPx);if(rectTouchesDangerZone(cG.width/2+textOffsetX,cG.height/2+textOffsetY,m.w,m.h,textAngle,safeW,safeH,cutW,cutH)) return true;}let compositeHits=0;for(let t=0;t<=1.0001;t+=0.2){const ww=safeW+(cutW-safeW)*t,hh=safeH+(cutH-safeH)*t;const pts=sampleShapeBoundaryPoints(ww,hh,180);for(const p of pts){if(!pointInDangerRing(p.x,p.y,safeW,safeH,cutW,cutH)) continue;if(photoPointHasCompositeContent(p.x,p.y)){compositeHits++;if(compositeHits>=2) return true;}}}return false;};function getBleedRiskStatus(){
  ...
  if(danger){
    return{level:1,zone:'danger',title:'有圖文內容碰到裁切線內側 2.5mm 危險區域，請再往內縮。',important:true};
  }
...
function validateDesignBeforeExport(action,opts){
  ...
  // 正式產檔必須與畫面上的印前檔案檢查共用同一個結果。
  // READY、已套用裁白邊方案、已接受警告或已選檔案整理，皆由 canProceed 決定；
  // 解析度不足、白邊未處理及尚未接受的警告仍維持阻擋。
  const preflightResult=lunyGetPreflightResult();
  if(preflightResult&&preflightResult.canProceed){
    approveBleedRiskTemporarily();
    return true;
  }

  // 正式輸出不得使用另一套舊出血規則覆蓋印前檢查結果。
...
function drawImportantSafetyRiskMask(ctx,canvas,cm2px){
  ...
  if(riskOverlay){
    if(riskOverlay.width!==W)riskOverlay.width=W;
    if(riskOverlay.height!==H)riskOverlay.height=H;
    const riskCtx=riskOverlay.getContext('2d');
    riskCtx.clearRect(0,0,W,H);
    riskCtx.fillStyle='rgba(239,68,68,0.30)';
    riskCtx.beginPath();
    addLunyRiskShapePath(riskCtx,shape.value,cx,cy,cutW,cutH,cm2px);
    addLunyRiskShapePath(riskCtx,shape.value,cx,cy,safeW,safeH,cm2px);
    riskCtx.fill('evenodd');
  }

  ctx.save();
  ctx.strokeStyle=caution?'rgba(22,163,74,0.92)':'rgba(211,22,45,0.78)';
...
function lunyUpdateSafetyDistanceBadge(show,caution){
  ...
  if(!badge){
    ...
    badge.innerHTML='<strong>⚠ 裁切風險區</strong><span>此範圍內容將可能被裁切</span>';
    anchor.before(badge);
    __lunySafetyDistanceBadge=badge;
    const align=()=>{
      badge.style.width=anchor.getBoundingClientRect().width+'px';
      const overlay=document.getElementById('lunyCutRiskOverlay');
      if(overlay&&overlay.lunyAlign)overlay.lunyAlign();
    };
    badge.lunyAlign=align;
    if(typeof ResizeObserver==='function')new ResizeObserver(align).observe(anchor);
    window.addEventListener('resize',align);
  }
  // Reserve a dedicated checkerboard header above the artwork, in normal flow.
  // The artwork canvas retains its dimensions and pointer coordinate mapping.
  badge.style.display='flex';
  badge.style.visibility=show?'visible':'hidden';
  badge.setAttribute('aria-hidden',show?'false':'true');
  anchor.classList.toggle('luny-cut-risk-header-visible',show);
  badge.lunyAlign();
}
let __lunyPreviewRAF=0;
let __lunyInteractiveDraw=false;
let __lunyInteractiveTimer=0;
let __lunyPointerInteractive=false;
let __lunyUiInputActive=false;
let __lunyUiInputTimer=0;
let __lunyUiInputEventActive=false;
function lunySyncInteractiveDraw(){
  __lunyInteractiveDraw=__lunyPointerInteractive||__lunyUiInputActive;
}
function lunyBeginInteractiveDraw(){
  __lunyPointerInteractive=true;
  lunySyncInteractiveDraw();
...
function lunyEndInteractiveDraw(delay){
  ...
  __lunyInteractiveTimer=setTimeout(function(){
    __lunyPointerInteractive=false;
    lunySyncInteractiveDraw();
...
function scheduleUiInputPreviewDraw(){
  __lunyUiInputActive=true;
  lunySyncInteractiveDraw();
  scheduleDrawPreview();
  if(__lunyUiInputTimer)clearTimeout(__lunyUiInputTimer);
  __lunyUiInputTimer=setTimeout(function(){
    __lunyUiInputTimer=0;
    __lunyUiInputActive=false;
    lunySyncInteractiveDraw();
    drawPreview();
  },120);
}
function lunyMarkUiInputEvent(){
  __lunyUiInputEventActive=true;
  Promise.resolve().then(function(){__lunyUiInputEventActive=false;});
}
document.addEventListener('input',lunyMarkUiInputEvent,true);
document.addEventListener('change',lunyMarkUiInputEvent,true);
...
function lunyUpdateWhiteEdgeBadge(){
  ...
  __lunyWhiteEdgeCheckTimer=setTimeout(function(){
    ...
    __lunyWhiteEdgeLastKey=latestKey;__lunyWhiteEdgeLastResult=show;
    badge.style.display=show?'block':'none';
    if(show)requestAnimationFrame(()=>lunyPositionWhiteEdgeBadge(badge));
  },90);
}
function drawPreview(){
  if(__lunyUiInputEventActive){
    scheduleUiInputPreviewDraw();
    return;
  }
  resizePreviewCanvas();
  ...
  handles=drawAll(ctxG,cG,CM2PX,{includeGuides:showGuides,includeSelection:shape.value!=='custom',showQRTestMark:showQRTest,showMinText:showTestText,isPreview:true,useFullImage:false});
...
window.LUNY_clearPreviewSelection=function(){
  activeTarget=null;
  handles=null;
  drawPreview();
  return true;
};
function lunyFitPreviewDisplay(){
  const frame=document.getElementById('lunyPreviewCanvasFrame');
  if(!frame||!frame.clientWidth||!frame.clientHeight)return;
  const maxW=Math.max(1,frame.clientWidth-24),maxH=Math.max(1,frame.clientHeight-64);
  const factor=Math.min(maxW/cG.width,maxH/cG.height);
  const values={width:(cG.width*factor).toFixed(3)+'px',height:(cG.height*factor).toFixed(3)+'px'};
  for(const name of ['width','height'])if(cG.style.getPropertyValue(name)!==values[name])cG.style.setProperty(name,values[name],'important');
  const label=document.getElementById('lunyPreviewActualSize');
  const size=lunyGetEffectiveSizeCm();
  const copy='成品尺寸：'+Number(size.widthCm)+' × '+Number(size.heightCm)+' cm｜畫面依比例縮放，非實際大小';
  if(label&&label.textContent!==copy)label.textContent=copy;
}
window.LUNY_fitPreviewDisplay=lunyFitPreviewDisplay;
...
function drawCustomCutlineDetectionLine(ctx,baseW,baseH,cm2px,exportPpi,technicalMarginPx){
...
function renderPrintCanvas(){
  ...
  drawCustomCutlineDetectionLine(octx,basePxW,basePxH,exportCm2Px,spec.actualPpi,spec.technicalMarginPx);
    ...
    try{
      canvas.toBlob(blob=>{
        if(blob&&blob.size>0){
          ...
          reject(new Error(`${label||'Canvas'} 未產生有效 PNG Blob`));
        }
      },'image/png');
    }catch(err){
      reject(err);
...
function lunyReadBlobArrayBuffer(blob){
  if(blob&&typeof blob.arrayBuffer==='function')return blob.arrayBuffer();
  return new Promise((resolve,reject)=>{
    const reader=new FileReader();
    reader.onload=()=>resolve(reader.result);
    reader.onerror=()=>reject(reader.error||new Error('Blob 讀取失敗'));
    reader.readAsArrayBuffer(blob);
  });
}

async function lunyBlobsEqualExact(first,second){
  if(!first||!second||first.size!==second.size)return false;
  const chunkSize=256*1024;
  for(let offset=0;offset<first.size;offset+=chunkSize){
    const end=Math.min(first.size,offset+chunkSize);
    const [firstBuffer,secondBuffer]=await Promise.all([
      lunyReadBlobArrayBuffer(first.slice(offset,end)),
      lunyReadBlobArrayBuffer(second.slice(offset,end))
    ]);
    const a=new Uint8Array(firstBuffer),b=new Uint8Array(secondBuffer);
...
async function lunyAssertDistinctOutputBlobs(printBlob,cutBlob,base){
  if(!printBlob||!cutBlob)return;
  const identical=await lunyBlobsEqualExact(printBlob,cutBlob);
  if(identical){
    console.error('[LUNY] 印刷檔與切割檔 Blob 完全相同，已停止送出',{
...
var LUNY_PREFLIGHT_VERSION='16.9.0-20260917';
var LUNY_FILE_PREP_SKU='FILE_PREP_BASIC';
var LUNY_FILE_PREP_LABEL='印刷檔案基礎整理';
var LUNY_FILE_PREP_FEE=300;
var __lunyPreflightWarningAccepted=false;
var __lunyPreflightWarningAcceptanceKey='';
var __lunyFilePrepSelected=false;
var __lunyAcceptedFitStateKey='';

function lunyResolutionSnapshot(){
  try{
    const api=window.LUNY_RESOLUTION_CHECKER;
    return api&&typeof api.getLastResult==='function'?api.getLastResult():null;
  }catch(e){return null;}
}

function lunyPlacedContentDanger(){
  const W=cG.width,H=cG.height,b=BLEED_CM*CM2PX,gap=GAP_CM*CM2PX;
  const cutW=W-2*b,cutH=H-2*b;
  const safeW=Math.max(1,cutW-2*gap),safeH=Math.max(1,cutH-2*gap);
  let iconDanger=false,textDanger=false;
  if(iconImg){
    iconDanger=rectTouchesDangerZone(
      W/2+iconOffsetX,H/2+iconOffsetY,
      iconImg.width*iconScale,iconImg.height*iconScale,iconAngle,
      safeW,safeH,cutW,cutH
    );
  }
  if(textStr){
    const fontPx=textSizeCM*CM2PX*textScale;
    const measured=measureTextBox(textStr,fontPx);
    textDanger=rectTouchesDangerZone(
      W/2+textOffsetX,H/2+textOffsetY,
      measured.w,measured.h,textAngle,
      safeW,safeH,cutW,cutH
    );
  }
  return{iconDanger,textDanger,any:iconDanger||textDanger};
}

function lunyPhotoBandTransition(x,y,gapPx){
  if(!img)return false;
  const W=cG.width,H=cG.height;
  const dx=x-W/2,dy=y-H/2,length=Math.max(1,Math.hypot(dx,dy));
  const ux=dx/length,uy=dy/length,step=Math.max(3,Number(gapPx||0)*.42);
  const samplePoints=[
    previewPointToPhotoPixel(x,y),
    previewPointToPhotoPixel(x-ux*step,y-uy*step),
    previewPointToPhotoPixel(x+ux*step,y+uy*step)
  ];
  const info=getPhotoPixelInfo();
  if(!info||samplePoints.some(point=>!point))return false;
  const colors=samplePoints.map(point=>getPhotoPixelRGBA(info,point.x,point.y));
  function changed(a,b){
    if(!a||!b)return false;
    if(Math.abs(a[3]-b[3])>=96)return true;
    return a[3]>=32&&b[3]>=32&&(rgbaDiff(a,b)>=78||Math.abs(rgbaLuma(a)-rgbaLuma(b))>=30);
  }
  return changed(colors[0],colors[1])||changed(colors[0],colors[2])||changed(colors[1],colors[2]);
}

function lunyFlattenedPhotoDangerAssessment(){
  if(!img)return{danger:false,caution:false,hits:0,samples:0,ratio:0,detection:'none'};
  const W=cG.width,H=cG.height,b=BLEED_CM*CM2PX,gap=GAP_CM*CM2PX;
  const cutW=W-2*b,cutH=H-2*b;
  const safeW=Math.max(1,cutW-2*gap),safeH=Math.max(1,cutH-2*gap);
  const sectorCount=24,sectorHits=Array(sectorCount).fill(0),sectorSamples=Array(sectorCount).fill(0),transitionSectorHits=Array(sectorCount).fill(0);
  let hits=0,samples=0,transitionHits=0;
  for(let t=0;t<=1.0001;t+=0.125){
    const ww=safeW+(cutW-safeW)*t;
    const hh=safeH+(cutH-safeH)*t;
    const pts=sampleShapeBoundaryPoints(ww,hh,360);
    for(const point of pts){
      if(!pointInDangerRing(point.x,point.y,safeW,safeH,cutW,cutH))continue;
      const angle=Math.atan2(point.y-H/2,point.x-W/2);
      const sector=Math.max(0,Math.min(sectorCount-1,Math.floor((angle+Math.PI)/(Math.PI*2)*sectorCount)));
      samples+=1;
      sectorSamples[sector]+=1;
      const transition=lunyPhotoBandTransition(point.x,point.y,gap);
      if(photoPointHasCompositeContent(point.x,point.y)||transition){
        hits+=1;
        sectorHits[sector]+=1;
        if(transition){transitionHits+=1;transitionSectorHits[sector]+=1;}
      }
    }
  }
  const ratio=samples?hits/samples:0;
  const sectorRatios=sectorHits.map((value,index)=>sectorSamples[index]?value/sectorSamples[index]:0);
  const maxSectorRatio=Math.max.apply(null,sectorRatios);
  let maxWindowRatio=0,hotSectorCount=0;
  sectorRatios.forEach(value=>{if(value>=0.55)hotSectorCount+=1;});
  for(let start=0;start<sectorCount;start++){
    let windowHits=0,windowSamples=0;
    for(let offset=0;offset<3;offset++){
      const index=(start+offset)%sectorCount;
      windowHits+=sectorHits[index];
      windowSamples+=sectorSamples[index];
    }
    if(windowSamples)maxWindowRatio=Math.max(maxWindowRatio,windowHits/windowSamples);
  }
  const broadDanger=samples>=240&&hits>=120&&ratio>=0.55;
  const moderateDenseDanger=samples>=1500&&hits>=600&&ratio>=0.43;
  const localizedDanger=samples>=240&&hits>=48&&maxSectorRatio>=0.62&&maxWindowRatio>=0.50;
  const danger=broadDanger||moderateDenseDanger||localizedDanger;
  const localizedCaution=!danger&&samples>=1500&&hits>=160&&ratio>=0.06&&maxSectorRatio>=0.60&&maxWindowRatio>=0.24&&hotSectorCount>=1;
  const transitionSectorCount=transitionSectorHits.filter(value=>value>=2).length;
  const borderCaution=!danger&&samples>=240&&transitionHits>=36&&transitionHits/samples>=0.01&&transitionSectorCount>=2;
  return{
    danger,caution:localizedCaution||borderCaution,
    hits,samples,ratio,maxSectorRatio,maxWindowRatio,hotSectorCount,transitionHits,transitionSectorCount,
    detection:broadDanger?'broad':(moderateDenseDanger?'moderate':(localizedDanger?'localized-danger':(localizedCaution?'localized-caution':(borderCaution?'border-caution':'none'))))
  };
}

function lunyFlattenedPhotoDanger(){
  return lunyFlattenedPhotoDangerAssessment().danger;
}

function lunyImageBorderWhiteRatio(){
  const info=getPhotoPixelInfo();
  if(!info||!info.w||!info.h)return 0;
  const step=Math.max(1,Math.floor(Math.max(info.w,info.h)/500));
  let total=0,white=0;
  function sample(x,y){
    const rgba=getPhotoPixelRGBA(info,x,y);
    if(!rgba)return;
    total+=1;
    if(rgba[3]<32||(rgba[0]>=242&&rgba[1]>=242&&rgba[2]>=242))white+=1;
  }
  for(let x=0;x<info.w;x+=step){sample(x,0);sample(x,info.h-1);}
  for(let y=step;y<info.h-step;y+=step){sample(0,y);sample(info.w-1,y);}
...
function lunyPhotoCoversBleed(){
  ...
  if(!img)return false;
  // Sample multiple bands; white RGB is valid coverage, transparent pixels are not.
  const bleed=BLEED_CM*CM2PX;
  for(const t of [0,.5,1]){
    const inset=2+bleed*t;
    if(!photoCoversPreviewShape(Math.max(1,cG.width-2*inset),Math.max(1,cG.height-2*inset)))return false;
  }
  return true;
}

function lunyFilePrepEligibleForWhiteMargin(resolution){
  const box=resolution&&resolution.contentBox;
  const whiteMargin=!!(
    box&&box.found&&(
      Number(box.areaRatio||1)<0.9||
      Number(box.sideRatioX||1)<0.96||
      Number(box.sideRatioY||1)<0.96
    )
  );
  return whiteMargin||lunyImageBorderWhiteRatio()>=0.35;
}

function lunyPreflightImageKey(){
  if(!img)return'none';
  ...
  return`${img.width}x${img.height}:${source.length}:${(hash>>>0).toString(36)}`;
}

function lunyPreflightStateKey(){
  const imageKey=lunyPreflightImageKey();
  return[
    imageKey,
    Number(scale||0).toFixed(5),
    Number(offsetX||0).toFixed(2),
    Number(offsetY||0).toFixed(2),
    Number(angle||0).toFixed(5),
    String(shape&&shape.value||''),
    String(wIn&&wIn.value||''),
    String(hIn&&hIn.value||''),
    String(getEdgeOption()||'off'),backgroundColor,eyedropperColor,
    textStr,textSizeCM,textScale,textOffsetX,textOffsetY,textAngle,textFill,
    iconImg?String(iconImg.src||''):'',iconScale,iconOffsetX,iconOffsetY,iconAngle
  ].join('|');
}

function lunyHasAcceptedFitForCurrentState(){
  return !!__lunyAcceptedFitStateKey&&
    __lunyAcceptedFitStateKey===lunyPreflightStateKey();
}

function lunyClearAcceptedFit(){
  __lunyAcceptedFitStateKey='';
}

function lunyWarningAcceptanceKey(kind,stateKey){
  return`${String(kind||'general')}|${String(stateKey||lunyPreflightStateKey())}`;
}

function lunyWarningAcceptedFor(kind){
  return __lunyPreflightWarningAccepted&&
    __lunyPreflightWarningAcceptanceKey===lunyWarningAcceptanceKey(kind);
}

function lunyMakePreflightResult(status,level,title,issues,recommendations,extra){
  return Object.assign({
    version:LUNY_PREFLIGHT_VERSION,
    stateKey:lunyPreflightStateKey(),
    status,level,title,
    issues:issues||[],
    recommendations:recommendations||[],
    checkedAt:new Date().toISOString(),
    canProceed:false,
    productionDisposition:'BLOCKED',
    filePrepEligible:false
  },extra||{});
}

function lunyClassifyPreflight(){
  const resolution=lunyResolutionSnapshot();
  const resolutionLevel=String(resolution&&resolution.level||'unknown');
  const resolutionSummary=resolution?{
    level:resolutionLevel,
    title:String(resolution.title||''),
    wholeDpi:Number(resolution.wholeDpi&&resolution.wholeDpi.effectiveDpi||0),
    contentDpi:Number(resolution.contentDpi&&resolution.contentDpi.effectiveDpi||0),
    pixelWidth:Number(resolution.pixelWidth||0),
    pixelHeight:Number(resolution.pixelHeight||0)
  }:{level:'unknown'};

  if(!img&&!iconImg&&!textStr){
    return lunyMakePreflightResult(
      'BLOCKED_REUPLOAD_REQUIRED','red','請先上傳要製作的圖片',
      ['尚未偵測到可製作的主圖。'],
      ['按「上傳新圖」選擇 JPG 或 PNG 檔案。'],
      {resolution:resolutionSummary}
    );
  }

  if(resolutionLevel==='danger'){
    return lunyMakePreflightResult(
      'BLOCKED_REUPLOAD_REQUIRED','red','解析度不足',
      ['目前檔案放大到此製作尺寸後，細節可能明顯模糊或產生鋸齒。'],
      ['改用原始設計檔重新輸出，建議至少 300 DPI。','不要使用截圖或通訊軟體壓縮後的圖片。','若需要重繪或重新排版，請聯繫客服另行報價。'],
      {resolution:resolutionSummary,uiStatusLabel:'解析度不足'}
    );
  }
  ...
  if(shape.value==='custom'){
    ...
    if(customStatus.level>=2){
      return lunyMakePreflightResult(
        'BLOCKED_AUTO_FIXABLE','red','客製刀線尚未通過檢查',
        [customStatus.title],
        ['簡化過細或過尖的圖案，再重新產生刀線。','無法簡化時請重新上傳輪廓較清楚的 PNG。'],
        {resolution:resolutionSummary}
      );
    }
  }

  const placed=lunyPlacedContentDanger();
  if(placed.any){
    const names=[];
    if(placed.textDanger)names.push('新增文字');
    if(placed.iconDanger)names.push('QR Code 或圖示');
    return lunyMakePreflightResult(
      'BLOCKED_AUTO_FIXABLE','red','重要內容太靠近裁切區',
      [`${names.join('、')}碰到裁切線內側 2.5 mm 的安全帶。文字、Logo、QR Code、條碼、電話、臉部、主體或獨立圖形不得進入此區。`],
      ['按「移回安全區」讓系統將新增內容移到較內側。','或自行縮小並往內移，再重新檢查。'],
      {resolution:resolutionSummary,autoFix:'MOVE_IMPORTANT_CONTENT'}
    );
  }

  const flattenedRisk=lunyFlattenedPhotoDangerAssessment();
  const fitAccepted=lunyHasAcceptedFitForCurrentState();
  if(!lunyPhotoCoversBleed()){
    return lunyMakePreflightResult(
      'BLOCKED_FILE_PREP_ELIGIBLE','red','請選擇貼紙邊緣效果',
      ['圖片尚未覆蓋印刷所需的外圍，請選擇你希望的成品效果。',
       '看起來是白色，不一定代表圖片已填滿；想讓邊緣維持白色，可選「確認保留白色邊緣」。'],
      ['確認保留白色邊緣：外圈會補成白色，請確認圖文沒有被覆蓋。',
       '用背景色補滿：先套用建議色，可再用色票調整。',
       '放大圖片填滿：等比例放大並置中，請確認邊緣內容；透明缺口仍需補白或補色。'],
      {resolution:resolutionSummary,flattenedRisk,filePrepEligible:true,
       uiStatusLabel:'選擇邊緣效果',edgeChoiceRequired:true,
       filePrepScope:['CENTER_AND_SCALE','SOLID_COLOR_BLEED'],
       filePrepDescription:'含整張圖等比例縮放、置中與單色補邊；不含拆字、改字、移動單一物件、重排或重繪。整理完成後提供預覽確認，製作天數從確認後起算。',
       autoFix:'FIT_TO_BLEED'}
    );
  }

  if(flattenedRisk.danger){
    return lunyMakePreflightResult(
      'BLOCKED_FILE_PREP_ELIGIBLE',
      'yellow',
      '確認清晰度',
      ['裝飾圖案、細線或邊框進入 2.5 mm 安全帶；單張合成圖片也可能無法辨別哪些內容重要。'],
      ['縮小圖片或往內移，讓重要圖文避開標示區域。','可選「確認保留白色邊緣」或「用背景色補滿」，再確認圖文完整。','不方便自行調整時，可加購印刷檔案基礎整理。'],
      {
        resolution:resolutionSummary,
        flattenedRisk,
        ...
        filePrepScope:['CENTER_AND_SCALE','SOLID_COLOR_BLEED'],
        filePrepDescription:'含整張圖等比例縮小、置中與單色補邊；不含拆字、改字、移動單一物件、重排或重繪。整理完成後提供預覽確認，製作天數從確認後起算。'
      }
    );
  }


  if(flattenedRisk.caution){
    return lunyMakePreflightResult(
      'BLOCKED_FILE_PREP_ELIGIBLE',
      'yellow',
      '確認清晰度',
      ['局部裝飾、細線或邊框接近 2.5 mm 安全帶，系統無法確定它是否為重要內容。'],
      ['縮小圖片或往內移，讓重要圖文避開標示區域。','可選「確認保留白色邊緣」或「用背景色補滿」，再確認圖文完整。','不方便自行調整時，可加購印刷檔案基礎整理。'],
      {
        resolution:resolutionSummary,
        flattenedRisk,
  ...
  if(resolutionLevel==='warning'||resolutionLevel==='unknown'){
    return lunyMakePreflightResult(
      'BLOCKED_FILE_PREP_ELIGIBLE',
      'yellow',
      '確認清晰度',
      [resolutionLevel==='warning'?'細節、小字或 QR Code 可能不如螢幕顯示清楚。':'系統目前無法取得完整解析度判定。'],
      ['改用更清晰的原始檔。','縮小製作尺寸，或縮小圖片後加白邊／邊框色。','不方便自行調整時，可加購印刷檔案基礎整理。'],
      {
        resolution:resolutionSummary,
        flattenedRisk,
        ...
        filePrepEligible:true,
        filePrepScope:['CENTER_AND_SCALE','SOLID_COLOR_BLEED'],
        filePrepDescription:'含整張圖等比例縮小、置中與單色補邊；不含重繪、改字或提升原檔解析度。整理完成後提供預覽確認，製作天數從確認後起算。'
      }
    );
  }
  ...
  if(fitAccepted){
    return lunyMakePreflightResult(
      'READY','green','已裁掉白邊並放大，可直接製作',[],
      ['已依圖片有效內容裁掉外圍白邊並放大至出血範圍。','此方案不需要加購印刷檔案基礎整理。'],
      {resolution:resolutionSummary,flattenedRisk,canProceed:true,productionDisposition:'AUTO_RELEASE',fitAccepted:true,edgeChoice:{mode:'fit',label:'裁掉白邊並放大',color:'#ffffff'}}
    );
  }
  ...
  if(['on','white','whiteedge','white-edge','border','edge'].includes(edgeMode)){
    return lunyMakePreflightResult(
      'READY','green','已選擇保留白邊',[],
      ['外圈已補成白色；請確認文字、Logo 與圖案完整，沒有被白邊覆蓋。','印刷檔與切割圖會使用相同的白邊設定。'],
      {resolution:resolutionSummary,flattenedRisk,canProceed:true,productionDisposition:'AUTO_RELEASE',edgeChoice:{mode:'white',label:'保留白邊',color:'#ffffff'}}
    );
  }
  if(['color','edgecolor','fullcolor'].includes(edgeMode)){
    ...
    return lunyMakePreflightResult(
      ...
      ['外圈已使用此顏色補滿；請確認圖文完整與顏色符合預期。','可點預覽下方的「邊緣色」色票更換顏色。'],
      {resolution:resolutionSummary,flattenedRisk,canProceed:true,productionDisposition:'AUTO_RELEASE',edgeChoice:{mode:'color',label:'單色補出血',color:selectedColor}}
    );
  }
  return lunyMakePreflightResult(
    'READY','green','檔案檢查完成，可直接製作',[],
    ['2.5 mm 安全帶內只有可裁切的背景、底色或無關緊要紋理，未偵測到細框、文字或獨立圖形。','可直接付款並自動進入製作。'],
    {resolution:resolutionSummary,flattenedRisk,canProceed:true,productionDisposition:'AUTO_RELEASE'}
  );
}

function lunyGetPreflightResult(){
  const result=lunyClassifyPreflight();
  // Geometric risk is independent of approval and other blocking issues.
  result.cutRiskDetected=!!((img||iconImg||textStr)&&(
    lunyPlacedContentDanger().any||
    ((risk)=>risk.danger||risk.caution)(result.flattenedRisk||lunyFlattenedPhotoDangerAssessment())
  ));
  ...
  const selectedScope=Array.isArray(result.filePrepScope)&&result.filePrepScope.length
    ?result.filePrepScope
    :defaultScope;
  result.filePrep={
    selected,
    sku:LUNY_FILE_PREP_SKU,
    label:LUNY_FILE_PREP_LABEL,
    fee:selected?LUNY_FILE_PREP_FEE:0,
    unitFee:LUNY_FILE_PREP_FEE,
    status:selected?'paid_pending':'not_selected',
    scope:selected?selectedScope.slice():[],
    description:result.filePrepDescription||'含白邊裁除、置中、比例調整與單色出血；不含重繪、改字或重排。'
  };
  if(selected)result.canProceed=true;
  return result;
}
...
function lunyResetArtworkDecisionState(){
  __lunyPreflightWarningAccepted=false;
  __lunyPreflightWarningAcceptanceKey='';
  __lunyFilePrepSelected=false;
  lunyClearAcceptedFit();
  window.LUNY_EDGE_FILL_MODE='off';
  window.LUNY_EDGE_COLOR='';
  window.LUNY_EYEDROPPER_COLOR='';
  eyedropperColor='';
  backgroundColor='';
  const off=document.querySelector('input[name="edgeOption"][value="off"]');
  ...
  document.dispatchEvent(new CustomEvent('luny:newArtworkStarted'));
}
window.LUNY_resetArtworkDecisionState=lunyResetArtworkDecisionState;
imgInput.addEventListener('change',function(event){
  if(event.target===imgInput&&imgInput.files&&imgInput.files[0])lunyResetArtworkDecisionState();
},true);

function lunyResetEdgeChoice(){
  lunyClearAcceptedFit();
  eyedropperColor='';
  window.LUNY_EYEDROPPER_COLOR='';
  window.LUNY_EDGE_COLOR='';
...
document.addEventListener('change',function(event){
  ...
  const mode=String(target.value||'off').toLowerCase();
  window.LUNY_EDGE_FILL_MODE=mode;
  if(!lunyEdgeColorSyncing&&mode==='color'){
    const color=document.getElementById('edgeColor')?.value||'#ffffff';
    lunySyncEdgeColorState('color',color);
  }
  const enabled=document.getElementById('edgeColorEnabled');
  if(enabled)enabled.checked=mode==='color';
  updateColorToolUI();
...
function lunyFitPhotoContentToBleed(){
  if(!img)return false;
  scale=Math.max(cG.width/Math.max(1,img.width),cG.height/Math.max(1,img.height))*1.005;
  offsetX=0;offsetY=0;
  angle=0;
  activeTarget='photo';
  window.LUNY_EDGE_FILL_MODE='off';
...
function lunyShrinkPhotoIntoSafeArea(){
  if(!img)return false;
  const W=cG.width,H=cG.height,b=BLEED_CM*CM2PX,gap=GAP_CM*CM2PX;
  const safeW=Math.max(1,W-2*b-2*gap),safeH=Math.max(1,H-2*b-2*gap);
  scale=Math.min(safeW/Math.max(1,img.width),safeH/Math.max(1,img.height))*.96;
  offsetX=0;offsetY=0;angle=0;activeTarget='photo';
  __lunyPreflightWarningAccepted=false;
  __lunyPreflightWarningAcceptanceKey='';
  __lunyFilePrepSelected=false;
  lunySetEdgeOption('on');
  drawPreview();
  return true;
}

function lunyMovePlacedContentInsideSafeArea(){
  const W=cG.width,H=cG.height,b=BLEED_CM*CM2PX,gap=GAP_CM*CM2PX;
  const safeW=Math.max(1,W-2*b-2*gap),safeH=Math.max(1,H-2*b-2*gap);
  if(iconImg){
    const factor=Math.min(1,(safeW*.9)/(iconImg.width*iconScale),(safeH*.9)/(iconImg.height*iconScale));
    iconScale*=factor;iconOffsetX=0;iconOffsetY=0;
  }
  if(textStr){
    const measured=measureTextBox(textStr,textSizeCM*CM2PX*textScale);
    const factor=Math.min(1,(safeW*.9)/measured.w,(safeH*.9)/measured.h);
    textScale*=factor;textOffsetX=0;textOffsetY=0;
  }
  __lunyPreflightWarningAccepted=false;
  __lunyPreflightWarningAcceptanceKey='';
  window.lunyResetBleedRiskDecision();
...
function lunyEnsurePreflightPanel(){
  let panel=document.getElementById('lunyPreflightPanel');
  if(panel)return panel;
  panel=document.createElement('section');
  panel.id='lunyPreflightPanel';
  panel.setAttribute('aria-live','polite');
  panel.style.cssText='max-width:520px;margin:12px auto;padding:14px 16px;border:1px solid #cbd5e1;border-radius:8px;background:#fff;color:#1f2937;text-align:left;box-sizing:border-box;font-size:13px;line-height:1.55;';
  const legacy=document.getElementById('lunyBleedRiskBox');
  if(legacy)legacy.style.display='none';
  const anchor=document.getElementById('edgeChoiceUI')||cG;
  anchor.insertAdjacentElement('beforebegin',panel);
  return panel;
}

function lunyPreflightButton(label,action,primary){
  return `<button type="button" data-luny-preflight-action="${action}" style="min-height:38px;padding:8px 11px;border-radius:6px;border:1px solid ${primary?'#111827':'#cbd5e1'};background:${primary?'#111827':'#ffffff'};color:${primary?'#ffffff':'#111827'};font-weight:700;cursor:pointer;">${label}</button>`;
}

var __lunyPreflightDetailsOpenKey='';
var __lunyFilePrepDescriptionOpen=false;

function lunyEnsurePreflightStyles(){
  if(document.getElementById('lunyPreflightTrafficLightStyles'))return;
  const style=document.createElement('style');
  style.id='lunyPreflightTrafficLightStyles';
  style.textContent='@keyframes lunyPreflightBlink{0%,49%{opacity:1}50%,100%{opacity:.28}}#lunyPreflightPanel .luny-preflight-manual-copy{color:#e60012;font-weight:900;animation:lunyPreflightBlink 1s steps(1,end) infinite}#lunyPreflightPanel button:focus-visible,#lunyPreflightPanel input:focus-visible{outline:3px solid rgba(37,99,235,.28);outline-offset:2px}';
  document.head.appendChild(style);
}

function lunyRenderPreflight(result){
  const panel=lunyEnsurePreflightPanel();
  lunyEnsurePreflightStyles();
  panel.dataset.preflightStatus=String(result&&result.status||'');
  panel.dataset.preflightLevel=String(result&&result.level||'');
  panel.dataset.preflightVersion=String(result&&result.version||'');
  panel.dataset.preflightStateKey=String(result&&result.stateKey||'');
  panel.dataset.preflightDiagnostics=JSON.stringify({
    flattenedRisk:result&&result.flattenedRisk||null,
    resolution:result&&result.resolution||null
  });
  const colors=result.level==='green'
    ?{border:'#238b57',background:'#f1faf4',text:'#17643e',chip:'#d9f2e1'}
    :(result.level==='yellow'
      ?{border:'#f25936',background:'#fff7f4',text:'#c83e1e',chip:'#ffe0d7'}
      :{border:'#d90429',background:'#fff4f5',text:'#b00020',chip:'#ffdce0'});
  panel.dataset.status=result.status;
  panel.style.setProperty('border-color',colors.border,'important');
  panel.style.setProperty('background',colors.background,'important');
  ...
  const detailKey=String(result.stateKey||result.status||'');
  const forceDetailsOpen=!!result.edgeChoiceRequired||String(result.uiStatusLabel||result.title||'')==='解析度不足';
  const detailsOpen=forceDetailsOpen||__lunyPreflightDetailsOpenKey===detailKey;
  const hasDetails=!!(issueHtml||adviceHtml);
  const detailHtml=hasDetails
    ?`<div id="lunyPreflightDetails" ${detailsOpen?'':'hidden'} style="margin-top:9px;padding:9px 10px;border:1px solid ${colors.border};border-radius:7px;background:#fff;color:#1f2937;">${issueHtml?`<ul style="margin:0 0 ${adviceHtml?'8px':'0'} 18px;padding:0;">${issueHtml}</ul>`:''}${adviceHtml?`<div style="font-weight:900;margin-bottom:4px;">調整建議：</div><ol style="margin:0 0 0 18px;padding:0;">${adviceHtml}</ol>`:''}</div>`
    :'';
  const choiceHtml=result.edgeChoice
    ...
    : '';
  const actions=[];
  const hasArtwork=!!(img||iconImg||textStr);
  if(result.autoFix==='MOVE_IMPORTANT_CONTENT')actions.push(lunyPreflightButton('移回安全區','safe',true));
  if(result.level==='yellow')actions.push(lunyPreflightButton('縮小圖片','shrink',true));
  if(hasArtwork&&String(result.uiStatusLabel||'')!=='解析度不足'){
    actions.push(lunyPreflightButton('確認保留白色邊緣','white',false));
    actions.push(lunyPreflightButton('用背景色補滿','color',false));
    actions.push(lunyPreflightButton('放大圖片填滿','fit',false));
  }
  ...
  const clarityConfirmationHtml=result.requiresClarityConfirmation
    ?`<label style="display:flex;gap:8px;align-items:flex-start;margin-top:10px;font-weight:900;"><input id="lunyPreflightAcceptWarning" type="checkbox" ${result.clarityAccepted?'checked':''} style="margin-top:4px;"><span class="luny-preflight-manual-copy">已確認圖文皆清晰無誤，可印刷${lunyHasCutRisk(result)?'；確認風險區內容可被裁切':''}</span></label>`
    :'';
  let decision='';
  if(result.filePrepEligible){
    ...
    decision+=`<div style="margin-top:12px;padding-top:12px;border-top:1px solid ${colors.border};"><div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px;"><label style="display:flex;gap:8px;align-items:flex-start;font-weight:800;"><input id="lunyFilePrepBasic" type="checkbox" ${__lunyFilePrepSelected?'checked':''} style="margin-top:4px;"><span>加購${LUNY_FILE_PREP_LABEL}　NT$${LUNY_FILE_PREP_FEE}／款</span></label><button id="lunyFilePrepDescriptionToggle" type="button" style="padding:3px 8px;border:1px solid ${colors.border};border-radius:999px;background:#fff;color:${colors.text};font-weight:800;">${__lunyFilePrepDescriptionOpen?'收合':'說明'}</button></div><div id="lunyFilePrepDescription" ${__lunyFilePrepDescriptionOpen?'':'hidden'} style="margin:6px 0 0 24px;font-size:12px;">${prepDescription}</div></div>`;
  }
  ...
  panel.querySelectorAll('[data-luny-preflight-action]').forEach(button=>{
    button.addEventListener('click',function(){
      ...
      if(action==='shrink')lunyShrinkPhotoIntoSafeArea();
  ...
  const prepToggle=panel.querySelector('#lunyFilePrepDescriptionToggle');
  if(prepToggle)prepToggle.addEventListener('click',function(){__lunyFilePrepDescriptionOpen=!__lunyFilePrepDescriptionOpen;lunyRenderPreflight(result);});
}

function lunyUpdatePreflight(){
  const result=__lunyInteractiveDraw
    ? lunyMakePreflightResult(
        'CHECKING','yellow','正在重新檢查圖片位置',[],
        ['完成移動或縮放後，系統會立即重新檢查安全距離與出血。'],
        {resolution:lunyResolutionSnapshot(),flattenedRisk:null,canProceed:false}
      )
    : lunyGetPreflightResult();
  // Keep the last completed decision while a new check is in progress.
  if(!img&&!iconImg&&!textStr)__lunyLastConfirmedCutRisk=false;
  else if(result.status!=='CHECKING')__lunyLastConfirmedCutRisk=lunyHasCutRisk(result);
  result.cutRiskDetected=__lunyLastConfirmedCutRisk;
  lunyRenderPreflight(result);
  window.__LUNY_PREFLIGHT_LAST_RESULT__=result;
  document.dispatchEvent(new CustomEvent('luny:preflightChanged',{detail:result}));
  return result;
}

window.LUNY_getPreflightResult=lunyGetPreflightResult;
window.LUNY_updatePreflight=lunyUpdatePreflight;
// Async image/resolution checks can finish after the upload preview was drawn.
// Refresh only the warning layer from the latest result; never trigger another
// preflight or change the artwork transform from this event.
let __lunyRiskResultFrame=0;
document.addEventListener('luny:preflightChanged',function(){
  if(__lunyRiskResultFrame)return;
  __lunyRiskResultFrame=requestAnimationFrame(function(){
    __lunyRiskResultFrame=0;
    if(!cG||!cG.isConnected)return;
    drawImportantSafetyRiskMask(ctxG,cG,CM2PX);
  });
});

window.LUNY_fixWhiteMarginAndFillBleed=lunyFitPhotoContentToBleed;
window.LUNY_shrinkPhotoIntoSafeArea=lunyShrinkPhotoIntoSafeArea;
window.LUNY_moveImportantContentInside=lunyMovePlacedContentInsideSafeArea;
window.LUNY_PREFLIGHT_VERSION=LUNY_PREFLIGHT_VERSION;
window.lunyConfirmBleedBeforeSave=function(){
  // 儲存按鈕可能與解析度檢查同時執行；先以目前設計狀態重新分類，
  // 再更新面板，避免非設計變更造成不一致的重新分類。
  const result=lunyGetPreflightResult();
  lunyRenderPreflight(result);
  window.__LUNY_PREFLIGHT_LAST_RESULT__=result;
  document.dispatchEvent(new CustomEvent('luny:preflightChanged',{detail:result}));
  ...
  const panel=document.getElementById('lunyPreflightPanel');
  if(panel)panel.scrollIntoView({behavior:'smooth',block:'center'});
  try{if(typeof setSaveStatus==='function')setSaveStatus(result.title);}catch(e){}
  return false;
};
document.addEventListener('luny:resolutionChecked',function(){
  // 解析度結果更新只應撤銷「解析度警告」的確認。
  // 僅清除舊版解析度警告狀態，不影響目前的檔案整理選擇。
  if(String(__lunyPreflightWarningAcceptanceKey||'').startsWith('resolution|')){
    __lunyPreflightWarningAccepted=false;
    __lunyPreflightWarningAcceptanceKey='';
    window.lunyResetBleedRiskDecision();
  }
...
/* Cut proof and print now share the same production bleed render. Guides are
   added only after the shared artwork has been produced. */
function renderExportCanvas(includeGuides,useFullImage,productionMode){
  const size=lunyGetEffectiveSizeCm();
  const spec=lunyResolveExportSpec(size.widthCm,size.heightCm,!!productionMode);
  const exportCm2Px=spec.actualPpi/2.54;
  const out=document.createElement('canvas');
  out.width=spec.pxW;out.height=spec.pxH;
  out.__lunyExportPpi=spec.actualPpi;out.__lunyExportSpec=spec;
  ...
  drawAll(octx,out,exportCm2Px,{
    ...
    autoBleedForPrint:!!productionMode
  });
  return out;
}

/* Public, lightweight finished-sticker source for packaging/application and
   checkout previews. Keep its white-edge/crop presentation in the same order
   as drawPreview(), while still excluding guides and selection handles. */
function lunyRenderCleanFinishedPreviewCanvas(){
  ...
  const cm2px=Math.max(1,Number(source.__lunyExportPpi)||300)/2.54;
  drawProductWhiteEdgePreview(context,source,cm2px);
  drawProductPreviewCrop(context,source,cm2px);
  return source;
}
window.LUNY_renderCleanPreviewCanvas=lunyRenderCleanFinishedPreviewCanvas;
window.LUNY_captureCleanPreviewDataUrl=function(maxSide){
  const source=lunyRenderCleanFinishedPreviewCanvas();
...
window.getPrintAndCutBlobs=async function(){
  ...
  let printBlob=null,cutBlob=null;
  let printSpec=null,cutSpec=null;

  try{
    // 先產生印刷檔並立即轉成 Blob，完成後釋放大型 Canvas，
    ...
    printBlob=await lunyCanvasToPngBlob(printCanvas,'印刷檔');
    ...
    cutBlob=await lunyCanvasToPngBlob(cutCanvas,'切割檔');
  ...
  if(!printBlob||!cutBlob){
    throw new Error("Missing print/cut Blob");
  }

  await lunyAssertDistinctOutputBlobs(printBlob,cutBlob,base);
  ...
  return{
    print:{
      filename:`${base}_印刷檔.png`,blob:printBlob,contentType:'image/png',sizeBytes:printBlob.size,
    ...
    cut:{
      filename:`${base}_切割檔.png`,blob:cutBlob,contentType:'image/png',sizeBytes:cutBlob.size,
...
/* LUNY 滿版底色 Phase 3：正式輸出由 drawAll/renderPrintCanvas 直接繪製；此區只保留預覽 overlay 與小縮圖裝飾。 */
/* LUNY 滿版底色修正版 v7.9：
   修正：
   1. 不再把滿版底色直接補畫到 canvasGuides 本體，避免縮放/拖曳時閃爍。
   2. 改用獨立透明 overlay canvas，滿版底色與提示文字固定在上層。
   3. 正式 print/cut 的滿版底色由 drawAll/renderPrintCanvas 直接寫入，不再攔截 toDataURL。
   4. 切割線預覽圖也會套用滿版底色，避免儲存設計後預覽與畫面不一致。
   5. 儲存設計縮圖改由明確的縮圖裝飾函式套用滿版底色，不修改 Canvas prototype。
   6. Phase 3 正式輸出使用 getPrintAndCutBlobs，不再建立大型 Base64 字串。
   7. 滿版底色可預設透明，讓已自行預留出血的客戶不需要再額外套色。 */
(function(){
  const BLEED_CM = 0.2;
  const GAP_CM = 0.25;
  const OVERLAY_ID = 'lunyFullBleedOverlayCanvas';
  const OVERLAY_MAX_SIDE_PX = 1800;
  const OVERLAY_MAX_TOTAL_PIXELS = 3200000;
  ...
  function drawEdgeRing(ctx,W,H,cm2px,color,withGuideLines,withWarning){
    ...
    if(color){
      ...
      ctx.fillStyle = color;
      ctx.beginPath();
      addShapePath(ctx,shapeValue,cx,cy,cutW,cutH,cm2px);
      // 與 applySelectedEdgeColor／正式印刷規則一致：填滿綠色安全線以外區域。
    ...
    if(withGuideLines){
      ...
      if(!color)stroke(bleedW,bleedH,'#888888',4,[8,8]);
  ...
  function renderMatchedPreviewCanvas(maxSide){
    const source=$('canvasGuides');
    if(!source||!source.width||!source.height)return null;
    const requested=Math.max(160,Math.min(1200,Number(maxSide)||960));
    // 直接依目前設計狀態重繪，因此可安全輸出 960px，不是把低解析畫布硬放大。
    const ratio=requested/Math.max(source.width,source.height);
    ...
    const widthCm=Math.max(1,parseFloat($('widthCm')?.value||'5'));
    const cm2px=width/(widthCm+BLEED_CM*2);
    drawAll(ctx,out,cm2px,{
      includeGuides:false,
      includeSelection:false,
    ...
    try{drawProductWhiteEdgePreview(ctx,out,cm2px);}catch(e){}
    try{drawProductPreviewCrop(ctx,out,cm2px);}catch(e){}
    // 裁切遮罩可能蓋住主畫布外圈，因此在最上層補回綠色安全線外的邊緣色。
    drawEdgeRing(ctx,width,height,cm2px,getEdgeColor(),false,false);
    return out;
  }
  window.LUNY_renderMatchedPreviewCanvas=renderMatchedPreviewCanvas;

  function decoratePreviewThumbCanvas(canvas){
    if(!canvas||!isFullBleedMode())return canvas;
    if(window.__lunyFullBleedShouldApplyColor&&!window.__lunyFullBleedShouldApplyColor())return canvas;
    const ctx=canvas.getContext&&canvas.getContext('2d');
    if(!ctx)return canvas;
    const wcm=Math.max(1,parseFloat($('widthCm')?.value||'5'));
    const cm2px=(canvas.width||1)/(wcm+BLEED_CM*2);
    drawEdgeRing(ctx,canvas.width,canvas.height,cm2px,getEdgeColor(),false,false);
    return canvas;
  }
  window.LUNY_decoratePreviewThumbCanvas=decoratePreviewThumbCanvas;
  ...
  function syncOverlayPosition(){
    ...
    if(maxSide>OVERLAY_MAX_SIDE_PX)limitScale=Math.min(limitScale,OVERLAY_MAX_SIDE_PX/maxSide);
    const totalPixels = targetW*targetH;
    if(totalPixels>OVERLAY_MAX_TOTAL_PIXELS)limitScale=Math.min(limitScale,Math.sqrt(OVERLAY_MAX_TOTAL_PIXELS/totalPixels));
    if(limitScale<1){
      targetW=Math.max(1,Math.round(targetW*limitScale));
      targetH=Math.max(1,Math.round(targetH*limitScale));
    }
  ...
  function renderOverlay(){
    const canvas = $('canvasGuides');
    if(!canvas)return;
    if(!isFullBleedMode()||(window.__lunyFullBleedShouldApplyColor&&!window.__lunyFullBleedShouldApplyColor())){
      const existingOverlay=$(OVERLAY_ID);
      if(existingOverlay){
        existingOverlay.style.display='none';
        if(existingOverlay.width!==1)existingOverlay.width=1;
        if(existingOverlay.height!==1)existingOverlay.height=1;
      }
      return;
    }
    ...
    drawEdgeRing(ctx,overlay.width,overlay.height,cm2px,getEdgeColor(),!!$('safetyGuideToggle')?.checked,false);
  ...
  function installPreviewThumbPatch(){
    ...
    if(typeof originalMakePreviewThumb==='function'){
      if(originalMakePreviewThumb.__lunyBackendPreviewMatchV1){
        thumbReady=true;
      }else{
        const patchedThumb=function(maxSize,quality){
          const requested=Number(maxSize);
          const safeMax=Math.max(160,Math.min(360,requested>0?requested:360));
          const safeQuality=Math.max(0.72,Math.min(0.86,Number(quality)||0.82));
          try{if(typeof window.drawPreview==='function')window.drawPreview();}catch(e){}
          const out=renderMatchedPreviewCanvas(safeMax);
          return out?out.toDataURL('image/jpeg',safeQuality):originalMakePreviewThumb.call(this,safeMax,safeQuality);
        };
        patchedThumb.__lunyBackendPreviewMatchV1=true;
        patchedThumb.__lunyFullBleedThumbPatchedV7=true;
        patchedThumb.__lunyFullBleedThumbPatchedV7947=true;
        window.makePreviewThumb=patchedThumb;
        thumbReady=true;
      }
    }

    let blobReady=false;
    const originalMakePreviewThumbBlob=window.makePreviewThumbBlob_;
    if(typeof originalMakePreviewThumbBlob==='function'){
      if(originalMakePreviewThumbBlob.__lunyBackendPreviewMatchV1){
        blobReady=true;
      }else{
        const patchedBlob=async function(maxSize,quality){
          try{if(typeof window.drawPreview==='function')window.drawPreview();}catch(e){}
          const requested=Math.max(960,Number(maxSize)||0);
          const out=renderMatchedPreviewCanvas(requested);
          if(!out)return originalMakePreviewThumbBlob.call(this,maxSize,quality);
          const widthPx=out.width,heightPx=out.height;
          const safeQuality=Math.max(0.84,Math.min(0.92,Number(quality)||0.86));
          const blob=await canvasToBlobSafe(out,'image/jpeg',safeQuality);
          if(!blob||!blob.size)throw new Error('預覽圖轉換失敗，未產生有效圖檔');
          try{out.width=1;out.height=1;}catch(e){}
          return {blob,contentType:'image/jpeg',widthPx,heightPx,sizeBytes:blob.size};
        };
        patchedBlob.__lunyBackendPreviewMatchV1=true;
        window.makePreviewThumbBlob_=patchedBlob;
        blobReady=true;
      }
    }
    return thumbReady&&blobReady;
  }
...
/* V9：印刷材質效果不得回退到 canvasGuides，避免把刀線、尺寸與警示一起套入材質圖。 */
(function(){
  if(window.__LUNY_MATERIAL_CLEAN_SOURCE_V9__) return;
  window.__LUNY_MATERIAL_CLEAN_SOURCE_V9__=true;

  function captureMaterialFinishedPreview(){
    if(typeof window.LUNY_captureCleanPreviewDataUrl!=='function'){
      throw new Error('LUNY_MATERIAL_CLEAN_PREVIEW_NOT_READY');
    }
    return window.LUNY_captureCleanPreviewDataUrl(1200);
  }
  captureMaterialFinishedPreview.__lunyMaterialCleanSourceV9=true;

  function installMaterialCleanSource(){
    var api=window.LUNY_LABEL_MATERIAL_PREVIEW;
    if(!api)return false;
    if(api.captureFinalDesign!==captureMaterialFinishedPreview){
      api.captureFinalDesign=captureMaterialFinishedPreview;
    }
    return true;
  }

  function installWithRetry(remaining){
    if(installMaterialCleanSource()||remaining<=0)return;
    window.setTimeout(function(){installWithRetry(remaining-1);},120);
  }

  installWithRetry(40);
  document.addEventListener('DOMContentLoaded',function(){installWithRetry(40);});
  window.addEventListener('load',function(){
    installWithRetry(40);
    window.setTimeout(function(){installWithRetry(10);},700);
  });
})();

(function(){
  let pending=false;
  function schedule(){if(pending)return;pending=true;requestAnimationFrame(function(){pending=false;if(window.LUNY_fitPreviewDisplay)window.LUNY_fitPreviewDisplay();});}
  function bind(){const frame=document.getElementById('lunyPreviewCanvasFrame');if(frame&&window.ResizeObserver)new ResizeObserver(schedule).observe(frame);schedule();}
