/**
 * LUNY 標籤貼紙頁｜四段預覽＋印前說明收合（新檔，不覆蓋既有程式）
 * Repo：lunytw-collab/luny-tool
 * 檔名：luny-label-preview-preflight-ux-20260922a.js
 *
 * 1shop 頁尾加一行（建議接在 upload-preview-ux-a 之後；可拿掉 upload-preview-ux-e 的雙預覽）：
 * <script src="https://cdn.jsdelivr.net/gh/lunytw-collab/luny-tool@main/luny-label-preview-preflight-ux-20260922a.js?v=20260922a"></script>
 *
 * A. 預覽四選一：成品預覽／刀線檢查／印刷材質效果／實貼效果
 *    （接既有 lunyFinishedPreviewSwitch、材質預覽、實貼預覽）
 * B. 印前檢查：完整按鈕保留；詳細說明預設收合，可展開
 * C. 不改 canProceed、不改報價、不改產檔 DPI
 */
(function () {
  "use strict";

  if (window.__LUNY_LABEL_PREVIEW_PREFLIGHT_UX_20260922A__) return;
  window.__LUNY_LABEL_PREVIEW_PREFLIGHT_UX_20260922A__ = true;
  ...
  function injectStyle() {
    if ($("lunyPreviewPreflightUxStyle20260922a")) return;
    var style = document.createElement("style");
    style.id = "lunyPreviewPreflightUxStyle20260922a";
    style.textContent =
      /* 藏舊雙預覽列 */
      ...
      /* 四段預覽 tab */
      "#lunyFinishedPreviewSwitch .luny-finished-preview-switch{display:flex;flex-wrap:wrap;gap:8px;align-items:center;}" +
      "#lunyFinishedPreviewSwitch .luny-finished-preview-tab{appearance:none;border:1px solid #d8d0c3;background:#fff;color:#2a2622;border-radius:999px;padding:8px 12px;font:inherit;font-size:13px;font-weight:800;cursor:pointer;line-height:1.2;}" +
      "#lunyFinishedPreviewSwitch .luny-finished-preview-tab[aria-selected='true']{border-color:#8b654d;background:#ffdc4d;color:#101010;}" +
      "#lunyFinishedPreviewSwitch .luny-finished-preview-tab:disabled{opacity:.45;cursor:not-allowed;}" +
      "#lunyFinishedPreviewSwitch .luny-finished-preview-helper{flex:1 1 100%;margin:4px 0 0;font-size:12px;color:#6f6860;line-height:1.5;}" +
      /* 成品預覽：藏輔助線／風險標 */
      ...
      "#lunyPreflightDetailsToggle20260922a{appearance:none;margin:8px 0 0;border:0;background:transparent;color:#6f6860;font:inherit;font-size:12px;font-weight:700;text-decoration:underline;cursor:pointer;padding:0;}" +
  ...
  function simplifyPreflight() {
    ...
    var toggle = $("lunyPreflightDetailsToggle20260922a");
    if (!toggle) {
      toggle = document.createElement("button");
      toggle.type = "button";
      toggle.id = "lunyPreflightDetailsToggle20260922a";
    ...
    // 一行短說明
    var short = $("lunyPreflightShort20260922a");
    if (!short) {
      short = document.createElement("p");
      short.id = "lunyPreflightShort20260922a";
