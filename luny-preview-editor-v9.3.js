/* LUNY 預覽主程式整合版 v7.9.22-danger-any-hit-blur-fill
 ...
 * v7.9.22：危險區域只要碰到圖文內容就顯示；補滿版邊緣改為抓安全範圍外 1mm，以模糊填滿補到裁切線與裁切線外 2mm；預覽只顯示成品效果。
 */
...
function applyMirrorBleedFromCutLine(ctx,canvas,cm2px){
  ...
  const scales=[1.00,1.02,1.05,1.09,1.14,1.20];
  scales.forEach((s,i)=>{
    ctx.save();
    ctx.filter=`blur(${(blurBase*(1+i*0.35)).toFixed(2)}px)`;
    ctx.translate(cx,cy);
    ctx.scale(s,s);
