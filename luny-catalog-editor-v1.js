...
function getCatalogQuoteDirect(){
  ...
  const materialText=material==="pearlescent"?"珠光貼紙":"銅板貼紙";
  const laminateText=laminate==="matte"?"霧膜":"亮膜";
...
function catalogCm(s){s=String(s||"").toUpperCase();return s==="A5"?["14.8","21"]:s==="A6"?["10.5","14.8"]:s==="A7"?["7.4","10.5"]:["",""]}function fillCatalogCm(q){q=q||{};var c=catalogCm(q.catalogSize||q.size||q.sizeText);if(c[0]&&!q.widthCm)q.widthCm=c[0];if(c[1]&&!q.heightCm)q.heightCm=c[1];if(c[0]&&!q.width)q.width=c[0];if(c[1]&&!q.height)q.height=c[1];return q}
