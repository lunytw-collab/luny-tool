/* LUNY label order flow v9.2-phase5-response-reconcile
...
async function hashDesignBlob_(blob){
  if(!blob)return"";
  try{
    if(
...
async function makeDesignFingerprint(payload,formalAssets){
  try{
    ...
    const hashes=await Promise.all([
      hashDesignBlob_(formalAssets?.print?.blob),
      hashDesignBlob_(formalAssets?.cut?.blob)
    ]);
    return [
      "v2",
...
const signed=await requestSignedUploadUrl(designId,assetType,contentType);
...
async function uploadCustomerSourceToCloud_(args){
  ...
  const prepared=await compressCustomerSourceFile_(file);
  let lastErr=null;
