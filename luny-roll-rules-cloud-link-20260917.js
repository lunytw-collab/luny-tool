(function(){
  ...
  window.__LUNY_ROLL_RULES_RUNTIME_V1__="2026-09-17.cloud-link.1";
  ...
  function ruleResult(){
    ...
    if(mixed&&(!diagram||readingDiagramState!=="ready"))return{ok:false,message:"稿件含兩種以上閱讀方向，請先上傳示意圖至雲端，再貼上有效的 HTTPS 分享連結。"};
  ...
  function readDiagramLink(){
    var input=byId("lunyRollDirectionDiagram"),status=byId("lunyRollDirectionDiagramStatus"),raw=String(input&&input.value||"").trim();
    diagram=null;readingDiagramState="idle";
    if(!raw){if(status)status.textContent="尚未填寫分享連結";return;}
    try{
      if(raw.length>2048||/\s/.test(raw))throw new Error("invalid");
      var url=new URL(raw);
      if(url.protocol!=="https:"||!url.hostname||url.username||url.password)throw new Error("invalid");
      // Existing checkout and designer reports display name; keep the usable URL there too.
      diagram={name:url.href,url:url.href,fileUrl:url.href,type:"cloud_link"};
      readingDiagramState="ready";
      if(status)status.textContent="已填寫分享連結；請確認設為「知道連結的人可檢視」。系統不會自動驗證存取權限。";
    }catch(error){readingDiagramState="error";if(status)status.textContent="請貼上完整的 HTTPS 雲端分享連結（最多 2,048 字元）。";}
  }
  ...
  function patchReset(){
    ...
    function patched(){var result=original.apply(this,arguments),split=byId("lunyRollSplitSpecified"),per=byId("lunyRollPerQuantity");var mixed=byId("lunyRollMixedReading"),link=byId("lunyRollDirectionDiagram"),wrap=byId("lunyRollDiagramWrap");if(mixed)mixed.checked=false;if(link)link.value="";if(wrap)wrap.hidden=true;diagram=null;readingDiagramState="idle";readDiagramLink();if(split)split.checked=false;if(per){per.value="2000";per.disabled=true;}setTimeout(refresh,0);return result;}
