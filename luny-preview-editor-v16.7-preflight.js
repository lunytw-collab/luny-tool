...
function lunyFlattenedPhotoDangerAssessment(){
  ...
  for(let t=0;t<=1.0001;t+=0.2){
    ...
    for(const point of pts){
      ...
      samples+=1;
      if(photoPointHasCompositeContent(point.x,point.y))hits+=1;
    }
  }
  const ratio=samples?hits/samples:0;
  /* A few textured pixels usually belong to an outer frame or photo edge.
     Block only when detail is dense across the danger ring. */
  return{danger:samples>=60&&hits>=36&&ratio>=0.55,hits,samples,ratio};
}
