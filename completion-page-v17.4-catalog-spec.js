/* LUNY completion page v17.4 - catalog spec compatibility 2026-09-07 */
(function(){
  "use strict";
...
The backend is authoritative: it queries 1SHOP by order number and requires paid,
the exact checkoutToken in the order note, and the exact SKU 343424 line amount.
Amount/time/group guessing is forbidden.
*/
(function installLunyPhase1CompletionPage(){
  ...
  window.__LUNY_PHASE1_COMPLETION_PAGE__ = "2026-09-03.17.3-persistent-summary";
  ...
  const CFG = {
    ...
    handoffMaxAgeHours: 12, // 與 GAS / 確認頁 / 1SHOP 保持一致
    bindTimeoutMs: 25000,
    summaryTimeoutMs: 45000,
    maxOrderPageText: 12000,
    // Browser retries are intentionally bounded; GAS keeps a low-rate paid queue.
  ...
  let summaryLoadingPromise = null;
  ...
  function getPublicOrderAccessToken(){
    const value = getUrlValue("t2");
    return /^[a-f0-9]{32,128}$/i.test(value) ? value : "";
  }
  ...
  function buildBindRequest(token, payload, orderNo, pageText, receiverText, receiverIdentity){
    ...
    return sanitize({
      ...
      configurationId: clean(payload.configurationId || ""),
      revision: Number(payload.revision || 0),
      syncKey: clean(payload.syncKey || payload.groupId || payload.cartKey || ""),
      cartFingerprint: clean(payload.cartFingerprint || ""),
      orderTotal,
  ...
  async function attemptBind(trigger){
    ...
    // 顯示明細與訂單綁定分開進行。即使舊瀏覽器沒有本機結帳快照，
    // 仍可用 1SHOP 訂單網址內的 t2 憑證向後端讀取已驗證的公開明細。
    void loadAndRenderSummary(orderNo, payload);
    ...
    if (!token){
      ...
      request.fallbackReason = "exact_oneshop_note_only";
    }else if (!payload){
    ...
    saveStatus(token || "", "binding", {
      ...
      message: !token
        ? "完成頁未取得 token，正在由後端查詢 1SHOP 備註中的完整對帳編號；不會用金額或時間猜測。"
        : (!payload
      ...
      if (bindStatus === "complete"){
        ...
        if (!resolvedToken){
          saveStatus("", "abnormal", {
            ...
            bindStatus: "abnormal",
            retryable: false,
            code: "COMPLETE_WITHOUT_RESOLVED_TOKEN",
            message: "後端回傳 complete，但沒有提供已核對的 checkoutToken，因此未標記完成。"
          });
          return;
        }


        // 後端可能在完成頁 URL 已遺失 token 時，從這筆 1SHOP 訂單備註
        // 精準找回 checkoutToken。此時要用「後端已核對的 token」重新讀取
        // 結帳前保存的不可變快照；不可把 items: [] 的診斷 payload 當成明細。
        const resolvedPayload =
          (
            payload &&
            clean(payload.checkoutToken) === resolvedToken
              ? payload
              : null
          ) ||
          loadTokenPayload(resolvedToken);
        ...
        if (resolvedPayload){
          persistOrderTokenMapping(
            ...
            resolvedToken,
            resolvedPayload,
            "server_resolved_token"
          );
        }


        markComplete(resolvedToken, orderNo, result);
        loadAndRenderSummary(orderNo, resolvedPayload);
        return;
      }
  ...
  function summaryCatalogMaterialText(value){
    const map = {
      pearlescent: "防水合成紙",
      shtte: "模造貼紙",
    ...
    return map[clean(value).toLowerCase()] || summaryMaterialText(value);
  }


  function summaryCatalogLaminateText(value){
    const map = {
      none: "無上膜",
      gloss: "亮膜",
      matte: "霧膜"
    ...
    return map[clean(value).toLowerCase()] || summaryLaminateText(value);
  }
  ...
  function summaryInfoRows(item, data){
    ...
    if (type === "CATALOG"){
      ...
      add(
        ...
        q.materialText ||
        q.materialLabel ||
        summaryCatalogMaterialText(q.material || item.material)
      );
      add(
        "上膜",
        q.laminateText ||
        q.laminateLabel ||
        summaryCatalogLaminateText(q.laminate || item.laminate)
      );
  ...
  async function loadAndRenderSummary(orderNo, localPayload){
    ...
    if (summaryLoadingPromise) return summaryLoadingPromise;


    summaryLoadingPromise = (async function(){
      // 同一台瀏覽器仍保有結帳快照時，優先使用送出前的不可變資料。
      if (
        localPayload &&
        Array.isArray(localPayload.items) &&
        localPayload.items.length
      ){
      ...
      // 跨瀏覽器／過一段時間再開啟時，以 1SHOP 訂單網址的 t2 作為
      // bearer-link 憑證。後端會再次向 1SHOP 核對訂單號與已付款狀態，
      // 並且只回傳不含姓名、電話、地址與檔案連結的公開明細。
      const accessToken = getPublicOrderAccessToken();
      ...
      try{
        ...
        if (
          !result ||
          result.ok !== true ||
          clean(result.orderNo).toUpperCase() !== clean(orderNo).toUpperCase() ||
        ...
        completionSummarySnapshot = sanitize(result);
        renderSummary(completionSummarySnapshot);
      }catch(_){
        // 明細備援失敗不影響既有訂單綁定與完成頁狀態。
      }
    })().finally(function(){
