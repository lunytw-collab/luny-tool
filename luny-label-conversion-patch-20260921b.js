 ...
 * 檔名：luny-label-conversion-patch-20260921b.js
 *
 * 1shop 試算頁最底部加一行（不要整頁貼 HTML）：
 * <script src="https://cdn.jsdelivr.net/gh/lunytw-collab/luny-tool@main/luny-label-conversion-patch-20260921b.js?v=20260921b"></script>
 *
 * 本檔只做：
 * - 文案人話化（上傳／信任／步驟）
 * - 材質卡順序：透明、牛皮 → 再銀龍系列（20260921b：修正重排打斷特殊底色點選）
...
(function () {
  ...
  if (window.__LUNY_LABEL_CONVERSION_PATCH_20260921B__) return;
  window.__LUNY_LABEL_CONVERSION_PATCH_20260921B__ = true;
  ...
  function injectStyle() {
    if ($("lunyConversionPatchStyle20260921b")) return;
    var style = document.createElement("style");
    style.id = "lunyConversionPatchStyle20260921b";
    style.textContent =
  ...
  function patchHero() {
    ...
    if (heading) {
      ...
      text(
        ...
        "100 張 NT$484 起。最常用 5×5 cm 銅板＋上膜，100 張 NT$565。選好尺寸與材質就看到價格；再上傳圖片確認畫面。矩形／圓形／橢圓形 100 張起，約 4–5 個工作天寄出。拱門形與客製形狀 500 張起，約 6–7 個工作天。"
      );
    }
  ...
  function patchMaterialGuide() {
    if ($("lunyMaterialGuide")) return;
    var label = document.querySelector('label[for="material"]');
  ...
  function bindStepScroll() {
    if (window.__LUNY_CONV_STEP_SCROLL_B__) return;
    window.__LUNY_CONV_STEP_SCROLL_B__ = true;
  ...
  function observe() {
    if (window.__LUNY_CONV_PATCH_OBSERVER_B__) return;
    window.__LUNY_CONV_PATCH_OBSERVER_B__ = true;
    var scheduled = false;
    new MutationObserver(function () {
      ...
      requestAnimationFrame(function () {
        ...
        patchHero();
        patchSteps();
        // 不在 MutationObserver 重排材質，避免 appendChild 打斷點選
        patchEdgeCopy();
        patchPreflightCopy();
      });
