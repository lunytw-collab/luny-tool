...
(function(){
  ...
  function installPreviewThumbPatch(){
    ...
    if(originalMakePreviewThumb.__lunyFullBleedThumbPatchedV7947) return true;

    const patched = function(maxSize, quality){
      // 結帳清單縮圖與畫面預覽分流：即使前端改用完整原圖重繪，
      // 儲存縮圖仍維持既有的輕量上限，避免增加傳輸、GAS 與清單載入負擔。
      const requestedMax = Number(maxSize);
      const requestedQuality = Number(quality);
      const safeMaxSize = Math.max(1,Math.min(360,requestedMax>0?requestedMax:360));
      const safeQuality = Math.max(0.1,Math.min(0.76,requestedQuality>0?requestedQuality:0.76));

      if(!isFullBleedMode()){
        return originalMakePreviewThumb.call(this,safeMaxSize,safeQuality);
      }
      ...
      if(!source) return originalMakePreviewThumb.call(this,safeMaxSize,safeQuality);
      ...
      decoratePreviewThumbCanvas(out);
      return out.toDataURL('image/jpeg',safeQuality);
    };
