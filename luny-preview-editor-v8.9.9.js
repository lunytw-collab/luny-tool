...
function makeMirrorPreparedSource(source, edgeOnly) {
  ...
  try {
    const imgd = x.getImageData(0, 0, c.width, c.height);
    const d = imgd.data;
    const w = c.width;
    const h = c.height;

    function isNearWhite(px, py) {
      const idx = (py * w + px) * 4;
      const r = d[idx];
      const g = d[idx + 1];
      const b = d[idx + 2];
      const a = d[idx + 3];
      return a > 20 && r > 236 && g > 236 && b > 236 && (Math.max(r, g, b) - Math.min(r, g, b) < 28);
    }

    const q = [];

    function clearIfEdgeWhite(px, py) {
      if (px < 0 || py < 0 || px >= w || py >= h) return;
      const idx = (py * w + px) * 4;
      if (d[idx + 3] === 0) return;

      if (isNearWhite(px, py)) {
        d[idx + 3] = 0;
        q.push([px, py]);
      }
    }
    ...
    for (let xx = 0; xx < w; xx++) {
      clearIfEdgeWhite(xx, 0);
      clearIfEdgeWhite(xx, h - 1);
    }

    for (let yy = 0; yy < h; yy++) {
      clearIfEdgeWhite(0, yy);
      clearIfEdgeWhite(w - 1, yy);
    }
    ...
    for (let qi = 0; qi < q.length; qi++) {
      ...
      const px = p[0];
      const py = p[1];
      clearIfEdgeWhite(px + 1, py);
      clearIfEdgeWhite(px - 1, py);
      clearIfEdgeWhite(px, py + 1);
      clearIfEdgeWhite(px, py - 1);
    }
    ...
    if (edgeOnly) {
      ...
      if (maxX >= minX && maxY >= minY) {
        ...
        const cx0 = minX + bw / 2;
        const cy0 = minY + bh / 2;
        const rx = Math.max(1, bw / 2);
        const ry = Math.max(1, bh / 2);
        const band = Math.max(8, Math.round(Math.min(bw, bh) * 0.22));
        ...
        for (let yy = 0; yy < h; yy++) {
          for (let xx = 0; xx < w; xx++) {
            ...
            const g = d[idx + 1];
            const b = d[idx + 2];
            const nearBlack = r < 120 && g < 120 && b < 120;
            const nx = (xx - cx0) / rx;
            const ny = (yy - cy0) / ry;
            const radial = Math.sqrt(nx * nx + ny * ny);
            const nearBoxEdge = (xx - minX < band) || (maxX - xx < band) || (yy - minY < band) || (maxY - yy < band);
            const keep = (radial > 0.62 || nearBoxEdge) && !nearBlack;
