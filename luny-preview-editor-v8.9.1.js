...
/* v7.9.16：補滿版邊緣＝抓紅線內側 2mm，鏡射延伸到紅線外側 2mm。
   - UI 可維持「補滿版邊緣」，不必讓客戶理解鏡射補邊。
   - 套用時機在印刷內容繪製完成後、輔助線繪製前，避免把紅線/綠線/灰線鏡射進印刷檔。
   - edgeOption=off：補滿版邊緣；edgeOption=mirror：相容舊版鏡射補邊。 */
...
function applyMirrorBleedFromCutLine(ctx,canvas,cm2px){
  ...
  if(edge!=='off'&&edge!=='mirror')return;
  // 若有明確勾選滿版底色，就保留滿版底色功能，不改成鏡射。
  if(edge==='off'&&isFullBleedColorApplyEnabled())return;
...
(function(){
  ...
  window.getEdgeTextValue = function(value, color){
    if(value === 'on') return '加白邊';
    if(value === 'mirror') return '補滿版邊緣';
