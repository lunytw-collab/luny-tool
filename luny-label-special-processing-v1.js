(function installLunySpecialProcessingV1(){
  "use strict";
  if(window.__LUNY_SPECIAL_PROCESSING_V1__) return;
  window.__LUNY_SPECIAL_PROCESSING_V1__="2026-09-06.1";

  var PRICING={
    version:"everprinter-benchmark-20260906-v1",
    whiteInkRate:2/11,
    foilBaseFee:3000,
    foilUnitFee:0.8,
    serialBaseFee:800,
    serialUnitFee:0.3,
    referenceUrl:"https://everprinter.com/product/pdt-81",
    referenceTaxDisplay:"未稅"
  };
  var state={
    basePrice:0,
    totalPrice:0,
  ...
  window.LUNY_SPECIAL_PROCESSING_PRICING_V1=PRICING;
  window.LUNY_SPECIAL_PROCESSING_STATE_V1=state;
  ...
  function calculateProcessing(basePrice,quantity,selection){
    ...
    var items=[];
    if(s.whiteInk){
      items.push({code:"WHITE_INK",label:"白墨",fee:Math.round(base*PRICING.whiteInkRate),formula:"base_product_price_x_2_div_11"});
    }
    if(s.foil){
      items.push({
        code:s.foilColor==="silver"?"FOIL_SILVER":"FOIL_GOLD",
        label:s.foilColor==="silver"?"燙銀":"燙金",
        fee:Math.round(PRICING.foilBaseFee+PRICING.foilUnitFee*qty),
        formula:"3000_plus_0.8_per_piece"
      });
    }
    if(s.serial){
      items.push({code:"SERIAL_NUMBER",label:"流水號",fee:Math.round(PRICING.serialBaseFee+PRICING.serialUnitFee*qty),formula:"800_plus_0.3_per_piece"});
    }
  ...
  function stripProcessingSummary(text){
    return String(text||"").split("\n").filter(function(line){
      return !/^商品原價：/.test(line)&&!/^特殊加工：/.test(line)&&!/^加工費：/.test(line)&&!/^含加工商品報價：/.test(line)&&!/^正式完稿檔：/.test(line)&&!/^製作狀態：/.test(line);
    }).join("\n").trim();
  }
  ...
  function resetArtwork(message){
    ...
    var input=byId("lunyProductionArtworkFile");
    if(input) input.value="";
    setArtworkStatus(message||"尚未選擇檔案","idle");
  }
  function getBlockingReason(){
    var selection=getSelection();
    if(!hasProcessing(selection)) return "";
    var signature=artworkConfigSignature();
    if(state.uploadState==="uploading") return "正式完稿檔正在上傳，請稍候。";
    if(state.uploadState==="error") return "正式完稿檔上傳未完成，請重新選擇檔案。";
    if(!state.artwork||state.artworkSignature!==signature) return "請先上傳包含目前加工項目的正式完稿檔。";
  ...
  function updatePreviewUploadCopy(active){
    ...
    if(active){
      ...
      if(helper) helper.textContent="此圖片只用於材質質感、裁切外觀與實貼情境預覽；特殊加工位置與實際生產內容，以正式完稿檔為準。";
    }else{
  ...
  function handleProcessingChange(){
    ...
    if((state.artwork||state.uploadState==="uploading")&&state.artworkSignature!==artworkConfigSignature()){
      resetArtwork("加工項目已變更，請重新上傳最新完稿檔。 ");
    }
  ...
  function artworkContentType(file){
  ...
  function isAllowedArtwork(file){return /\.(ai|pdf|zip)$/i.test(String(file?.name||""));}
  ...
  async function waitForArtworkUploader(){
    for(var i=0;i<40;i+=1){
      if(typeof window.uploadBlobAssetToCloud==="function") return window.uploadBlobAssetToCloud;
      await new Promise(function(resolve){setTimeout(resolve,150);});
    }
    throw new Error("檔案上傳服務尚未載入，請重新整理後再試。");
  }
  async function handleArtworkChange(event){
    ...
    var generation=++state.uploadGeneration;
    state.artwork=null;
    state.artworkSignature="";
    if(!hasProcessing()){
      state.uploadState="idle";
    ...
    if(!isAllowedArtwork(file)){
      ...
      input.value="";
      setArtworkStatus("檔案格式不符，請上傳 AI、PDF 或 ZIP。","error");
      syncSaveGate();
      return;
    }
    state.uploadState="uploading";
    setArtworkStatus("正在上傳："+file.name,"uploading");
    syncSaveGate();
    try{
      var uploader=await waitForArtworkUploader();
      var artworkRef=makeArtworkRef();
      var uploaded=await uploader({
        designId:artworkRef,
        assetType:"customer_source",
        blob:file,
        contentType:artworkContentType(file)
      });
      if(generation!==state.uploadGeneration) return;
      if(signature!==selectionSignature()||configSignature!==artworkConfigSignature()){
        state.uploadState="idle";
        setArtworkStatus("加工項目已變更，請重新上傳最新完稿檔。","error");
        syncSaveGate();
        return;
      }
      state.artwork={
        ...
        objectName:String(uploaded?.objectName||""),
        fileUrl:String(uploaded?.fileUrl||""),
        gsUrl:String(uploaded?.gsUrl||""),
        bucket:String(uploaded?.bucket||""),
        originalFilename:String(file.name||""),
        originalContentType:artworkContentType(file),
      ...
      if(!state.artwork.fileUrl&&!state.artwork.gsUrl&&!state.artwork.objectName){
        throw new Error("正式完稿檔上傳後未取得檔案識別碼。");
      }
      state.artworkSignature=configSignature;
      state.uploadState="done";
      setArtworkStatus("已上傳："+file.name+"｜待檔案確認","done");
    }catch(error){
      if(generation!==state.uploadGeneration) return;
      state.artwork=null;
      state.artworkSignature="";
      state.uploadState="error";
      setArtworkStatus("上傳未完成："+(error?.message||"請重新選擇檔案"),"error");
      console.error("[LUNY special processing] production artwork upload failed",error);
    }finally{
      if(generation===state.uploadGeneration) syncSaveGate();
    }
  }
  function getSpecialProcessingPayload(){
    ...
    return{
      version:"LUNY_SPECIAL_PROCESSING_V1",
      selected:hasProcessing(selection),
      status:hasProcessing(selection)?"pending_file_confirmation":"not_selected",
      productionSourcePriority:hasProcessing(selection)?"uploaded_production_artwork":"generated_print_assets",
      previewPurpose:"material_and_application_simulation_only",
      actualProductionBasis:"confirmed_production_artwork",
      ...
      pricingBenchmark:{
        ...
        referenceUrl:PRICING.referenceUrl,
        referenceTaxDisplay:PRICING.referenceTaxDisplay
      }
    };
  }
  function patchPriceEngine(){
    var engine=window.LUNY_PRICE_ENGINE;
    if(!engine||typeof engine.calculatePrice!=="function"||engine.calculatePrice.__lunySpecialProcessingV1) return false;
    ...
    patchedCalculatePrice.__lunySpecialProcessingV1=true;
  ...
  function patchOrderPayload(){
    if(typeof window.buildOrderPayload!=="function"||window.buildOrderPayload.__lunySpecialProcessingV1) return false;
    ...
    function patchedBuildOrderPayload(){
      ...
      if(special.selected){
        ...
        summary+="\n正式完稿檔："+(special.productionArtwork?.originalFilename||"尚未上傳");
        summary+="\n製作狀態：待檔案確認";
      }
    ...
    patchedBuildOrderPayload.__lunySpecialProcessingV1=true;
  ...
  function specialPreflightResult(){
    ...
    return{
      ...
      issues:[],
      recommendations:["右側圖片僅供材質與實貼預覽；實際生產以已上傳的正式完稿檔為準。"],
      canProceed:true,
  ...
  function patchPreflight(){
    var changed=false;
    if(typeof window.lunyConfirmBleedBeforeSave==="function"&&!window.lunyConfirmBleedBeforeSave.__lunySpecialProcessingV1){
      ...
      patchedConfirm.__lunySpecialProcessingV1=true;
      patchedConfirm.__lunyOriginal=originalConfirm;
      window.lunyConfirmBleedBeforeSave=patchedConfirm;
      changed=true;
    }
    if(typeof window.LUNY_getPreflightResult==="function"&&!window.LUNY_getPreflightResult.__lunySpecialProcessingV1){
      ...
      patchedResult.__lunySpecialProcessingV1=true;
  ...
  function enhanceCheckoutSummary(){
    ...
    nodes.forEach(function(node,index){
      ...
      var filename=special.productionArtwork?.originalFilename||"待補";
      var detailText="特殊加工："+names+"｜加工費 NT$ "+money(special.totalFee)+"｜正式完稿檔："+filename+"｜待檔案確認";
  ...
  function patchCheckoutSummary(){
    if(typeof window.renderCheckoutSummary!=="function"||window.renderCheckoutSummary.__lunySpecialProcessingV1) return false;
    ...
    patchedRenderCheckoutSummary.__lunySpecialProcessingV1=true;
  ...
  function handleArtworkRelevantSpecChange(event){
    ...
    if((quantityAffectsArtwork||sizeAffectsArtwork)&&(state.artwork||state.uploadState==="uploading")&&state.artworkSignature!==artworkConfigSignature()){
      resetArtwork(quantityAffectsArtwork?"流水號數量已變更，請重新上傳標示最新起訖碼的完稿檔。":"尺寸或形狀已變更，請重新上傳相符的正式完稿檔。");
    }
    setTimeout(function(){syncPrice();syncSaveGate();},0);
  }
  function patchResetEditor(){
    if(typeof window.resetEditorForNextDesign!=="function"||window.resetEditorForNextDesign.__lunySpecialProcessingV1) return false;
    ...
    patchedResetEditor.__lunySpecialProcessingV1=true;
  ...
  function bind(){
    ...
    byId("lunyProductionArtworkFile")?.addEventListener("change",handleArtworkChange);
    var priceEl=byId("price");
    ...
    window.addEventListener("luny:custom-size-updated",function(){
      if((state.artwork||state.uploadState==="uploading")&&state.artworkSignature!==artworkConfigSignature()){
        resetArtwork("尺寸或形狀已變更，請重新上傳相符的正式完稿檔。");
      }
      syncSaveGate();
    });
    renderSelectionControls();
    ...
    document.documentElement.setAttribute("data-luny-special-processing","v1");
  }
