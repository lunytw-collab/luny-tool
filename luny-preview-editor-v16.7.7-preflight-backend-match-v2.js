/* LUNY v16.7.7-backend-preview-match-v2：恢復畫布邊緣色，並統一只補出血線至裁切線。 */
...
(function(){
  ...
  function drawEdgeRing(ctx,W,H,cm2px,color,withGuideLines,withWarning){
    ...
    if(color){
      ...
      // 與 applySelectedEdgeColor／正式印刷檔一致：只補出血線至裁切線。
  ...
  function renderMatchedPreviewCanvas(maxSide){
    ...
    // 裁切遮罩可能蓋住主畫布外圈，因此在最上層按正式印刷範圍補回邊緣色。
  ...
  function renderOverlay(){
    ...
    // 前台畫布立即顯示邊緣色；drawEdgeRing 已限制為出血線至裁切線。
