...
/* LUNY：成品導向 / 裁切風險預覽 v8.9 minimal */
.luny-risk-intro{max-width:420px;margin:6px auto 10px;padding:10px 12px;border-radius:14px;background:#f8fafc;border:1px solid #e5e7eb;color:#4b5563;font-size:13px;line-height:1.55;text-align:left;}
.edge-choice.luny-edge-methods{display:grid !important;grid-template-columns:repeat(2,minmax(0,1fr)) !important;gap:8px !important;}
.edge-choice.luny-edge-methods label{min-width:0;}
.edge-choice.luny-edge-methods .edge-option{display:block;min-height:44px;padding:9px 8px;border-radius:12px;font-size:13px;font-weight:800;line-height:1.25;}
.edge-choice.luny-edge-methods .edge-note{display:block;margin-top:4px;font-size:11px;line-height:1.35;color:#6b7280;}

.edge-choice.luny-edge-methods label:has(input[value="mirror"]){display:none !important;}
@media (max-width:520px){.edge-choice.luny-edge-methods{grid-template-columns:1fr !important;}}



/* LUNY：移除預覽輔助功能區塊 v1
   隱藏：裁切風險提示、查看安全範圍線、強制顯示危險區域、補滿版/白邊切換、滿版底色/邊框顏色。 */
#lunyBleedRiskBox,
#edgeChoiceUI,
#edgeColorWrap{
  display:none !important;
...
<script
  src="https://cdn.jsdelivr.net/gh/lunytw-collab/luny-tool@main/luny-preview-editor-v9.8.js?v=20260709-1"
  onload="console.log('✅ preview editor v9.6 canvas-eyedropper loaded', typeof drawPreview); if(typeof drawPreview==='function') drawPreview();"
  onerror="console.error('❌ preview editor v9.6 canvas-eyedropper 載入失敗');"
></script>
<script
...
<script>
/* LUNY：移除預覽輔助功能區塊 v1
   固定預設補滿版邊緣；把客人不需要操作的檢查/切換 UI 全部隱藏並停用。 */
(function removePreviewHelperBlocks(){
  function apply(){
    var off = document.querySelector('input[name="edgeOption"][value="off"]');
    if(off) off.checked = true;

    document.querySelectorAll('input[name="edgeOption"]').forEach(function(input){
      input.disabled = true;
      input.tabIndex = -1;
    });

    ['edgeColorEnabled','edgeColor','importantContentMode','safetyGuideToggle'].forEach(function(id){
    ...
    ['lunyBleedRiskBox','edgeChoiceUI','edgeColorWrap'].forEach(function(id){
      var el = document.getElementById(id);
      if(!el) return;
