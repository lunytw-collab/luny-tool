...
function lunyCustomDrawGuides(ctx,cx,cy,dims,b,gap,cm2px){
  ...
  ctx.save();ctx.beginPath();ctx.rect(0,0,ctx.canvas.width,ctx.canvas.height);lunyCustomAddPath(ctx,previewW,previewH,cm2px);ctx.clip('evenodd');drawCheckerboard(ctx,0,0,ctx.canvas.width,ctx.canvas.height,16,'#f7f7f7','#e9e9e9');ctx.fillStyle='rgba(255,255,255,.28)';ctx.fillRect(0,0,ctx.canvas.width,ctx.canvas.height);ctx.restore();
...
function drawGuides(ctx,shapeValue,cx,cy,dims,b,gap,cm2px){
  ...
  ctx.save();ctx.beginPath();ctx.rect(0,0,ctx.canvas.width,ctx.canvas.height);addShapeSubPath(previewW,previewH);ctx.clip('evenodd');drawCheckerboard(ctx,0,0,ctx.canvas.width,ctx.canvas.height,16,'#f7f7f7','#e9e9e9');ctx.fillStyle='rgba(255,255,255,0.28)';ctx.fillRect(0,0,ctx.canvas.width,ctx.canvas.height);ctx.restore();
...
function addLunyRiskShapePath(ctx,shapeValue,cx,cy,w,h,cm2px){if(shapeValue==='custom'&&lunyCustomAddPath(ctx,w,h,cm2px))return;const s=shapeValue;const rpx=0.1*cm2px;if(s==='circle'){const r=Math.min(w,h)/2;ctx.arc(cx,cy,r,0,Math.PI*2);}else if(s==='roundrect'){const x=cx-w/2,y=cy-h/2;roundedRectPath(ctx,x,y,w,h,rpx);ctx.closePath();}else if(s==='ellipse'){ctx.ellipse(cx,cy,w/2,h/2,0,0,Math.PI*2);}else if(s==='arch'){const x=cx-w/2,y=cy-h/2;archPath(ctx,x,y,w,h,cm2px);ctx.closePath();}}function drawProductPreviewCrop(ctx,canvas,cm2px){
  ...
  const blocked=!!(
    preflight&&preflight.level==='red'&&(
      preflight.autoFix==='MOVE_IMPORTANT_CONTENT'||
      preflight.flattenedRisk&&preflight.flattenedRisk.danger
    )
  );
  const caution=!!(
    preflight&&preflight.level==='yellow'&&
    preflight.flattenedRisk&&(
      preflight.flattenedRisk.danger||
      preflight.flattenedRisk.caution
    )
  );
  ...
  ctx.save();
  ctx.fillStyle=caution?'rgba(245,158,11,0.18)':'rgba(210,44,68,0.22)';
  ctx.beginPath();
  addLunyRiskShapePath(ctx,shape.value,cx,cy,cutW,cutH,cm2px);
...
var LUNY_PREFLIGHT_VERSION='16.8.1';
...
function lunyClassifyPreflight(){
  ...
  if(placed.any){
    ...
    return lunyMakePreflightResult(
      ...
      ['按「移回安全區」讓系統將新增內容移到安全線內。','或自行縮小並拖曳到安全線內，再重新檢查。'],
  ...
  if(flattenedRisk.danger){
    return lunyMakePreflightResult(
      ...
      ['縮小圖片，讓重要內容完整回到安全線內。','加白邊或加邊框色後再確認預覽。','不方便自行調整時，可加購印刷檔案基礎整理。'],
      {
        resolution:resolutionSummary,
        flattenedRisk,
  ...
  if(!fitAccepted&&!lunyPhotoCoversBleed()){
    const eligible=lunyFilePrepEligibleForWhiteMargin(resolution);
    if(eligible){
      return lunyMakePreflightResult(
        'BLOCKED_FILE_PREP_ELIGIBLE','red','圖片外圍有白底或沒有預留出血',
        ['有效圖案沒有完整延伸到灰色出血線，裁切後可能露出白邊。'],
        ['先按「置中」查看結果。','也可選擇「加白邊」或「加邊框色」。','不方便自行調整時，可加購印刷檔案基礎整理。'],
        {resolution:resolutionSummary,flattenedRisk,filePrepEligible:true,autoFix:'TRIM_WHITE_AND_FILL'}
      );
    }
    return lunyMakePreflightResult(
      'BLOCKED_FILE_PREP_ELIGIBLE','red','圖片尚未鋪滿出血範圍',
      ['圖片沒有完整延伸到灰色出血線，裁切後可能露出白邊。'],
      ['可按「置中」後確認裁切結果。','也可選擇「加白邊」、「加邊框色」，或加購印刷檔案基礎整理。'],
      {
        resolution:resolutionSummary,
        flattenedRisk,
        filePrepEligible:true,
        filePrepScope:['CENTER_AND_SCALE','SOLID_COLOR_BLEED'],
        filePrepDescription:'含整張圖等比例縮放、置中與單色出血；不含拆字、改字、移動單一物件、重排或重繪。整理完成後提供預覽確認，製作天數從確認後起算。',
...
function lunyGetPreflightResult(){
  ...
  if(window.LUNY_ARTWORK_FLOW)return window.LUNY_ARTWORK_FLOW.transform(result);
  return Object.assign(result,{canProceed:false,productionDisposition:'BLOCKED',title:(!img?'請先上傳要製作的圖片':'圖片確認流程載入中'),status:(!img?'NO_IMAGE':'ARTWORK_LOADING')});
}
...
function lunyFitPhotoContentToBleed(){
  ...
  const box=resolution&&resolution.contentBox&&resolution.contentBox.found
    ? resolution.contentBox
    : {x:0,y:0,width:Number(resolution&&resolution.pixelWidth||img.width),height:Number(resolution&&resolution.pixelHeight||img.height)};
  const sourceW=Math.max(1,Number(resolution&&resolution.pixelWidth||img.width));
  const sourceH=Math.max(1,Number(resolution&&resolution.pixelHeight||img.height));
  const ratioX=img.width/sourceW,ratioY=img.height/sourceH;
  const bx=Number(box.x||0)*ratioX,by=Number(box.y||0)*ratioY;
  const bw=Math.max(1,Number(box.width||sourceW)*ratioX);
  const bh=Math.max(1,Number(box.height||sourceH)*ratioY);
  scale=Math.max(cG.width/bw,cG.height/bh)*1.025;
...
function lunyRenderPreflight(result){
  if(window.LUNY_ARTWORK_FLOW){window.LUNY_ARTWORK_FLOW.renderEngine(result,lunyEnsurePreflightPanel());return;}
  ...
  const forceDetailsOpen=String(result.uiStatusLabel||result.title||'')==='解析度不足';
  ...
  if(hasArtwork&&String(result.uiStatusLabel||'')!=='解析度不足'){
    actions.push(lunyPreflightButton('加白邊','white',false));
    actions.push(lunyPreflightButton('加邊框色','color',false));
...
/* Runs inside the pinned editor closure. Read actual geometry; never infer
   preservation from the edge radio (white/color paint over the safety ring). */
window.LUNY_ARTWORK_BRIDGE = (() => {
  let cachedKey='', cached=null;
  const pathCanvas=document.createElement('canvas');
  const pc=pathCanvas.getContext('2d');
  function key(){
    return JSON.stringify([lunyPreflightStateKey(),lunyGetEffectiveSizeCm(),
      imgFull&&[imgFull.naturalWidth,imgFull.naturalHeight],
      iconImg&&[iconImg.src,iconScale,iconOffsetX,iconOffsetY,iconAngle],
      textStr,textSizeCM,textScale,textOffsetX,textOffsetY,textAngle,textFill,
      backgroundColor,eyedropperColor,CM2PX,cG.width,cG.height]);
  }
  function safePath(){
    const b=BLEED_CM*CM2PX,g=GAP_CM*CM2PX;
    pathCanvas.width=cG.width;pathCanvas.height=cG.height;
    pc.beginPath();
    addLunyRiskShapePath(pc,shape.value,cG.width/2,cG.height/2,
      Math.max(1,cG.width-2*b-2*g),Math.max(1,cG.height-2*b-2*g),CM2PX);
  }
  function inside(w,h,x,y,a){
    // All source rectangle edges, including its white/transparent margins.
    // Convex standard shapes: this conservative test never removes artwork.
    for(let side=0;side<4;side++)for(let i=0;i<=32;i++){
      const t=i/32,px=side===0?-w/2+t*w:side===1?w/2:side===2?w/2-t*w:-w/2;
      const py=side===0?-h/2:side===1?-h/2+t*h:side===2?h/2:h/2-t*h;
      const r=rot(px,py,a);
      if(!pc.isPointInPath(cG.width/2+x+r.x,cG.height/2+y+r.y))return false;
    }
    return true;
  }
  function snapshot(){
    const k=key();if(k===cachedKey&&cached)return cached;
    safePath();
    const photoInside=!!img&&inside(img.width*scale,img.height*scale,offsetX,offsetY,angle);
    const m=textStr?measureTextBox(textStr,textSizeCM*CM2PX*textScale):null;
    const placedInside=(!iconImg||inside(iconImg.width*iconScale,iconImg.height*iconScale,iconOffsetX,iconOffsetY,iconAngle))&&
      (!m||inside(m.w,m.h,textOffsetX,textOffsetY,textAngle));
    cachedKey=k;
    cached={key:k,imageKey:lunyPreflightImageKey(),hasImage:!!img,shape:shape.value,
      size:lunyGetEffectiveSizeCm(),bleedMm:BLEED_CM*10,safeMm:GAP_CM*10,
      photoInside,placedInside,rawBleedCovered:!!img&&photoCoversPreviewShape(Math.max(1,cG.width-4),Math.max(1,cG.height-4)),
      edge:getEdgeOption(),color:eyedropperColor||'#ffffff',background:backgroundColor,
      photo:img?{width:img.width,height:img.height,scale,x:offsetX,y:offsetY,angle}:null};
    return cached;
  }
  function fit(mode,color){
    if(!img||shape.value==='custom')return false;
    safePath();
    let lo=0,hi=Math.min(cG.width/img.width,cG.height/img.height);
    for(let i=0;i<36;i++){
      const mid=(lo+hi)/2;
      if(inside(img.width*mid,img.height*mid,0,0,angle))lo=mid;else hi=mid;
    }
    scale=lo*.98;offsetX=0;offsetY=0;activeTarget='photo';
    __lunyPreflightWarningAccepted=false;__lunyPreflightWarningAcceptanceKey='';__lunyFilePrepSelected=false;
    lunyClearAcceptedFit();
    if(mode==='color'){
      backgroundColor=color;window.LUNY_BACKGROUND_COLOR=color;
      lunySyncEdgeColorState('color',color);
    }else{
      backgroundColor='';window.LUNY_BACKGROUND_COLOR='';eyedropperColor='';
      lunySetEdgeOption('on');
    }
    window.lunyResetBleedRiskDecision();cachedKey='';drawPreview();return true;
  }
  function fill(color){
    // Explicit single-color background; original JPG whites remain whites.
    backgroundColor=color;window.LUNY_BACKGROUND_COLOR=color;
    lunySyncEdgeColorState('color',color);cachedKey='';drawPreview();
  }
  function original(){
    backgroundColor='';window.LUNY_BACKGROUND_COLOR='';
    lunyResetEdgeChoice();cachedKey='';drawPreview();
  }
  function proof(target,showRisk,dxMm,dyMm){
    if(!img)return false;
    const s=lunyGetEffectiveSizeCm(),maxSide=760;
    const p=maxSide/Math.max(s.widthCm+2*BLEED_CM,s.heightCm+2*BLEED_CM);
    const source=document.createElement('canvas');
    source.width=Math.round((s.widthCm+2*BLEED_CM)*p);source.height=Math.round((s.heightCm+2*BLEED_CM)*p);
    drawAll(source.getContext('2d'),source,p,{includeGuides:false,includeSelection:false,
      showQRTestMark:false,showMinText:false,isPreview:false,useFullImage:true,autoBleedForPrint:true});
    target.width=source.width;target.height=source.height;
    const c=target.getContext('2d'),cx=target.width/2,cy=target.height/2;
    drawCheckerboard(c,0,0,target.width,target.height,16,'#f4f4f4','#e8e8e8');
    c.save();c.beginPath();
    addLunyRiskShapePath(c,shape.value,cx,cy,s.widthCm*p,s.heightCm*p,p);c.clip();
    c.fillStyle='#fff';c.fillRect(0,0,target.width,target.height);
    c.drawImage(source,-Number(dxMm||0)*p/10,-Number(dyMm||0)*p/10);
    if(showRisk){
      c.beginPath();addLunyRiskShapePath(c,shape.value,cx,cy,s.widthCm*p,s.heightCm*p,p);
      addLunyRiskShapePath(c,shape.value,cx,cy,Math.max(1,(s.widthCm-2*GAP_CM)*p),Math.max(1,(s.heightCm-2*GAP_CM)*p),p);
      c.fillStyle='rgba(231,90,50,.30)';c.fill('evenodd');
    }
    c.restore();source.width=1;source.height=1;return true;
  }
  return {version:'20260914-v1',snapshot,fit,fill,original,proof,getResult:()=>lunyGetPreflightResult()};
})();
...
(function(){
  ...
  function renderMatchedPreviewCanvas(maxSide){
    ...
    drawEdgeRing(ctx,width,height,cm2px,getEdgeColor(),true,true);
    return out;
  }
  ...
  function decoratePreviewThumbCanvas(canvas){
    ...
    drawEdgeRing(ctx,canvas.width,canvas.height,cm2px,getEdgeColor(),true,true);
    return canvas;
  }
