...
var LUNY_PREFLIGHT_VERSION='16.7.9';
...
function lunyClassifyPreflight(){
  ...
  if(fitAccepted){
    return lunyMakePreflightResult(
      ...
      {resolution:resolutionSummary,flattenedRisk,canProceed:true,fitAccepted:true,edgeChoice:{mode:'fit',label:'裁掉白邊並放大',color:'#ffffff'}}
    );
  }
...
function lunyRenderPreflight(result){
  ...
  if(isBleedFix&&(result.status==='BLOCKED_FILE_PREP_ELIGIBLE'||result.status==='BLOCKED_AUTO_FIXABLE')){
    actions.push(lunyPreflightButton('套用安全白邊','white',false));
