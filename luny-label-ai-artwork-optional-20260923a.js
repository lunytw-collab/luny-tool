...
function createPanel(){
  ...
  var grid=photo?.closest(".upload-grid");
  if(!block||!grid)return null;
  var panel=byId("lunyIllustratorArtworkPanel");
  if(!panel){
    panel=document.createElement("details");panel.id="lunyIllustratorArtworkPanel";
    ...
    panel.append(summary);grid.after(panel);panel.append(block);
  }
  block.hidden=false;
  ...
  if(rules&&!rules.querySelector("[data-luny-ai-general-rule]")){
    var rule=document.createElement("li");rule.dataset.lunyAiGeneralRule="1";rule.textContent="連結權限請設為知道連結的人可檢視或下載；訂單製作完成前請勿刪除、移動或更換檔案。";rules.prepend(rule);
  }
  configureSampleLink();
  return panel;
}
function configureSampleLink(){
  ...
  if(SAMPLE_FILE_URL&&result.valid&&!result.empty){link.href=result.url;link.target="_blank";link.rel="noopener noreferrer";link.removeAttribute("aria-disabled");link.textContent="查看 Illustrator 完稿範例檔";}
  else{link.href="#";link.removeAttribute("target");link.setAttribute("aria-disabled","true");link.textContent="Illustrator 完稿範例檔（連結待補）";}
}
function syncPanel(){
  var panel=createPanel();if(!panel)return;
  var required=isSpecialSelected();
  panel.dataset.required=required?"true":"false";
  byId("lunyProductionArtworkBlock").hidden=false;
...
function patchOrderPayload(){
  ...
  if(typeof current!=="function"||current.__lunyIllustratorArtworkOptionalV1)return false;
  function patched(){
  ...
  patched.__lunyIllustratorArtworkOptionalV1=true;patched.__lunySpecialProcessingV3=true;patched.__lunyOriginal=current;window.buildOrderPayload=patched;return true;
}
...
function patchCheckoutSummary(){
  ...
  patched.__lunyIllustratorArtworkOptionalV1=true;patched.__lunySpecialProcessingV3=true;patched.__lunyOriginal=current;window.renderCheckoutSummary=patched;enhanceCheckoutSummary();return true;
}
function patchReset(){
  var current=window.resetEditorForNextDesign;
  if(typeof current!=="function"||current.__lunyIllustratorArtworkOptionalV1)return false;
  function patched(){var result=current.apply(this,arguments);setTimeout(function(){var panel=byId("lunyIllustratorArtworkPanel");if(panel)panel.open=false;syncPanel();},0);return result;}
  patched.__lunyIllustratorArtworkOptionalV1=true;patched.__lunySpecialProcessingV3=true;patched.__lunyOriginal=current;window.resetEditorForNextDesign=patched;return true;
}
function bind(){
  createPanel();syncPanel();
  byId("lunyProductionArtworkUrl")?.addEventListener("change",handleLink);
  byId("lunyProductionArtworkUrl")?.addEventListener("blur",handleLink);
  ["lunyProcessWhiteInk","lunyProcessFoil","lunyProcessSerial"].forEach(function(id){byId(id)?.addEventListener("change",function(){setTimeout(syncPanel,0);});});
  document.querySelectorAll('input[name="lunyFoilColor"]').forEach(function(node){node.addEventListener("change",function(){setTimeout(syncPanel,0);});});
  byId("lunyIllustratorSampleLink")?.addEventListener("click",function(event){if(event.currentTarget.getAttribute("aria-disabled")==="true"){event.preventDefault();var note=byId("lunyIllustratorSampleNote");if(note)note.textContent="範例檔連結準備中，完成後將顯示於此。";}});
  patchOrderPayload();patchCheckoutSummary();patchReset();
  var list=byId("checkoutDesignList");if(list&&window.MutationObserver)new MutationObserver(enhanceCheckoutSummary).observe(list,{childList:true,subtree:true});
  var attempts=0;var timer=setInterval(function(){attempts+=1;patchOrderPayload();patchCheckoutSummary();patchReset();syncPanel();if(attempts>=60)clearInterval(timer);},200);
  document.documentElement.setAttribute("data-luny-illustrator-artwork","optional-20260923a");
}
