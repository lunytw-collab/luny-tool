...
function lunyCustomComputeCutline(){
  ...
  const key=lunyCustomStateKey();if(__lunyCustomCutlineCache&&__lunyCustomCutlineCache.key===key)return __lunyCustomCutlineCache;
  ...
  for(const mm of offsets){const points=lunyCustomBuildOutline(base,w,h,pxPerMm,mm);if(mm===2)cutAnalysisPoints=points;pointsByOffset[mm]=points.map(p=>({x:p.x/w,y:p.y/h}));}
  ...
  const result={key,pointsByOffset,minAngle,fits,analysisWidth:w,analysisHeight:h,warnings:[]};
  if(!fits)result.warnings.push('圖案加上 2mm 刀線後超出目前成品尺寸，請將圖案縮小或移回中央。');
  if(minAngle<LUNY_CUSTOM_MIN_ANGLE_DEG-.25)result.warnings.push(`刀線仍有 ${minAngle.toFixed(1)}° 的銳角，請縮小細節或改用較簡單圖案。`);
  __lunyCustomCutlineCache=result;return result;
}
