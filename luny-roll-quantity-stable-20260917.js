(function installLunyRollQuantityPricesV1(){
  ...
  function fill(){
    ...
    wrap.querySelectorAll(".luny-quantity-row[data-quantity-value]").forEach(function(row){
      ...
      if(row.dataset.lunyRollPriceReady!=="1")row.dataset.lunyRollPriceReady="1";
      filled++;
    });
    var unitHint=byId("unitPriceHint");
    if(unitHint&&currentResult){
      var roundedUnit=Math.round(currentResult.unitPrice*10)/10;
      var hintText="約 "+roundedUnit.toLocaleString("zh-TW")+" 元 / 張";
      if(unitHint.textContent!==hintText)unitHint.textContent=hintText;
      if(unitHint.style.display!=="block")unitHint.style.display="block";
    }
    return filled>0;
  }
