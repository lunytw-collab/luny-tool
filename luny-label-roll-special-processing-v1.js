(function installLunyRollSpecialProcessingV1(){
  "use strict";
  if(window.__LUNY_SPECIAL_PROCESSING_V3__) return;
  window.__LUNY_SPECIAL_PROCESSING_V3__="2026-09-10.roll-v1";
  ...
  function getRollSplitFee(){return byId("lunyRollSplitSpecified")?.checked?1000:0;}
  ...
  function getDeliveryTimeText(){
    var total=11+(isExtendedShape()?4:0)+(hasProcessing()?4:0);
    return "約 "+total+" 個工作天寄出";
  }
  ...
  function getBasePrice(){
    ...
    if((data.specialProcessingAppliedV1||data.rollSplitSpecified)&&Number.isFinite(Number(data.baseProductPrice))){
  ...
  function stripProcessingSummary(text){
    return String(text||"").split("\n").filter(function(line){
      return !/^商品原價：/.test(line)&&!/^特殊加工：/.test(line)&&!/^加工費：/.test(line)&&!/^含加工商品報價：/.test(line)&&!/^指定分捲費：/.test(line)&&!/^商品總報價：/.test(line)&&!/^加工交期：/.test(line)&&!/^正式完稿(?:檔| AI 連結)：/.test(line)&&!/^製作狀態：/.test(line);
    }).join("\n").trim();
  }
  function renderBreakdown(){
    ...
    box.hidden=!state.items.length&&!state.splitFee;
    ...
    if(state.splitFee){
      ...
      splitRow.className="lsp-price-row";
      var splitLabel=document.createElement("span");
      splitLabel.textContent="指定分捲費";
      var splitValue=document.createElement("strong");
      splitValue.textContent="NT$ "+money(state.splitFee);
      splitRow.append(splitLabel,splitValue);
      itemsBox.appendChild(splitRow);
    }
  }
  ...
  function syncPrice(){
    ...
    try{
      ...
      var splitFee=getRollSplitFee();
      state.basePrice=base;
      state.processingFee=result.fee;
      state.splitFee=splitFee;
      state.totalPrice=result.total+splitFee;
      state.items=result.items;
      var nextText=String(Math.round(state.totalPrice||0));
      ...
      data.rollSplitSpecified=splitFee>0;
      data.rollSplitFee=splitFee;
      data.price=state.totalPrice;
      data.totalPrice=state.totalPrice;
      window.LUNY_CURRENT_PRICE_DATA=data;
      var orderLink=byId("orderLink");
      if(orderLink) orderLink.dataset.price=String(state.totalPrice);
      ...
      if(result.items.length){
        ...
        window.currentSummary=baseSummary+"\n特殊加工："+result.items.map(function(item){return item.label;}).join("、")+"\n加工費：NT$ "+money(result.fee)+"\n含加工商品報價：NT$ "+money(result.total)+(splitFee?"\n指定分捲費：NT$ "+money(splitFee):"")+"\n商品總報價：NT$ "+money(state.totalPrice)+"\n加工交期：另加 4 個工作天\n製作狀態：待檔案確認";
      }else{
        window.currentSummary=baseSummary;
        if(base>0){
          if(/^費用：NT\$/m.test(window.currentSummary)) window.currentSummary=window.currentSummary.replace(/^費用：NT\$\s*[^\n]+$/m,"費用：NT$ "+money(state.totalPrice));
          else window.currentSummary+=(window.currentSummary?"\n":"")+"費用：NT$ "+money(state.totalPrice);
          if(splitFee) window.currentSummary+="\n指定分捲費：NT$ "+money(splitFee);
        }
      }
      var summaryEl=byId("summaryText");
      if(summaryEl) summaryEl.textContent=window.currentSummary||"";
      renderBreakdown();
      document.dispatchEvent(new CustomEvent("luny:special-processing-price-updated",{detail:{basePrice:base,processingFee:result.fee,splitFee:splitFee,totalPrice:state.totalPrice,items:result.items}}));
  ...
  function patchOrderPayload(){
    ...
    function patchedBuildOrderPayload(){
      ...
      payload.quote.rollSplitSpecified=state.splitFee>0;
      payload.quote.rollSplitFee=state.splitFee;
      ...
      if(special.selected){
        ...
        summary+="\n含加工商品報價：NT$ "+money(state.basePrice+state.processingFee);
        if(state.splitFee) summary+="\n指定分捲費：NT$ "+money(state.splitFee);
        summary+="\n商品總報價：NT$ "+money(state.totalPrice);
        summary+="\n加工交期：另加 4 個工作天";
  ...
  function bind(){
    ...
    var rollSplit=byId("lunyRollSplitSpecified");
    if(rollSplit) rollSplit.addEventListener("change",function(){syncPrice();});
