(function () {
  ...
  /* LUNY label price engine v38.3
     更新：橢圓形改用短邊／長邊連動下拉選單，並沿用精確公版尺寸驗證。
     更新：拱門形與客製形狀皆為 500 張起印，並使用 6～7 個工作天交期。
     更新：9×9 cm 與 11×11 cm 正方形改用各自對應的市場基準模數。
     更新：移除急件與特急件，所有標籤貼紙皆以一般件計價。
     更新：一般形狀改用健豪數位標籤貼紙目前可選的精確尺寸規格。
     更新：客製形狀不受健豪公版尺寸清單限制，且起訂量改為 500 張。
     ...
     原 v36：
     調整特殊形狀 / 拱門型加價邏輯，形狀加價不受急件與特急件倍率影響。
     所有形狀的一般件皆顯示 4～5 工作天。
  */
  ...
  const CUSTOM_MIN_QUANTITY = 500;

  // 急件產能限制：最多 135 張大紙，但切割時間上限控制在 5000 張貼紙。
  const RUSH_SHEET_LIMIT = 135;
  ...
  // 健豪「20101 數位標籤貼紙」線上估價器目前可直接選擇的標準尺寸。
  // 單位為 mm；方形 / 矩形與橢圓形可旋轉後比對，愛心依官方寬高方向比對。
  const GAINHOW_SQUARE_LARGE_LONG_SIDES_MM = {
    105: [105,110,115,120,125,130,135,140,145,150,155,160,165,170,175,180,190,195,200,300],
    110: [110,115,120,125,130,135,140,145,150,155,160,165,170,175,180,185,190,200,300],
    115: [115,120,125,130,135,140,145,150,155,160,165,170,175,180,185,190,200],
    120: [120,125,130,135,140,145,150,155,160,165,170,175,180,185,190,195,200,300],
    125: [125,130,135,140,145,150,155,160,165,175,180,185,190,195,200],
    130: [130,135,140,145,150,160,165,170,175,180,185,190,195,200,300],
    135: [135,140,145,150,155,160,165,170,175,180,190,195,200,300],
    140: [140,145,150,155,160,165,170,180,185,190,200,300],
    145: [145,150,165,170,200,300],
    150: [150,160,170,175,180,185,190,195,200,300],
    155: [160,170,190,200],
    160: [160,170,175,180,190,195,200,300]
  };

  const GAINHOW_ELLIPSE_SIZE_CODES = new Set((
    "D1015 D1020 D1025 D1030 D1035 D1040 D1045 D1050 D1095 " +
    "D1520 D1525 D1530 D1535 D1540 D1545 D1550 D1555 D1560 " +
    "D2025 D2030 D2035 D2040 D2045 D2050 D2055 D2060 D2070 " +
    "D2530 D2535 D2540 D2545 D2550 D2555 D2560 D2565 D2570 D2595 " +
    "D3035 D3040 D3045 D3050 D3055 D3060 D3065 D3070 D3080 D3085 D3090 D3095 " +
    "D3540 D3545 D3550 D3555 D3560 D3565 D3570 D3575 D3580 D3585 D3595 " +
    "D4045 D4050 D4055 D4060 D4065 D4070 D4075 D4080 D4085 D4090 " +
    "D4550 D4555 D4560 D4565 D4570 D4575 D4580 D4585 D4590 D4595 " +
    "D5055 D5060 D5065 D5070 D5075 D5080 D5085 D5090 D5095 " +
    "D5560 D5565 D5570 D5575 D5580 D5585 D5590 D5595 D6595"
  ).split(/\s+/));

  const GAINHOW_HEART_SIZE_CODES = new Set(
    "E2325 E2830 E3035 E3842 E5052 E5260 E6060 E8797".split(/\s+/)
  );
  ...
  function getTieredCustomShapeExtraFee(quantity) {
    ...
    if (q <= 300) return 200;
    if (q <= 500) return 400;
    if (q <= 3000) return 900;
    if (q <= 4000) return 890;
    if (q <= 5000) return 870;
    if (q <= 6000) return 850;
    if (q <= 7000) return 830;
    if (q <= 8000) return 810;
    if (q <= 9000) return 790;
  ...
  function isCustomShapeValue(shape) {
  ...
  function isExtendedDeliveryShapeValue(shape) {
    const s = String(shape || "").trim().toLowerCase();
    return s === "arch" || isCustomShapeValue(s);
  }

  function requires500Minimum(shape) {
    return isExtendedDeliveryShapeValue(shape);
  }

  function getNormalDeliveryTextByShape(shape) {
    return isExtendedDeliveryShapeValue(shape)
      ? "一般件(6～7 工作天寄出)"
      : "一般件(4～5 工作天寄出)";
  }

  function toGainHowMm(valueCm) {
    const rawMm = Number(valueCm) * 10;
    const roundedMm = Math.round(rawMm);
    if (!Number.isFinite(rawMm) || Math.abs(rawMm - roundedMm) > 0.001) return null;
    return roundedMm;
  }

  function isFiveMmStep(valueMm) {
    return Number.isInteger(valueMm) && valueMm % 5 === 0;
  }

  function isGainHowSquareSize(widthCm, heightCm) {
    const widthMm = toGainHowMm(widthCm);
    const heightMm = toGainHowMm(heightCm);
    if (widthMm === null || heightMm === null) return false;

    const shortMm = Math.min(widthMm, heightMm);
    const longMm = Math.max(widthMm, heightMm);
    if (!isFiveMmStep(shortMm) || !isFiveMmStep(longMm)) return false;

    if (shortMm >= 10 && shortMm <= 100) {
      return (longMm >= shortMm && longMm <= 200) || longMm === 300;
    }

    const allowedLongSides = GAINHOW_SQUARE_LARGE_LONG_SIDES_MM[shortMm];
    return Array.isArray(allowedLongSides) && allowedLongSides.includes(longMm);
  }

  function isGainHowCircleSize(widthCm, heightCm) {
    const widthMm = toGainHowMm(widthCm);
    const heightMm = toGainHowMm(heightCm);
    if (widthMm === null || heightMm === null || widthMm !== heightMm) return false;
    return widthMm >= 10 && widthMm <= 165 && isFiveMmStep(widthMm);
  }

  function isGainHowEllipseSize(widthCm, heightCm) {
    const widthMm = toGainHowMm(widthCm);
    const heightMm = toGainHowMm(heightCm);
    if (widthMm === null || heightMm === null) return false;
    ...
    const shortMm = Math.min(widthMm, heightMm);
    const longMm = Math.max(widthMm, heightMm);
    return GAINHOW_ELLIPSE_SIZE_CODES.has(`D${shortMm}${longMm}`);
  }

  function isGainHowHeartSize(widthCm, heightCm) {
    const widthMm = toGainHowMm(widthCm);
    const heightMm = toGainHowMm(heightCm);
    if (widthMm === null || heightMm === null) return false;
    return GAINHOW_HEART_SIZE_CODES.has(`E${widthMm}${heightMm}`);
  }
  ...
  function normalizeShapeForProductionLimit(shape) {
    ...
    if (s === "circle" || s === "round" || s === "圓形") {
      return "circle";
    }

    if (s === "ellipse" || s === "oval" || s === "橢圓形") {
      return "ellipse";
    }
  ...
  function isWithinProductionSizeLimit(shape, widthCm, heightCm) {
    ...
    if (normalizedShape === "square") {
      return isGainHowSquareSize(w, h);
    }

    if (normalizedShape === "circle") {
      return isGainHowCircleSize(w, h);
    }

    if (normalizedShape === "ellipse") {
      return isGainHowEllipseSize(w, h);
    }

    if (normalizedShape === "heart") {
      return isGainHowHeartSize(w, h);
    }
  ...
  function getSpecQuantityLimitResult(shape, widthCm, heightCm, mappedModule) {
    ...
    return {
      ...
      blocked: true,
      maxQty: 0,
      message: "此尺寸目前無法製作，請依下方尺寸範圍調整，或改選客製形狀"
    };
  }
  ...
  function adjustQuantityOptions(estimatedModule, shape, widthCm, heightCm, mappedModule) {
    ...
    const availableQuantityLevels = requires500Minimum(shape)
      ? QUANTITY_LEVELS.filter(q => q >= CUSTOM_MIN_QUANTITY)
      : QUANTITY_LEVELS;

    quantitySelect.innerHTML = availableQuantityLevels
      .map(q => {
        const disabled = limitResult.blocked || (limitResult.limited && q > limitResult.maxQty)
          ? " disabled"
          : "";
    ...
    if (availableQuantityLevels.includes(current) && (!limitResult.limited || current <= limitResult.maxQty)) {
    ...
    quantitySelect.value = String(availableQuantityLevels[0]);
  }
  ...
  function updateLaminateOptions(material) {
    ...
    if (["glossSilver", "matteSilver"].includes(material)) {
      laminate.innerHTML = `
        <option value="亮膜">亮膜</option>
      `;
      laminate.value = "亮膜";
  ...
  function enforceNormalOnlyUrgency() {
    const urgentSelect = getEl("urgent");
    if (!urgentSelect) return "normal";

    Array.from(urgentSelect.options || []).forEach(option => {
      if (option.value !== "normal") option.remove();
    });

    if (!urgentSelect.querySelector('option[value="normal"]')) {
      const normalOption = document.createElement("option");
      normalOption.value = "normal";
      normalOption.textContent = "一般件(4～5 工作天寄出)";
      urgentSelect.appendChild(normalOption);
    }
  ...
  function updateUrgentOptions() {
    return enforceNormalOnlyUrgency();
  }
  ...
  function getCorrectedPricingModule(widthCm, heightCm, mappedModule) {
    ...
    if (longSide <= 9.5 && shortSide <= 9.5 && area >= 80) {
      return 10;
    }
    ...
    if (longSide <= 11.5 && shortSide <= 11.5 && area >= 115) {
      return 7;
    }
  ...
  function getSmallQuantityRoundingModule(module) {
  ...
  function roundPrice(price, pricingModule, quantity) {
    const amount = Number(price) || 0;
    const module = getSmallQuantityRoundingModule(pricingModule);
    const qty = parseInt(quantity, 10) || 0;

    // 88～12 模的 100 / 300 張保留「市場價減 100 元」的精確金額。
    if ((qty === 100 || qty === 300) && module >= 12 && module <= 88) {
      return Math.round(amount);
    }

    // 11～3 模及其他既有價格維持個位數無條件捨去。
  ...
  function applyUrgentPrice(basePrice, urgent, quantity) {
    // v38 起標籤貼紙不提供急件 / 特急件；保留函式名稱供舊程式呼叫。
  ...
  function applyExtraFees(basePrice, shapeForPricing, urgent, shapeValue, widthCm, heightCm, quantity, pricingModule) {
    ...
    return roundPrice(total, pricingModule, quantity);
  }
  ...
  function getUrgentLabel(_urgent, shapeValue) {
    return getNormalDeliveryTextByShape(shapeValue);
  }
  ...
  function getMaterialLabelForUrl(material, laminate) {
    ...
    if (material === "lowResidueArtpaper") {
      return laminate === "無" ? "低殘膠銅板貼紙" : "低殘膠銅板貼紙+上膜";
    }

    if (material === "lowResiduePearlescent") {
      return laminate === "無"
        ? "低殘膠珠光貼紙"
        : "低殘膠珠光貼紙+上膜";
    }

    if (material === "glossSilver") return "亮銀龍貼紙+亮膜";
    if (material === "matteSilver") return "消光銀龍貼紙+亮膜";
  ...
  function calculatePrice() {
    ...
    if (normalOption) {
      normalOption.textContent = getNormalDeliveryTextByShape(shapeValue);
    }
    ...
    const specLimitResult = getSpecQuantityLimitResult(
      shapeValue,
      widthCm,
      heightCm,
      mappedModule
    ...
    if (specLimitResult.blocked) {
      if (errorMessageDisplay) errorMessageDisplay.textContent = specLimitResult.message;
      if (orderNote) {
        orderNote.textContent = specLimitResult.message;
        orderNote.style.display = "block";
      }
    ...
    if (typeof materialTable === "object" && !Array.isArray(materialTable)) {
      ...
      if (["artpaper", "lowResidueArtpaper"].includes(material) && (laminate === "亮膜" || laminate === "霧膜")) {
        key = "上膜";
      }
    ...
    const total = applyExtraFees(baseTotal, shapeForPricing, urgent, shapeValue, widthCm, heightCm, finalQuantity, pricingModule);
    ...
    if (upgradeHint) {
      ...
      if (nextQty && closest.price[nextQty]) {
        ...
        if (nextSpecUnavailable || nextRushUnavailable || nextSuperrushUnavailable) {
          ...
          const nextTotal = applyExtraFees(nextBaseTotal, shapeForPricing, urgent, shapeValue, widthCm, heightCm, nextQty, pricingModule);
  ...
  window.LUNY_PRICE_ENGINE = {
    ...
    isCustomShapeValue,
    isExtendedDeliveryShapeValue,
    requires500Minimum,
    getNormalDeliveryTextByShape,
    isGainHowSquareSize,
    isGainHowCircleSize,
    isGainHowEllipseSize,
    isGainHowHeartSize,
  ...
  window.CUSTOM_MIN_QUANTITY = CUSTOM_MIN_QUANTITY;
  window.isCustomShapeValue = isCustomShapeValue;
  window.isGainHowSquareSize = isGainHowSquareSize;
  window.isGainHowCircleSize = isGainHowCircleSize;
  window.isGainHowEllipseSize = isGainHowEllipseSize;
  window.isGainHowHeartSize = isGainHowHeartSize;
  ...
  window.getShapeLabel = getShapeLabel;
  window.getUrgentLabel = getUrgentLabel;
  ...
  window.enforceNormalOnlyUrgency = enforceNormalOnlyUrgency;
