/**
 * LUNY 標籤貼紙頁｜上傳與預覽 UX e（新檔，不覆蓋既有程式）
 * Repo：lunytw-collab/luny-tool
 * 檔名：luny-label-upload-preview-ux-20260922e.js
 *
 * 請刪掉 20260922b／c／d，改掛本檔（可與 a、smart-bleed 並存）：
 * <script src="https://cdn.jsdelivr.net/gh/lunytw-collab/luny-tool@main/luny-label-upload-preview-ux-20260922e.js?v=20260922e"></script>
 *
 * 20260922e：雙預覽列掛在 #previews 外並強制顯示（修正被空預覽區藏住）。
 */
(function () {
  "use strict";

  if (window.__LUNY_LABEL_UPLOAD_PREVIEW_UX_20260922E__) return;
  window.__LUNY_LABEL_UPLOAD_PREVIEW_UX_20260922E__ = true;
  ...
  function injectStyle() {
    if ($("lunyUploadPreviewUxStyle20260922e")) return;
    var style = document.createElement("style");
    style.id = "lunyUploadPreviewUxStyle20260922e";
    style.textContent =
      "#lunyPreviewUxMount20260922e{margin:10px 0 12px;}" +
      "#lunyPreviewModeBar20260922e{display:flex;flex-wrap:wrap;align-items:center;gap:8px;margin:0;padding:10px 12px;border:1px solid #e9e4d9;border-radius:12px;background:#fffdf8;}" +
      "#lunyPreviewModeBar20260922e .luny-mode-label{font-size:12px;font-weight:800;color:#6f6860;margin-right:2px;}" +
      "#lunyPreviewModeBar20260922e .luny-mode-btn{appearance:none;border:1px solid #d8d0c3;background:#fff;color:#2a2622;border-radius:999px;padding:7px 12px;font:inherit;font-size:13px;font-weight:800;cursor:pointer;line-height:1.2;}" +
      "#lunyPreviewModeBar20260922e .luny-mode-btn.is-active{border-color:#8b654d;background:#ffdc4d;color:#101010;}" +
      "#lunyPreviewStatus20260922e{flex:1 1 180px;min-width:160px;padding:8px 10px;border-radius:10px;font-size:12.5px;font-weight:800;line-height:1.45;}" +
      "#lunyPreviewStatus20260922e[data-tone=green]{background:#eef9f1;border:1px solid #86c89a;color:#175b31;}" +
      "#lunyPreviewStatus20260922e[data-tone=yellow]{background:#fff8e8;border:1px solid #f0c36a;color:#8a4b16;}" +
      "#lunyPreviewStatus20260922e[data-tone=red]{background:#fff1f0;border:1px solid #f0b4ae;color:#9b2c2c;}" +
      "#lunyPreviewStatus20260922e[data-tone=gray]{background:#f5f3ef;border:1px solid #e0dbd2;color:#6f6860;}" +
  ...
  function ensureMount() {
    var mount = $("lunyPreviewUxMount20260922e");
    ...
    mount.id = "lunyPreviewUxMount20260922e";
  ...
  function ensureBar() {
    ...
    var bar = $("lunyPreviewModeBar20260922e");
    ...
    bar.id = "lunyPreviewModeBar20260922e";
    bar.innerHTML =
      ...
      '<div id="lunyPreviewStatus20260922e" role="status" data-tone="gray">上傳圖片後，這裡會告訴你能不能製作。</div>';
  ...
  function updateStatus(result) {
    ensureBar();
    var el = $("lunyPreviewStatus20260922e");
    if (!el) return;
