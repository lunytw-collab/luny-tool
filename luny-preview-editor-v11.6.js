/* LUNY v7.9.33：預設原始狀態／圖滿版選項整合／安全線外單邊1mm取樣 */
 ...
 * v7.9.33：預設呈現原始圖片，不自動補圖；「圖滿版」改為色彩調整工具中的選項；圖滿版取樣基準改為安全線外單邊 1mm。
...
function isMirrorBleedEdgeMode(){
  ...
  // 預設 off：完全不補圖，呈現原始狀態。
  // 只有色彩調整工具中的「圖滿版」開啟時才執行自動延伸。
  return ['auto','image-full','imagefull','mirror','bleed','full'].includes(edge);
}
function applyMirrorBleedFromCutLine(ctx,canvas,cm2px){
  ...
  if(isMirrorBleedEdgeMode()){
    ...
    return{level:0,zone:'bleed',title:'已開啟圖滿版，系統會抓安全線外單邊 1mm，以低模糊紋理延伸方式補到裁切線外 2mm，盡量保留原本外框設計。',important:false};
  }
  window.__lunyDangerZoneHit=false;
...
(function(){
  ...
  window.getEdgeTextValue = function(value, color){
    ...
    return (window.__lunyFullBleedShouldApplyColor && window.__lunyFullBleedShouldApplyColor())
      ...
      : (isMirrorBleedEdgeMode() ? '圖滿版' : '原始狀態');
  };
