/* LUNY v16.7.4 preflight：互動中停用舊綠燈，放手後以高密度環狀掃描重新判定。 */
...
var LUNY_PREFLIGHT_VERSION='16.7.4';
...
function lunyFlattenedPhotoDangerAssessment(){
  if(!img)return{danger:false,hits:0,samples:0,ratio:0};
  const W=cG.width,H=cG.height,b=BLEED_CM*CM2PX,gap=GAP_CM*CM2PX;
  const cutW=W-2*b,cutH=H-2*b;
  const safeW=Math.max(1,cutW-2*gap),safeH=Math.max(1,cutH-2*gap);
  ...
  return{
    danger:broadDanger||moderateDenseDanger||localizedDanger,
    hits,samples,ratio,maxSectorRatio,maxWindowRatio,hotSectorCount,
    detection:broadDanger?'broad':(moderateDenseDanger?'moderate':(localizedDanger?'localized':'none'))
  };
}
...
function lunyPreflightStateKey(){
  const imageKey=img?`${img.width}x${img.height}`:'none';
  return[
    imageKey,
...
function lunyClassifyPreflight(){
  ...
  if(resolutionLevel==='warning'||resolutionLevel==='unknown'){
    return lunyMakePreflightResult(
      __lunyPreflightWarningAccepted?'WARNING_ACCEPTED':'WARNING_ACCEPTANCE_REQUIRED',
      'yellow',
      ...
      {resolution:resolutionSummary,flattenedRisk,canProceed:__lunyPreflightWarningAccepted}
    );
  }
...
function lunyRenderPreflight(result){
  ...
  if(result.status==='WARNING_ACCEPTANCE_REQUIRED'||result.status==='WARNING_ACCEPTED'){
    decision=`<label style="display:flex;gap:8px;align-items:flex-start;margin-top:12px;font-weight:700;"><input id="lunyPreflightAcceptWarning" type="checkbox" ${__lunyPreflightWarningAccepted?'checked':''} style="margin-top:4px;"><span>我已確認預覽中的文字、Logo 與 QR Code 清楚，仍使用此檔製作。</span></label>`;
  }
  ...
  if(accept)accept.addEventListener('change',function(){__lunyPreflightWarningAccepted=!!accept.checked;lunyUpdatePreflight();});
