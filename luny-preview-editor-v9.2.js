/* LUNY 預覽主程式整合版 v7.9.21-default-mirror-product-preview
 ...
 * v7.9.21：移除模式切換 UI，預設固定補滿版邊緣；改抓安全範圍外 1mm 補到裁切線與裁切線外 2mm；預覽只顯示成品效果。
 */
...
function applyMirrorBleedFromCutLine(ctx,canvas,cm2px){
  ...
  if(canvas.id==='canvasGuides'&&W*H>99999999){
  ...
  if(isMirrorBleedEdgeMode()){
    ...
    if(danger){
      return{level:1,zone:'safe',title:'有圖文內容碰到裁切線內側 2mm 危險區域，建議再往內縮。',important:true};
    }
    return{level:0,zone:'bleed',title:'已選擇補滿版邊緣，系統會自動抓安全範圍外的 1mm 補到裁切線，並延伸到裁切線外 2mm。',important:false};
  }
  window.__lunyDangerZoneHit=false;
...
/* LUNY v7.9.21：移除模式切換 UI，固定使用補滿版邊緣；預覽畫面預設只顯示成品效果。 */
(function(){
  function hideModeUI(){
