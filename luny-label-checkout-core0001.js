...
async function sendCheckoutStarted_(items,total) {
  ...
  try {
    ...
    console.log(
      "✅ checkoutStarted sent"
    );

  } catch (err) {

    console.error(
      "❌ checkoutStarted failed",
      err
    );
...
function getSavedDesigns_() {
  ...
  try {
    ...
    return JSON.parse(
      localStorage.getItem("LUNY_LABEL_SAVED_DESIGNS_V1") || "[]"
    );
    ...
    items.map(function(item,index){
      ...
      return `
        <div class="checkout-summary-item">
          <strong>${index + 1}. ${q.productName || "標籤貼紙"}</strong><br>
...
window.renderCheckoutSummary =
  ...
  const payload = {
    ...
    images: {
      print: {
        filename: "label_print_" + designId + ".png",
        dataURL
      },
      cut: {
        filename: "label_cut_" + designId + ".png",
        dataURL
      },
      preview: {
        filename: "label_preview_" + designId + ".png",
        dataURL
      }
    },
