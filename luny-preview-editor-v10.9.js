/* LUNY 預覽主程式整合版 v7.9.36-qr-min-1-3-visible-warning
 ...
 * v7.9.36：QR Code 最小安全尺寸改為 1.3 × 1.3cm；若主圖 QR 偵測不到，改顯示保險提醒，不再完全無提示。
 */
...
const LUNY_QR_MIN_CM=1.3;let __lunyQrCheckSeq=0,__lunyQrCheckTimer=null,__lunyQrDetector=null,__lunyQrDetectorReady=null,__lunyQrLastStatus=null;
...
function renderQrSizeCheckStatus(st){const box=ensureQrSizeCheckUI();if(!box)return;if(!st){hideQrSizeCheckUI();return;}__lunyQrLastStatus=st;if(st.unknown){box.style.display='block';box.style.background='#fff7ed';box.style.borderColor='#fdba74';box.style.color='#9a3412';box.innerHTML='<div style="font-weight:900;text-align:center;">⚠️ QR Code 尺寸提醒</div><div style="margin-top:4px;text-align:center;">目前沒有成功偵測到 QR Code。</div><div style="margin-top:4px;text-align:center;font-size:12px;color:#9a3412;">若圖中含 QR Code，請確認 QR Code 本體至少 1.3 × 1.3 cm，不含外圍白邊。</div>';return;}const ok=st.w>=LUNY_QR_MIN_CM&&st.h>=LUNY_QR_MIN_CM;const bg=ok?'#f0fff4':'#fff7ed';const bd=ok?'#a8dfb3':'#fdba74';const fg=ok?'#166534':'#9a3412';const prefix=ok?'✅':'⚠️';box.style.display='block';box.style.background=bg;box.style.borderColor=bd;box.style.color=fg;box.innerHTML=`<div style="font-weight:900;text-align:center;">${prefix} QR Code 尺寸檢查</div><div style="margin-top:4px;text-align:center;">${st.source}約 ${formatQrCm(st.w)} × ${formatQrCm(st.h)} cm</div><div style="margin-top:4px;text-align:center;font-size:12px;color:${fg};">${ok?'尺寸足夠，可正常製作。':'建議 QR Code 本體至少 1.3 × 1.3 cm，不含外圍白邊，避免印刷後不易掃描。'}</div>`;}
...
function getIndependentQrSizeStatus(){if(!iconImg)return null;const body=getQrBodyBoxFromImage(iconImg);const bodyW=body?body.w:iconImg.width;const bodyH=body?body.h:iconImg.height;return{source:'QR Code 本體',w:(bodyW*iconScale)/CM2PX,h:(bodyH*iconScale)/CM2PX,type:'icon'};}
function qrSideStatusFromBarcodeResults(results){if(!results||!results.length)return null;let picked=null;results.forEach(r=>{let w=0,h=0;if(r.cornerPoints&&r.cornerPoints.length>=4){const pts=r.cornerPoints;const d01=Math.hypot(pts[1].x-pts[0].x,pts[1].y-pts[0].y);const d12=Math.hypot(pts[2].x-pts[1].x,pts[2].y-pts[1].y);const d23=Math.hypot(pts[3].x-pts[2].x,pts[3].y-pts[2].y);const d30=Math.hypot(pts[0].x-pts[3].x,pts[0].y-pts[3].y);w=(d01+d23)/2;h=(d12+d30)/2;}else if(r.boundingBox){w=r.boundingBox.width;h=r.boundingBox.height;}if(w>0&&h>0){const st={source:'圖中偵測到的 QR Code ',w:w/CM2PX,h:h/CM2PX,type:'detected'};if(!picked||Math.min(st.w,st.h)<Math.min(picked.w,picked.h))picked=st;}});return picked;}
