...
var __lunyPreflightSelectedMethod='';
var __lunyPreflightSelectedImageKey='';
...
function lunyClearPreflightSelectedMethod(){
  __lunyPreflightSelectedMethod='';
  __lunyPreflightSelectedImageKey='';
}
...
function lunyResetArtworkDecisionState(){
  ...
  lunyClearPreflightSelectedMethod();
...
function lunyHasCurrentPreflightMethod(){
  return !!__lunyPreflightSelectedMethod&&
    __lunyPreflightSelectedImageKey===lunyPreflightImageKey();
}
...
document.addEventListener('click',function(event){
  const button=event.target&&event.target.closest
    ...
    :null;
  if(!button)return;
  const action=String(button.dataset.lunyPreflightAction||'');
  if(action==='fit'||action==='white'||action==='color'){
    __lunyPreflightSelectedMethod=action;
    __lunyPreflightSelectedImageKey=lunyPreflightImageKey();
...
function lunyRenderPreflight(result){
  ...
  if(result.edgeChoice||lunyHasCurrentPreflightMethod())actions.push(lunyPreflightButton('上一步','edge-reset',false));
...
/* Public finished-sticker source for packaging/application and checkout previews.
   It keeps the selected white edge while excluding guides, cut lines and handles. */
