...
async function buildFreshSafeOrderPayloadForSave(){
  ...
  if(!payload || !payload.images || !printDataURL || !cutDataURL){
...
async function saveDesignToGAS(){
  ...
  // v7.9.14 關鍵修正：
  // 儲存前置檢查只做出血確認，不在這裡產生圖檔。
  // 圖檔必須在 runSaveAttempt() 內，每次重試前重新產生，避免印刷檔吃到舊 canvas / 切割檔。
  try{
    if(typeof window.lunyConfirmBleedBeforeSave === "function"){
      ...
      console.warn("[LUNY] lunyConfirmBleedBeforeSave 尚未載入，請確認預覽主程式已更新至 v7.9.14 integrated。");
    }
  }catch(err){
...
<script src="https://cdn.jsdelivr.net/gh/lunytw-collab/luny-tool/luny-preview-editor-v7.9.14-integrated.js?v=20260612-1"></script>
