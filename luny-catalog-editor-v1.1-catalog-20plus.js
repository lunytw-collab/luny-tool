/*
 * LUNY Catalog Sticker Editor v1
 * Catalog spec compatibility: 2026-09-07.1
 * 拆自圖鑑貼紙前端頁面。
 * 用途：圖鑑貼紙前端主要互動、儲存設計、checkout bridge、catalogFileUrl 客人雲端資料夾模式。
 */
...
(function resetCatalogCheckoutToken(){try{window.LUNY_PRODUCT_TYPE="CATALOG";window.currentProductType="CATALOG";window.currentProductName="圖鑑貼紙";var t=localStorage.getItem("LUNY_CHECKOUT_TOKEN")||"";if(/^LUNY-LABEL-/i.test(t))localStorage.removeItem("LUNY_CHECKOUT_TOKEN")}catch(e){}})();
(function forceCatalogSaveAreaVisible(){function s(){try{var a=document.getElementById("previewOrderArea");if(a){a.style.display="block";a.style.visibility="visible";a.style.opacity="1"}["saveDesignBtn","orderLink"].forEach(function(id){var b=document.getElementById(id);if(b){b.style.display="";b.style.visibility="visible";b.disabled=false}})}catch(e){}}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",s):s();setTimeout(s,300);setTimeout(s,900)})();
...
function saveSavedDesignsForCheckout(arr){const seen=new Set();const clean=[];(Array.isArray(arr)?arr:[]).forEach(x=>{if(!x||!x.designId)return;const item=normalizeCartItemForProduct(x);let key=String(item.productType||"")+"|"+String(item.designId);try{key+="|"+JSON.stringify(item.quote||{});}catch(e){}if(seen.has(key))return;seen.add(key);clean.push(item);});saveCartItemsForCheckout(clean);const currentType=getLunyProductType();const currentOnly=clean.filter(x=>String(x.productType||"").toUpperCase()===currentType);localStorage.setItem(SAVED_DESIGNS_KEY,JSON.stringify(currentOnly));syncLegacyDesignStorage(clean);}function getLunyProductType(){const explicit=String(window.LUNY_PRODUCT_TYPE||window.currentProductType||"").trim();if(explicit)return explicit.toUpperCase();if(document.getElementById("catalogFileUrl"))return "CATALOG";if(location.pathname.indexOf("fullcut")>=0)return "FULLCUT";return "LABEL";}function makeCheckoutToken(){return"LUNY-"+getLunyProductType()+"-"+new Date().toISOString().slice(0,10).replace(/-/g,"")+"-"+Math.random().toString(36).slice(2,8).toUpperCase();}
function getOrCreateCheckoutToken(){let token=localStorage.getItem(CHECKOUT_TOKEN_KEY)||"";if(getLunyProductType()==="CATALOG"&&/^LUNY-LABEL-/i.test(token)){token="";localStorage.removeItem(CHECKOUT_TOKEN_KEY);}if(!token){token=makeCheckoutToken();localStorage.setItem(CHECKOUT_TOKEN_KEY,token);}return token;}
function createFreshCheckoutToken(){const token=makeCheckoutToken();localStorage.setItem(CHECKOUT_TOKEN_KEY,token);return token;}function waitLunyFrame(){return new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)));}function sleepLuny(ms){return new Promise(resolve=>setTimeout(resolve,ms));}async function waitCanvasReadyForSave(){try{if(typeof drawPreview==="function")drawPreview();}catch(e){console.warn("[LUNY] drawPreview before save failed",e);}await waitLunyFrame();await waitLunyFrame();const source=document.getElementById("canvasGuides");if(!source||!source.width||!source.height){throw new Error("預覽圖尚未準備完成，請稍後再試");}return source;}function makePreviewThumb(maxSize=360,quality=0.76){const source=document.getElementById("canvasGuides");if(!source)return"";try{if(typeof drawPreview==="function")drawPreview();}catch(e){}const sw=source.width||1;const sh=source.height||1;const max=Math.max(160,Number(maxSize)||360);const ratio=Math.min(1,max/sw,max/sh);const w=Math.max(1,Math.round(sw*ratio));const h=Math.max(1,Math.round(sh*ratio));const out=document.createElement("canvas");out.width=w;out.height=h;const ctx=out.getContext("2d");ctx.fillStyle="#ffffff";ctx.fillRect(0,0,w,h);try{ctx.drawImage(source,0,0,w,h);return out.toDataURL("image/jpeg",quality);}catch(e){console.warn("[LUNY] previewThumb 產生失敗",e);return"";}}function getCheckoutTotal(){return loadSavedDesignsForCheckout().reduce((sum,item)=>{return sum+(parseInt(item?.quote?.price||item?.price||"0",10)||0);},0);}function addSavedDesignToCheckoutList(item){
  saveLastEditProductUrl();
  const productType=String(item?.productType||getLunyProductType()).toUpperCase();
  const productName=productType==="CATALOG"?"圖鑑貼紙":(productType==="FULLCUT"?"全斷貼紙":"標籤貼紙");
...
function getCheckoutProductName(item){
  ...
  if(rawType === "CATALOG" || rawCode.indexOf("圖鑑") >= 0) return "圖鑑貼紙";
...
function getCheckoutProductTypeForGroup(item){
  const rawType = String(item && item.productType || "").toUpperCase();
  const rawCode = String(item && item.productCode || "");
  if(rawType === "CATALOG" || rawCode.indexOf("圖鑑") >= 0) return "CATALOG";
...
function renderCheckoutSummary(){
  ...
  list.innerHTML=groups
    ...
    .map(group=>{
      return `
<div class="checkout-product-group" style="margin:14px 0 18px;"><div class="checkout-product-title" style="font-weight:700;font-size:15px;color:#111827;margin:0 0 8px;text-align:left;">${group.name}</div>
  ${group.items.map((row,groupIndex)=>{
    ...
    return `
  <div class="checkout-design-item"><img class="checkout-design-thumb" src="${preview}" alt="${group.name}第 ${groupIndex + 1} 款預覽圖"><div><div class="checkout-design-title">${groupIndex + 1}. ${group.name}｜小計 NT$ ${row.price}</div><div class="checkout-design-info">
        ...
        數量：${q.quantity || ""} 張<br>
        急件：${urgentText}
      </div></div><div class="checkout-design-actions"><button type="button" class="checkout-delete-btn" onclick="deleteSavedDesign(${row.index})">刪除此款</button></div></div>`;
  }).join("")}
</div>`;
    })
    .join("");
...
function normalizeCatalogLaminateForPayload(value){
  const raw=String(value||"").trim();
  const lower=raw.toLowerCase();
  if(lower==="gloss"||raw.indexOf("亮")>=0)return"gloss";
  if(lower==="matte"||raw.indexOf("霧")>=0||raw.indexOf("雾")>=0)return"matte";
  return lower||"gloss";
}
function normalizeCatalogUrgentForPayload(value){
  const raw=String(value||"").trim();
  const lower=raw.toLowerCase();
  if(lower==="rush"||raw.indexOf("急件")>=0)return"rush";
  return"normal";
}
function normalizeCatalogCutlineForPayload(value){
  const raw=String(value||"").trim();
  const lower=raw.toLowerCase();
  if(lower==="designer"||raw.indexOf("設計師")>=0||raw.indexOf("设计师")>=0)return"designer";
  return"self";
}
function getCatalogDomValue(id,fallback){
  const el=document.getElementById(id);
  if(!el)return fallback;
  if(el.type==="checkbox")return !!el.checked;
  return String(el.value||"").trim();
}
function getCatalogQuoteDirect(){
  const catalogSize=getCatalogDomValue("catalogSize","A5")||"A5";
  const material=normalizeCatalogMaterialForPayload(getCatalogDomValue("material","pearlescent"));
  const laminate=normalizeCatalogLaminateForPayload(getCatalogDomValue("laminate","gloss"));
  const quantity=parseInt(getCatalogDomValue("quantity","20"),10)||20;
  const urgent=normalizeCatalogUrgentForPayload(getCatalogDomValue("urgent","normal"));
  const cutlineService=normalizeCatalogCutlineForPayload(getCatalogDomValue("catalogCutlineService","self"));
  if(window.LUNY_CATALOG_PRICING&&typeof window.LUNY_CATALOG_PRICING.getPrice==="function"){
    const q=window.LUNY_CATALOG_PRICING.getPrice({
      material,
      laminate,
      size:catalogSize,
      quantity,
    ...
    return Object.assign({},q,{
      ...
      productCode:"圖鑑貼紙",
      status:"待人工檢查",
      summary:`圖鑑貼紙｜${catalogSize}｜${q.materialText}｜${q.laminateText}｜${quantity}張｜${q.urgentText}｜${q.cutlineServiceText}｜$${q.price}`
    });
  }
  const materialText=getMaterialTextValue(material);
  const laminateText=getLaminateTextValue(laminate);
  ...
  return{
    ...
    catalogSize,
    size:catalogSize,
    sizeText:catalogSize,
    quantity,
    urgent,
    urgentText,
    urgentFee:urgent==="rush"?300:0,
    cutlineService,
    cutlineServiceText,
    cutlineFee:cutlineService==="designer"?600:0,
    basePrice:0,
    price:parseInt(document.getElementById("price")?.textContent||"0",10)||0,
    total:parseInt(document.getElementById("price")?.textContent||"0",10)||0,
    status:"待人工檢查"
  };
}
function makeCatalogPreviewThumb(q){
  try{
...
function buildOrderPayload(){
  const quote=getCatalogQuoteDirect();
  const catalogFileUrl=getCatalogDomValue("catalogFileUrl","");
  const catalogShareChecked=!!getCatalogDomValue("catalogShareChecked",false);
  const catalogNote=getCatalogDomValue("catalogNote","");
  const catalogPreviewDataURL=makeCatalogPreviewThumb(quote);
  return{
    productType:"CATALOG",
    productCode:"圖鑑貼紙",
    productName:"圖鑑貼紙",
    ...
    catalogNote,
    catalog:{
      fileUrl:catalogFileUrl,
      shareChecked:catalogShareChecked,
      note:catalogNote,
      status:"待人工檢查"
    },
    fileMode:"catalog_folder",
    printFileUrl:catalogFileUrl,
    cutFileUrl:catalogFileUrl,
    designFolderUrl:catalogFileUrl,
    images:{
      preview:{
        filename:"catalog_preview.png",
        dataURL:catalogPreviewDataURL
      }
    },
    previewThumb:catalogPreviewDataURL,
    previewUrl:catalogPreviewDataURL,
    previewDataUrl:catalogPreviewDataURL,
    thumbnail:catalogPreviewDataURL,
    status:"待人工檢查",
...
function catalogCm(s){s=String(s||"").toUpperCase();return s==="A4"?["21","29.7"]:s==="A5"?["14.8","21"]:s==="A6"?["10.5","14.8"]:s==="A7"?["7.4","10.5"]:["",""]}function fillCatalogCm(q){q=q||{};var c=catalogCm(q.catalogSize||q.size||q.sizeText);if(c[0]&&!q.widthCm)q.widthCm=c[0];if(c[1]&&!q.heightCm)q.heightCm=c[1];if(c[0]&&!q.width)q.width=c[0];if(c[1]&&!q.height)q.height=c[1];return q}
function makeCatalogDraftDataURL(q){
  try{
    const c=document.createElement("canvas");c.width=480;c.height=320;
    const x=c.getContext("2d");
    x.fillStyle="#fff";x.fillRect(0,0,480,320);
    x.strokeStyle="#d1d5db";x.lineWidth=4;x.strokeRect(24,24,432,272);
    x.fillStyle="#111827";x.textAlign="center";x.font="700 32px -apple-system,BlinkMacSystemFont,'Noto Sans TC',sans-serif";
    x.fillText("圖鑑貼紙",240,110);
    x.fillStyle="#4b5563";x.font="500 19px -apple-system,BlinkMacSystemFont,'Noto Sans TC',sans-serif";
    x.fillText(String(q.catalogSize||q.size||"")+"｜"+String(q.materialText||""),240,160);
    x.fillText(String(q.laminateText||"")+"｜"+String(q.quantity||"")+"張",240,195);
    x.fillText("雲端連結型商品｜待人工檢查",240,235);
    return c.toDataURL("image/png");
  }catch(e){
    return"data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMB/atZMxkAAAAASUVORK5CYII=";
  }
}
async function saveCatalogDesignDraftToGAS(savedItem,payload){
  const q=fillCatalogCm(savedItem.quote||{});savedItem.quote=q;
  const placeholder=makeCatalogDraftDataURL(q);
  const catalogFileUrl=String(savedItem.catalogFileUrl||payload?.catalogFileUrl||"").trim();
  const bodyObj={
    type:"design",
    v:4,
    designId:savedItem.designId,
    savedAt:savedItem.createdAt||new Date().toISOString(),
    productType:"CATALOG",
    productCode:"圖鑑貼紙",
    productName:"圖鑑貼紙",
    quote:q,
    ...
    catalogShareChecked:!!savedItem.catalogShareChecked,
    catalogNote:savedItem.catalogNote||"",
    catalog:savedItem.catalog||{fileUrl:catalogFileUrl,status:"待人工檢查"},
    fileMode:"catalog_folder",
    printFileUrl:catalogFileUrl,
    cutFileUrl:catalogFileUrl,
    designFolderUrl:catalogFileUrl,
    ...
    images:{
      preview:{
        filename:"catalog_preview_"+savedItem.designId+".png",
        dataURL:placeholder
      }
    },
    previewThumb:savedItem.previewThumb||placeholder,
    previewUrl:savedItem.previewThumb||placeholder,
    previewDataUrl:savedItem.previewThumb||placeholder,
    thumbnail:savedItem.previewThumb||placeholder,
    page:{href:location.href,path:location.pathname,title:document.title}
  };
  const json=await postJsonToGAS(GAS_SAVE_URL,bodyObj);
  if(!json||!json.ok){
    throw new Error((json&&json.error)?json.error:"GAS 圖鑑草稿儲存失敗");
  }
  return json;
}
async function saveDesignToGAS(){
  if(window.__LUNY_SAVE_DESIGN_GLOBAL_LOCK__){console.warn("[LUNY CATALOG] save ignored: global lock");return null;}
  if(isSavingDesign){console.warn("[LUNY CATALOG] save ignored: already saving");return null;}
  ...
  try{
    ...
    const q=fillCatalogCm(payload.quote||{});payload.quote=q;
    const catalogFileUrl=String(payload.catalogFileUrl||"").trim();
    if(!catalogFileUrl){
      alert("請先貼上設計檔雲端連結。");
      throw new Error("請先貼上設計檔雲端連結");
    }
    if(!/^https?:\/\/.+/i.test(catalogFileUrl)){
      alert("請貼上完整網址，需要包含 https://");
      throw new Error("雲端連結格式不正確");
    }
    if(!payload.catalogShareChecked){
      alert("請先確認雲端連結已開啟共享權限。");
      throw new Error("尚未確認共享權限");
    }
    if(!q.price){
      alert("此規格尚未設定價格，請重新選擇。");
      throw new Error("此規格尚未設定價格");
    }
    const finalDesignId=newDesignId();
    const previewDataUrl=payload.previewThumb||makeCatalogPreviewThumb(q);
    const savedItem={
      ...
      heightCm:q.heightCm||"",
      price:q.price||0,
      catalogFileUrl:payload.catalogFileUrl||"",
      catalogShareChecked:!!payload.catalogShareChecked,
      catalogNote:payload.catalogNote||"",
      catalog:payload.catalog||{},
      fileMode:"catalog_folder",
      printFileUrl:payload.catalogFileUrl||"",
      cutFileUrl:payload.catalogFileUrl||"",
      designFolderUrl:payload.catalogFileUrl||"",
      status:"待人工檢查",
    ...
    setSaveStatus("正在建立圖鑑貼紙草稿…");
    const draftJson=await saveCatalogDesignDraftToGAS(savedItem,payload);
    if(draftJson&&draftJson.designId){
      savedItem.designId=draftJson.designId;
    }
    ...
    isDesignReadyForCheckout=false;
    console.error("[saveDesignToGAS catalog direct] failed:",err);
    setSaveStatus("儲存未完成："+(err&&err.message?err.message:"請確認資料後再按一次"));
  ...
  const checkoutPayload = {
    ...
    items: designs.map((item,index)=>{
      ...
      return {
        ...
        catalogFileUrl:item.catalogFileUrl||item.printFileUrl||item.designFolderUrl||"",
        catalogShareChecked:!!item.catalogShareChecked,
        catalogNote:item.catalogNote||"",
        catalog:item.catalog||{},
        fileMode:item.fileMode||"",
        printFileUrl:item.printFileUrl||item.catalogFileUrl||"",
        cutFileUrl:item.cutFileUrl||item.catalogFileUrl||"",
        designFolderUrl:item.designFolderUrl||item.catalogFileUrl||"",
        previewThumb: preview,
        previewUrl: preview,
        previewDataUrl: preview,
        thumbnail: preview
