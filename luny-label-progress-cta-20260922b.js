/**
 * LUNY 標籤貼紙頁｜CTA 內嵌四步進度 b（新檔，不覆蓋既有程式）
 * Repo：lunytw-collab/luny-tool
 * 檔名：luny-label-progress-cta-20260922b.js
 *
 * <script src="https://cdn.jsdelivr.net/gh/lunytw-collab/luny-tool@main/luny-label-progress-cta-20260922b.js?v=20260922b"></script>
 *
 * 20260922b：
 * - 修「一進頁就在第 3 步」：材質／尺寸須實際操作過才算完成（不因預設值跳過）
 * - 仍整合在 CTA 內；藏舊 STEP pill；不改 canProceed／報價／DPI
 */
(function () {
  "use strict";

  if (window.__LUNY_LABEL_PROGRESS_CTA_20260922B__) return;
  window.__LUNY_LABEL_PROGRESS_CTA_20260922B__ = true;
  ...
  function injectStyle() {
    if ($("lunyProgressCtaStyle20260922b")) return;
    var style = document.createElement("style");
    style.id = "lunyProgressCtaStyle20260922b";
    style.textContent =
      ...
      "#lunyProgressInCta20260922b .luny-pc-label{" +
      "margin:0;color:#6b7280;font-size:11px;font-weight:700;line-height:1.35;}" +
      "#lunyProgressInCta20260922b .luny-pc-steps{" +
      "display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:4px;}" +
      "#lunyProgressInCta20260922b .luny-pc-step{" +
      ...
      "#lunyProgressInCta20260922b .luny-pc-step[data-state='done']{" +
      "border-color:#c6dfce;background:#edf7f0;color:#25603b;}" +
      "#lunyProgressInCta20260922b .luny-pc-step[data-state='current']{" +
      "border-color:#e6c84a;background:#fff8d6;color:#202020;box-shadow:0 1px 0 rgba(0,0,0,.04);}" +
      "#lunyProgressInCta20260922b .luny-pc-step[data-state='todo']{" +
      "opacity:.72;cursor:default;}" +
      "#lunyProgressInCta20260922b .luny-pc-step:disabled{cursor:default;}" +
      /* 手機：塞進 dock 最上方 */
      "#lunyUXDock #lunyProgressInCta20260922b{" +
      "grid-column:1/-1;padding:0 0 2px;border-bottom:1px solid #eee;margin:0 0 6px;}" +
      "@media (max-width:720px){" +
      "#lunyUXDock #lunyProgressInCta20260922b .luny-pc-step{min-height:30px;font-size:10px;padding:5px 2px;}" +
      "#lunyUXDock #lunyProgressInCta20260922b .luny-pc-label{font-size:10px;}" +
      "body{padding-bottom:220px!important;}" +
      "}" +
      /* 電腦：主按鈕上方 */
      "#lunyUXUploadAnchor #lunyProgressInCta20260922b," +
      "#lunyProgressCtaDesktopHost20260922b #lunyProgressInCta20260922b{" +
      "margin:0 0 10px;}" +
      "#lunyProgressCtaDesktopHost20260922b{margin:10px 0 0;}";
  ...
  function sizeReady() {
    ...
    var shape = (($s = $("shape")) && $s.value || "").toLowerCase();
    if (shape === "custom") {
      var side = Number(($s = $("customLongSideCm")) && $s.value || 0);
      return side >= 1;
    }
    if (shape === "circle") {
      var d = Number(($s = $("widthCm")) && $s.value || 0);
      return d >= 1;
    }
    var w = Number(($s = $("widthCm")) && $s.value || 0);
    var h = Number(($s = $("heightCm")) && $s.value || 0);
    return w >= 1 && h >= 1;
  }
  ...
  function buildMount() {
    ...
    root.id = "lunyProgressInCta20260922b";
    root.setAttribute("aria-label", "訂購進度");
    root.innerHTML =
      '<p class="luny-pc-label" id="lunyProgressCtaLabel20260922b"></p>' +
  ...
  function ensureMount() {
    injectStyle();
    var existing = $("lunyProgressInCta20260922b");
    var dock = $("lunyUXDock");
    ...
    if (mobile && dock) {
      ...
      var deskHost = $("lunyProgressCtaDesktopHost20260922b");
      if (deskHost) deskHost.hidden = true;
      return $("lunyProgressInCta20260922b");
    }

    // Desktop: above primary CTA in upload anchor, else quote action grid
    var anchor = $("lunyUXUploadAnchor");
    var host = $("lunyProgressCtaDesktopHost20260922b");
    if (!host) {
      host = document.createElement("div");
      host.id = "lunyProgressCtaDesktopHost20260922b";
  ...
  function render() {
    ...
    var label = $("lunyProgressCtaLabel20260922b");
    if (label && cur) {
      var text =
        st.done[4]
          ? "已完成・可前往結帳或再做一款"
