/* LUNY label order flow v9.2-phase5.5-file-prep
   Phase 5.5：每款設計保存 preflight 結果；基礎檔案整理固定 NT$300 並計入 1SHOP 結帳總額。
   ...
   預覽與 print/cut 產生由配套的 luny-preview-editor-v16.6-phase3.js 負責。
*/
...
window.LUNY_PRODUCT_TYPE = window.LUNY_PRODUCT_TYPE || "LABEL";
window.currentProductType = window.currentProductType || window.LUNY_PRODUCT_TYPE || "LABEL";
window.currentProductName = window.currentProductName || "標籤貼紙";
...
function makeLunyRequestId(scope){
  ...
  try{
    ...
    random=Array.from(bytes).map(v=>v.toString(36)).join("");
  }catch(e){
    random=Math.random().toString(36).slice(2)+Math.random().toString(36).slice(2);
  }
  return `req_${String(scope||"request")}_${now}_${random.slice(0,24)}`;
}
function getStableLunyRequestId(scope,checkoutToken,orderNo){
  const identity=[String(scope||"request"),String(checkoutToken||""),String(orderNo||"")].join("::");
  const key=LUNY_REQUEST_ID_PREFIX+identity;
  let requestId="";
  try{requestId=sessionStorage.getItem(key)||localStorage.getItem(key)||"";}catch(e){}
  if(!requestId){
    requestId=makeLunyRequestId(scope);
    try{sessionStorage.setItem(key,requestId);}catch(e){}
    try{localStorage.setItem(key,requestId);}catch(e){}
  }
  return requestId;
}
window.LUNY_getStableRequestId=getStableLunyRequestId;
const DESIGN_ID_KEY="LUNY_DESIGN_ID";const PENDING_DESIGN_IDS_KEY="LUNY_PENDING_DESIGN_IDS";const PENDING_DESIGN_BACKUP_KEY="LUNY_PENDING_DESIGN_BACKUP_V1";const PERSIST_SECONDS=60*60*24*14;const PENDING_DESIGNS_KEY="LUNY_PENDING_DESIGNS_V1";const LAST_SAVED_DESIGN_KEY="LUNY_LAST_SAVED_DESIGN_V1";const CART_KEY_STORAGE="LUNY_CART_KEY";const GROUP_ID_STORAGE="LUNY_GROUP_ID";const ORDER_SESSION_STORAGE="LUNY_ORDER_SESSION_ID";const SAVED_DESIGNS_KEY="LUNY_SAVED_DESIGNS_V2";const CHECKOUT_TOTAL_KEY="LUNY_CHECKOUT_TOTAL_AMOUNT";const CHECKOUT_PAYLOAD_KEY="LUNY_CHECKOUT_PAYLOAD_V2";const CHECKOUT_TOKEN_KEY="LUNY_CHECKOUT_TOKEN";const SHARED_CART_ITEMS_KEY="LUNY_CART_ITEMS_V1";const CONFIRMED_ORDERS_KEY="LUNY_CONFIRMED_ORDERS_V1";const ORDER_PREVIEW_SNAPSHOT_KEY="LUNY_LAST_ORDER_PREVIEW_SNAPSHOT_V1";const ORDER_COMPLETED_AT_KEY="LUNY_LAST_ORDER_COMPLETED_AT";const CHECKOUT_IN_PROGRESS_KEY="LUNY_CHECKOUT_IN_PROGRESS_V1";const LEGACY_DESIGN_ID_KEYS=["latestDesignId","pendingDesignIds","lunyDesignIds","luny_order_draft_ids",PENDING_DESIGN_IDS_KEY];const LEGACY_DRAFTS_KEY="pendingDesignDrafts";const CHECKOUT_PRODUCT_URL=window.LUNY_CHECKOUT_PRODUCT_URL||"https://www.luny.tw/label-stickers#Type=Product&ID=JZ8LKanp10ZOX20KkXVbQw04";const LUNY_CHECKOUT_CONFIRM_URL=window.LUNY_CHECKOUT_CONFIRM_URL||"https://www.luny.tw/checkout-confirm";const LUNY_CONTINUE_SHOPPING_URL=window.LUNY_CONTINUE_SHOPPING_URL||"https://www.luny.tw/#sticker-products";const LAST_EDIT_PRODUCT_URL_KEY="LUNY_LAST_EDIT_PRODUCT_URL";
...
function lunyRoundMs_(value){
  return Math.round((Number(value)||0)*10)/10;
}
function lunyErrorTimingInfo_(err){
  return {
    errorCode:String(err&&err.code||""),
    errorName:String(err&&err.name||""),
    errorMessage:String(err&&err.message?err.message:err||"").slice(0,240)
  };
}
function createLunySaveTiming_(){
  const trace={
    version:LUNY_SAVE_TIMING_VERSION,
    traceId:"save_"+Date.now().toString(36)+"_"+Math.random().toString(36).slice(2,8),
    startedAt:new Date().toISOString(),
    startMs:lunyTimingNow_(),
    productType:String(getLunyProductType()).toUpperCase(),
...
function markLunySaveTiming_(trace,stage,details){
  if(!trace||trace.finished)return null;
  const entry={
    stage:String(stage||"stage"),
    elapsedMs:lunyRoundMs_(lunyTimingNow_()-trace.startMs),
  ...
  const output={
    ...
    designId:trace.designId||"",
    ...entry
  };
  console.log(`[LUNY TIMING] ${entry.stage}`,output);
  return entry;
}
function finishLunySaveTiming_(trace,outcome,details){
  if(!trace)return null;
  if(trace.finished)return window.__LUNY_LAST_SAVE_TIMING__||null;
  markLunySaveTiming_(trace,"save_finished",{outcome:String(outcome||"unknown"),...(details||{})});
  trace.finished=true;
  const summary={
    version:trace.version,
    traceId:trace.traceId,
    ...
    totalMs:lunyRoundMs_(lunyTimingNow_()-trace.startMs),
    gasAttempts:Number(trace.gasAttempts||0),
    stages:trace.stages.slice()
  };
  window.__LUNY_LAST_SAVE_TIMING__=summary;
  try{sessionStorage.setItem(LUNY_LAST_SAVE_TIMING_KEY,JSON.stringify(summary));}catch(e){}
  console.log("[LUNY TIMING SUMMARY]",summary);
  try{console.log("[LUNY TIMING SUMMARY JSON] "+JSON.stringify(summary));}catch(e){}
  return summary;
}
window.LUNY_SAVE_TIMING_VERSION=LUNY_SAVE_TIMING_VERSION;
window.LUNY_getLastSaveTiming=function(){
  if(window.__LUNY_LAST_SAVE_TIMING__)return window.__LUNY_LAST_SAVE_TIMING__;
  try{return JSON.parse(sessionStorage.getItem(LUNY_LAST_SAVE_TIMING_KEY)||"null");}catch(e){return null;}
};
async function hashDesignBlob_(blob,label){
  if(!blob)return"";
  const timingTrace=window.__LUNY_ACTIVE_SAVE_TIMING__||null;
  const startedMs=lunyTimingNow_();
  try{
    if(
      ...
      markLunySaveTiming_(timingTrace,`fingerprint_${String(label||"blob")}_ready`,{
        durationMs:lunyRoundMs_(lunyTimingNow_()-startedMs),
    ...
    console.warn("[LUNY] 正式圖檔指紋計算失敗，改用檔案資訊",err);
    markLunySaveTiming_(timingTrace,`fingerprint_${String(label||"blob")}_fallback`,{
      durationMs:lunyRoundMs_(lunyTimingNow_()-startedMs),
      sizeBytes:Number(blob.size||0),
      method:"file-info",
      ...lunyErrorTimingInfo_(err)
    });
  }
  if(timingTrace&&!timingTrace.stages.some(item=>item.stage===`fingerprint_${String(label||"blob")}_ready`||item.stage===`fingerprint_${String(label||"blob")}_fallback`)){
    markLunySaveTiming_(timingTrace,`fingerprint_${String(label||"blob")}_fallback`,{
      durationMs:lunyRoundMs_(lunyTimingNow_()-startedMs),
      sizeBytes:Number(blob.size||0),
      method:"file-info"
  ...
  return [String(blob.type||""),String(blob.size||0)].join(":");
}
async function makeDesignFingerprint(payload,formalAssets){
  try{
    const q=payload&&payload.quote?payload.quote:{};
    const imgFile=document.getElementById("imgFile")?.files?.[0]||null;
    const iconFile=document.getElementById("iconFile")?.files?.[0]||null;
    const hashes=await Promise.all([
      hashDesignBlob_(formalAssets?.print?.blob,"print"),
      hashDesignBlob_(formalAssets?.cut?.blob,"cut")
    ]);
    return [
      "v2",
      String(window.LUNY_PRODUCT_TYPE||window.currentProductType||""),
      String(location.pathname||""),
      q.shape||"",q.widthCm||"",q.heightCm||"",q.material||"",q.laminate||"",q.quantity||"",q.urgent||"",q.edgeOption||"",q.price||"",
      imgFile?.name||"",imgFile?.size||0,imgFile?.lastModified||0,
      iconFile?.name||"",iconFile?.size||0,iconFile?.lastModified||0,
    ...
    console.warn("[LUNY] 設計指紋建立失敗，使用本次安全識別",err);
    return [
      "fallback",
      String(window.LUNY_PRODUCT_TYPE||window.currentProductType||""),
      String(location.pathname||""),
...
function readPendingDesignSave_(){
  try{
    const raw=sessionStorage.getItem(LUNY_PENDING_DESIGN_SAVE_KEY);
    const parsed=raw?JSON.parse(raw):null;
    if(!parsed||!parsed.designId||!parsed.fp)return null;
    if(Date.now()-Number(parsed.at||0)>LUNY_PENDING_DESIGN_SAVE_MAX_AGE_MS){
      sessionStorage.removeItem(LUNY_PENDING_DESIGN_SAVE_KEY);
      return null;
    }
    return parsed;
  }catch(e){
...
function rememberPendingDesignSave_(entry){
  window.__LUNY_LAST_SAVE_FINGERPRINT__=entry;
  window.__LUNY_CURRENT_SAVE_DESIGN_ID__=entry.designId;
  try{
    sessionStorage.setItem(
      LUNY_PENDING_DESIGN_SAVE_KEY,
      JSON.stringify({
        fp:entry.fp,
...
async function getIdempotentDesignId(payload,formalAssets){
  const fp=await makeDesignFingerprint(payload,formalAssets);
  const now=Date.now();
  const memory=window.__LUNY_LAST_SAVE_FINGERPRINT__||null;
  const memoryMaxAge=memory&&memory.finished
    ? LUNY_FINISHED_DESIGN_DEDUPE_MS
    : LUNY_PENDING_DESIGN_SAVE_MAX_AGE_MS;

  if(
    memory&&
    memory.fp===fp&&
    memory.designId&&
    now-Number(memory.at||0)<memoryMaxAge
  ){
    console.warn("[LUNY] reuse designId for same design",memory.designId);
    if(!memory.finished)rememberPendingDesignSave_({...memory,at:now});
    return memory.designId;
  }

  const pending=readPendingDesignSave_();
  if(pending&&pending.fp===fp){
    const reused={...pending,at:now,pending:true,finished:false};
    rememberPendingDesignSave_(reused);
    console.warn("[LUNY] reconcile pending design with same designId",reused.designId);
    return reused.designId;
  }

  const designId=newDesignId();
  rememberPendingDesignSave_({fp,at:now,designId,pending:true,finished:false});
  return designId;
}
function markDesignSaveFinished(designId){
  try{
    const cache=window.__LUNY_LAST_SAVE_FINGERPRINT__;
    if(cache&&cache.designId===designId){
      window.__LUNY_LAST_SAVE_FINGERPRINT__={
        ...cache,
        at:Date.now(),
        pending:false,
        finished:true
...
<div class="checkout-product-group" style="margin:14px 0 18px;">
  ...
  ${group.items.map((row,groupIndex)=>{
    const item=row.item;
    const q=item.quote||{};
    const filePrep=q.filePrep||{};
    const filePrepSelected=!!filePrep.selected;
    const filePrepFee=parseInt(q.filePrepFee||filePrep.fee||"0",10)||0;
    const basePrice=parseInt(q.basePrice||Math.max(0,row.price-filePrepFee),10)||0;
    ...
    const isNameSticker=String(item.productType||"").toUpperCase()==="NAME_STICKER" || /姓名貼/.test(String(item.productCode||""));
    const quantityText=isNameSticker ? (q.quantityText||"一份") : `${q.quantity||""} 張`;
    const edgeText=q.edgeText||(typeof getEdgeTextValue === "function" ? getEdgeTextValue(q.edgeOption, q.edgeColor) : (q.edgeOption === "on" ? "加白邊" : `滿版底色 ${q.edgeColor||""}`));
  ...
  <div class="checkout-design-item">
    ...
    <div>
      ...
      ${filePrepSelected?`<div style="margin:3px 0 5px;color:#8f1d2c;font-size:12px;font-weight:700;">商品 NT$ ${basePrice} ＋ 印刷檔案基礎整理 NT$ ${filePrepFee}</div>`:''}
      <div class="checkout-design-info">
        ...
        邊框：${edgeText}<br>
        數量：${quantityText}<br>
        急件：${urgentText}${filePrepSelected?`<br>檔案整理：已加購，整理完成確認後起算製作天數`:''}
      </div>
    </div>
...
cloned.imageInfo={
removed:true,
reason:"top_level_design_images_moved_to_cloud_storage",
hasPrint:!!(cloned.images&&cloned.images.print),
hasCut:!!(cloned.images&&cloned.images.cut)
};
...
}async function postJsonToGAS(url,obj,timeoutMs){
const controller=(typeof AbortController!=="undefined")?new AbortController():null;
const timer=controller?setTimeout(()=>controller.abort(),Number(timeoutMs)||30000):null;
try{
const options={method:"POST",headers:{"Content-Type":"text/plain;charset=UTF-8"},body:JSON.stringify(obj)};
...
try{
  ...
  const err=new Error("GAS 回應暫時無法確認，將以相同設計編號重試。");
  err.code="GAS_NON_JSON_RESPONSE";
  err.retryable=true;
  err.responseStatus=Number(res&&res.status||0);
  err.responseContentType=String(res&&res.headers&&res.headers.get("content-type")||"");
  try{err.responseOrigin=new URL(res.url).origin;}catch(urlErr){err.responseOrigin="";}
  err.responsePreview=String(text||"").replace(/\s+/g," ").slice(0,240);
  console.warn("[LUNY] GAS 回應非 JSON",{
    code:err.code,
    status:err.responseStatus,
    contentType:err.responseContentType,
    origin:err.responseOrigin,
    preview:err.responsePreview
  });
  throw err;
}
return json;
...
function isRetryableGasResult_(json){
...
function isRetryableGasError_(err){
  if(!err)return false;
  ...
  for(let attempt=1;attempt<=attempts;attempt++){
    if(attempt>1){
      ...
      if(typeof options.onRetry==="function"){
        try{options.onRetry(attempt,waitMs,lastJson,lastErr);}catch(e){}
      }

      markLunySaveTiming_(timingTrace,"gas_retry_wait",{
        attempt,
        waitMs,
        priorResultCode:String(lastJson&&lastJson.code||""),
        priorErrorCode:String(lastErr&&lastErr.code||"")
      });
    ...
    try{
      ...
      lastErr=err;
      lastJson=null;
      markLunySaveTiming_(timingTrace,"gas_attempt_error",{
        attempt,
        durationMs:lunyRoundMs_(lunyTimingNow_()-gasAttemptStartedMs),
        ...lunyErrorTimingInfo_(err)
      });
      if(!isRetryableGasError_(err)||attempt>=attempts)throw err;
    }
  }
...
const LUNY_UPLOAD_SIGNER_URL=window.LUNY_UPLOAD_SIGNER_URL||"https://luny-upload-signer-377746408360.asia-east1.run.app";
// 客戶原圖壓縮規格：完整保留原始範圍，不放大、不套用任何預覽編輯。
// 4600px 可涵蓋目前最大貼紙尺寸約 300ppi 的需求；同時限制總像素，降低手機記憶體壓力。
const LUNY_CUSTOMER_SOURCE_MAX_SIDE=4600;
const LUNY_CUSTOMER_SOURCE_MAX_PIXELS=16000000;
const LUNY_CUSTOMER_SOURCE_JPEG_QUALITY=0.92;
function buildPublicGcsUrlFromSigned_(signed){
  signed=signed||{};
  const bucket=String(signed.bucket||"").trim();
  const objectName=String(signed.objectName||"").trim();
  if(!bucket||!objectName)return "";
  return "https://storage.googleapis.com/"+bucket+"/"+objectName.split("/").map(encodeURIComponent).join("/");
}
async function requestSignedUploadUrl(designId,assetType,contentType){
const res=await fetch(`${LUNY_UPLOAD_SIGNER_URL}/create-upload-url`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({designId,assetType,contentType:contentType||"image/png"})});
const json=await res.json();
if(!json||!json.ok){throw new Error((json&&json.error)?json.error:"取得雲端上傳網址失敗");}
return json;
}
async function uploadBlobToSignedUrl(uploadUrl,blob,contentType){
const res=await fetch(uploadUrl,{method:"PUT",headers:{"Content-Type":contentType||blob.type||"image/png"},body:blob});
if(!res.ok){throw new Error(`上傳雲端失敗：HTTP ${res.status}`);}
return true;
}
async function uploadBlobAssetToCloud(args){
const designId=args&&args.designId;
const assetType=args&&args.assetType;
const blob=args&&args.blob;
const contentType=args&&args.contentType||blob&&blob.type||"image/png";
const timingTrace=window.__LUNY_ACTIVE_SAVE_TIMING__||null;
if(!designId)throw new Error("缺少 designId，無法上傳圖檔");
if(!assetType)throw new Error("缺少 assetType，無法上傳圖檔");
if(!blob||typeof blob.size!=="number"||blob.size<=0)throw new Error(`${assetType} 沒有可上傳的 Blob 圖檔`);
const signerStartedMs=lunyTimingNow_();
let signed=null;
try{
  signed=await requestSignedUploadUrl(designId,assetType,contentType);
  markLunySaveTiming_(timingTrace,`upload_${assetType}_signed_url_ready`,{
    durationMs:lunyRoundMs_(lunyTimingNow_()-signerStartedMs),
    sizeBytes:Number(blob.size||0)
  });
}catch(err){
  markLunySaveTiming_(timingTrace,`upload_${assetType}_signed_url_error`,{
    durationMs:lunyRoundMs_(lunyTimingNow_()-signerStartedMs),
    sizeBytes:Number(blob.size||0),
    ...lunyErrorTimingInfo_(err)
  });
...
console.log(`[LUNY] 準備上傳 ${assetType}`,{sizeKB:Math.round(blob.size/1024),contentType,objectName:signed.objectName});
const uploadStartedMs=lunyTimingNow_();
try{
  await uploadBlobToSignedUrl(signed.uploadUrl,blob,contentType);
  markLunySaveTiming_(timingTrace,`upload_${assetType}_complete`,{
    durationMs:lunyRoundMs_(lunyTimingNow_()-uploadStartedMs),
    sizeBytes:Number(blob.size||0)
  });
}catch(err){
  markLunySaveTiming_(timingTrace,`upload_${assetType}_error`,{
    durationMs:lunyRoundMs_(lunyTimingNow_()-uploadStartedMs),
    sizeBytes:Number(blob.size||0),
    ...lunyErrorTimingInfo_(err)
  });
...
const publicUrl=signed.publicUrl||buildPublicGcsUrlFromSigned_(signed);
return {
  assetType,
  objectName:signed.objectName||"",
  fileUrl:publicUrl||signed.fileUrl||"",
  gsUrl:signed.fileUrl||"",
...
function getCustomerSourceFile_(){
  try{
    return document.getElementById("imgFile")?.files?.[0]||null;
  }catch(e){
    return null;
  }
}
function canvasToLunyBlob_(canvas,contentType,quality,label){
    ...
    try{
      canvas.toBlob(blob=>{
        if(blob&&blob.size>0){
          ...
          reject(new Error((label||"客戶原圖")+"轉換失敗，未產生有效圖檔"));
        }
      },contentType,quality);
    }catch(err){
      reject(err);
...
async function makePreviewThumbBlob_(maxSize=220,quality=0.62){
  const source=document.getElementById("canvasGuides");
  if(!source)throw new Error("找不到預覽畫布，無法產生預覽圖 Blob");
  try{if(typeof drawPreview==="function")drawPreview();}catch(e){}
  const sw=source.width||1;
  const sh=source.height||1;
  const max=Math.max(160,Number(maxSize)||220);
  ...
  try{
    ...
    if(!ctx)throw new Error("瀏覽器無法建立預覽圖畫布");
    ctx.fillStyle="#ffffff";
    ctx.fillRect(0,0,w,h);
    ctx.drawImage(source,0,0,w,h);
    if(typeof window.LUNY_decoratePreviewThumbCanvas==="function"){
      window.LUNY_decoratePreviewThumbCanvas(out);
    }
    const blob=await canvasToLunyBlob_(out,"image/jpeg",quality,"預覽圖");
    return {blob,contentType:"image/jpeg",widthPx:w,heightPx:h,sizeBytes:blob.size};
  }finally{
    try{out.width=1;out.height=1;}catch(e){}
  }
}
async function decodeCustomerSourceImage_(file){
    ...
    try{
      ...
      if(bitmap&&bitmap.width&&bitmap.height){
        return {
          ...
          cleanup:function(){
            try{if(typeof bitmap.close==="function")bitmap.close();}catch(e){}
          }
        };
      }
    }catch(err){
      console.warn("[LUNY] createImageBitmap 讀取客戶原圖失敗，改用 Image 備援",err);
    }
  }
  ...
  return await new Promise((resolve,reject)=>{
    ...
    img.onerror=function(){
      ...
      settled=true;
      cleanupUrl();
      reject(new Error("客戶原圖格式無法由瀏覽器讀取"));
    };
    img.src=objectUrl;
  });
}
async function compressCustomerSourceFile_(file){
  if(!file)throw new Error("找不到客戶最初上傳的原圖");
  if(!String(file.type||"").toLowerCase().startsWith("image/")){
    throw new Error("客戶原圖不是可讀取的圖片格式");
  }

  const decoded=await decodeCustomerSourceImage_(file);
  let canvas=null;
  try{
    const sourceWidth=Math.max(1,Number(decoded.width||0));
    const sourceHeight=Math.max(1,Number(decoded.height||0));
    const sideRatio=Math.min(1,LUNY_CUSTOMER_SOURCE_MAX_SIDE/Math.max(sourceWidth,sourceHeight));
    const pixelRatio=Math.min(1,Math.sqrt(LUNY_CUSTOMER_SOURCE_MAX_PIXELS/(sourceWidth*sourceHeight)));
    const ratio=Math.min(sideRatio,pixelRatio,1);
    ...
    if(!ctx)throw new Error("瀏覽器無法建立客戶原圖壓縮畫布");

    // 只做完整範圍等比例縮小；不套用預覽畫布的裁切、位移、旋轉、出血、底色或調色。
    ctx.clearRect(0,0,outputWidth,outputHeight);
    ctx.imageSmoothingEnabled=true;
    ctx.imageSmoothingQuality="high";
    ctx.drawImage(decoded.source,0,0,sourceWidth,sourceHeight,0,0,outputWidth,outputHeight);

    const originalType=String(file.type||"").toLowerCase();
    const keepPng=originalType==="image/png";
    const contentType=keepPng?"image/png":"image/jpeg";
    const quality=keepPng?undefined:LUNY_CUSTOMER_SOURCE_JPEG_QUALITY;
    const blob=await canvasToLunyBlob_(canvas,contentType,quality);
    ...
    return {
      ...
      outputWidth,
      outputHeight,
      compressed:ratio<1||blob.size<Number(file.size||0)
    };
  }finally{
    try{if(decoded&&typeof decoded.cleanup==="function")decoded.cleanup();}catch(e){}
    if(canvas){
      try{canvas.width=1;canvas.height=1;}catch(e){}
    }
  }
}
async function uploadCustomerSourceToCloud_(args){
  const designId=args&&args.designId;
  ...
  let prepared=null;
  try{
    prepared=await compressCustomerSourceFile_(file);
    markLunySaveTiming_(timingTrace,"customer_source_compress_ready",{
      durationMs:lunyRoundMs_(lunyTimingNow_()-compressStartedMs),
      originalSizeBytes:Number(file.size||0),
      uploadSizeBytes:Number(prepared&&prepared.blob&&prepared.blob.size||0)
    });
  }catch(err){
    markLunySaveTiming_(timingTrace,"customer_source_compress_error",{
      durationMs:lunyRoundMs_(lunyTimingNow_()-compressStartedMs),
      originalSizeBytes:Number(file.size||0),
      ...lunyErrorTimingInfo_(err)
    });
  ...
  // 客戶原圖使用自己的單檔重試；不改動既有 preview／print／cut 的產生與驗證規則。
  for(let attempt=1;attempt<=2;attempt++){
    try{
      ...
      const signerStartedMs=lunyTimingNow_();
      const signed=await requestSignedUploadUrl(designId,"customer_source",prepared.contentType);
      markLunySaveTiming_(timingTrace,"upload_customer_source_signed_url_ready",{
        attempt,
        durationMs:lunyRoundMs_(lunyTimingNow_()-signerStartedMs),
        sizeBytes:Number(prepared.blob.size||0)
      });
      console.log("[LUNY] 準備上傳 customer_source",{
        originalFilename:prepared.originalFilename,
        originalSizeKB:Math.round(prepared.originalSizeBytes/1024),
        uploadSizeKB:Math.round(prepared.blob.size/1024),
        sourceSize:prepared.sourceWidth+"x"+prepared.sourceHeight,
        outputSize:prepared.outputWidth+"x"+prepared.outputHeight,
        contentType:prepared.contentType,
        objectName:signed.objectName,
        attempt
      });
      const uploadStartedMs=lunyTimingNow_();
      await uploadBlobToSignedUrl(signed.uploadUrl,prepared.blob,prepared.contentType);
      markLunySaveTiming_(timingTrace,"upload_customer_source_complete",{
        attempt,
        durationMs:lunyRoundMs_(lunyTimingNow_()-uploadStartedMs),
        sizeBytes:Number(prepared.blob.size||0)
      });
      ...
      lastErr=err;
      markLunySaveTiming_(timingTrace,"upload_customer_source_attempt_error",{
        attempt,
        ...lunyErrorTimingInfo_(err)
      });
      console.warn(`[LUNY] customer_source 第 ${attempt} 次上傳失敗`,err);
    }
  }

  throw lastErr||new Error("客戶原圖上傳失敗");
}
const LUNY_ASSET_UPLOAD_CONCURRENCY=2;

async function runLunyUploadJobsWithLimit_(jobs,limit){
  ...
  const workers=Array.from({length:workerCount},async()=>{
    while(true){
      ...
      try{
        ...
        failures.push({key:job.key,label:job.label,error:err});
      }
    }
  });
  await Promise.all(workers);
  if(failures.length){
    const error=new Error(failures.map(item=>`${item.label}：${item.error&&item.error.message?item.error.message:item.error}`).join("；"));
    error.assetFailures=failures;
    throw error;
  }
}

function createLunyUploadState_(customerSourceRequired){
  return {
    preview:null,
    print:null,
    cut:null,
    customerSource:null,
    customerSourceRequired:!!customerSourceRequired
  };
}

async function uploadDesignAssetsToCloud(designId,formalAssets,previewAsset,existingResult){
  const customerSourceFile=getCustomerSourceFile_();
  const result=existingResult||createLunyUploadState_(!!customerSourceFile);
  result.customerSourceRequired=!!customerSourceFile;
  const jobs=[];

  function addBlobJob(key,label,assetType,asset,statusText){
    if(result[key])return;
    const blob=asset&&asset.blob;
    ...
    jobs.push({
      ...
      run:async()=>{
        ...
        result[key]=await uploadBlobAssetToCloud({
          ...
          assetType,
          blob,
          contentType:asset.contentType||blob.type||"image/png"
        });
  ...
  addBlobJob("preview","預覽圖","preview",previewAsset,"正在上傳預覽圖…");
  addBlobJob("print","印刷檔","print",formalAssets&&formalAssets.print,"正在上傳印刷圖…");
  addBlobJob("cut","切割檔","cut",formalAssets&&formalAssets.cut,"正在上傳刀模圖…");

  if(customerSourceFile&&!result.customerSource){
    jobs.push({
      key:"customerSource",
      label:"客戶原圖",
      run:async()=>{
...
function renderLunyOrderPreviewOnThankYouPage(){return;}function getPendingDesignIds(){hydratePendingDesignStorageFromAllSources();const ids=[];function add(list){(list||[]).forEach(id=>{const v=String(id||"").trim();if(v&&!ids.includes(v))ids.push(v);});}add(uniqueDesignIdsFromItems(loadPendingDesigns()));add(uniqueDesignIdsFromItems(getCheckoutPayloadItems()));add(uniqueDesignIdsFromItems(loadSavedDesignsForCheckout()));add(readDesignIdsFromLocation());add(readDesignIdsFromStorageText(getLunyCookie(PENDING_DESIGN_IDS_KEY)));const backup=readBackupObject();if(backup){add(backup.designIds||[]);add(uniqueDesignIdsFromItems(backup.items||[]));}add(readDesignIdsFromStorageKey(PENDING_DESIGN_IDS_KEY));add(readDesignIdsFromStorageKey(DESIGN_ID_KEY));LEGACY_DESIGN_ID_KEYS.forEach(key=>add(readDesignIdsFromStorageKey(key)));return ids;}function clearPendingDesignsAfterConfirmed(){window.__lc=1;try{localStorage.setItem(ORDER_COMPLETED_AT_KEY,String(Date.now()));}catch(e){}[PENDING_DESIGNS_KEY,DESIGN_ID_KEY,LAST_SAVED_DESIGN_KEY,SAVED_DESIGNS_KEY,CHECKOUT_TOTAL_KEY,CHECKOUT_PAYLOAD_KEY,CHECKOUT_TOKEN_KEY,PENDING_DESIGN_IDS_KEY,PENDING_DESIGN_BACKUP_KEY,CART_KEY_STORAGE,ORDER_SESSION_STORAGE,CHECKOUT_IN_PROGRESS_KEY,"LUNY_PENDING_ORDER_V1",LEGACY_DRAFTS_KEY,"latestDesignId","pendingDesignIds","lunyDesignIds","luny_order_draft_ids"].forEach(key=>{try{localStorage.removeItem(key);}catch(e){}try{sessionStorage.removeItem(key);}catch(e){}});removeLunyCookie(PENDING_DESIGN_IDS_KEY);removeLunyCookie(PENDING_DESIGN_BACKUP_KEY);LEGACY_DESIGN_ID_KEYS.forEach(key=>{try{localStorage.removeItem(key);}catch(e){}});}function clearLegacyCheckoutGhosts(){try{[PENDING_DESIGNS_KEY,LAST_SAVED_DESIGN_KEY,CHECKOUT_PAYLOAD_KEY,CHECKOUT_TOTAL_KEY,CHECKOUT_TOKEN_KEY,PENDING_DESIGN_IDS_KEY,PENDING_DESIGN_BACKUP_KEY,CART_KEY_STORAGE,ORDER_SESSION_STORAGE,CHECKOUT_IN_PROGRESS_KEY,"LUNY_PENDING_ORDER_V1","latestDesignId","pendingDesignIds","lunyDesignIds","luny_order_draft_ids",LEGACY_DRAFTS_KEY].forEach(key=>{localStorage.removeItem(key);try{sessionStorage.removeItem(key);}catch(e){}});removeLunyCookie(PENDING_DESIGN_IDS_KEY);removeLunyCookie(PENDING_DESIGN_BACKUP_KEY);}catch(e){console.warn("[LUNY] clearLegacyCheckoutGhosts failed",e);}}async function confirmOrderToGAS(orderNo,receiverName){
  ...
  const bindRequestId=getStableLunyRequestId(
    "bind_order",
    checkoutToken,
    finalOrderNo
  );
  ...
  const bodyObj={
    ...
    phase2FrontendVersion:LUNY_PHASE2_FRONTEND_VERSION,
  ...
  const json=await postJsonToGASWithRetry(
    ...
    stripLargeImageDataForSheet(bodyObj),
    {
      ...
      onRetry:function(attempt,waitMs){
        console.warn("[LUNY] 正式訂單回寫忙碌，準備重試",{
          attempt:attempt,
          waitMs:waitMs,
          orderNo:finalOrderNo
        });
  ...
  await logOrderDebugToGAS(
    "write_order_success",
    {
      ...
      productionSync:json.productionSync||"queued",
      queueStatus:json.queueStatus||"pending",
...
async function buildFreshSafeOrderPayloadForSave(){
  ...
  if(typeof window.getPrintAndCutBlobs!=="function"){
    throw new Error("Phase 3 正式輸出尚未載入，找不到 getPrintAndCutBlobs()。");
  }
  const formalAssets=await window.getPrintAndCutBlobs();
  if(!formalAssets?.print?.blob||!formalAssets?.cut?.blob){
    throw new Error("印刷檔或切割檔 Blob 產生失敗，請重新儲存設計。");
  }
  return {payload,formalAssets};
}
async function saveDesignToGAS(){
  ...
  if(!btn){return null;}
  let activeDesignId="";
  let designMetadataWriteStarted=false;
  let backendDesignConfirmed=false;
  let saveConfirmed=false;
  const saveTiming=createLunySaveTiming_();
  window.__LUNY_ACTIVE_SAVE_TIMING__=saveTiming;
  markLunySaveTiming_(saveTiming,"save_started");

  try{
    if(typeof window.lunyConfirmBleedBeforeSave==="function"){
      const ok=window.lunyConfirmBleedBeforeSave();
      if(!ok){
        finishLunySaveTiming_(saveTiming,"cancelled",{reason:"bleed-check"});
        if(window.__LUNY_ACTIVE_SAVE_TIMING__===saveTiming)window.__LUNY_ACTIVE_SAVE_TIMING__=null;
        markLunyBleedSaveCancelled();
        return null;
      }
    }else{
      const unavailableError=new Error("印前檔案檢查尚未載入，已停止儲存。請重新整理頁面後再試。");
      unavailableError.code="LUNY_PREFLIGHT_UNAVAILABLE";
      finishLunySaveTiming_(saveTiming,"cancelled",{reason:"preflight-unavailable"});
      if(window.__LUNY_ACTIVE_SAVE_TIMING__===saveTiming)window.__LUNY_ACTIVE_SAVE_TIMING__=null;
      if(typeof setSaveStatus==="function")setSaveStatus(unavailableError.message);
      alert(unavailableError.message);
      return null;
    }
    markLunySaveTiming_(saveTiming,"bleed_check_done");
  }catch(err){
    if(isLunyBleedRiskCancelError(err)){
      finishLunySaveTiming_(saveTiming,"cancelled",{reason:"bleed-check-error"});
      if(window.__LUNY_ACTIVE_SAVE_TIMING__===saveTiming)window.__LUNY_ACTIVE_SAVE_TIMING__=null;
      markLunyBleedSaveCancelled();
      return null;
    }
    finishLunySaveTiming_(saveTiming,"error",lunyErrorTimingInfo_(err));
    if(window.__LUNY_ACTIVE_SAVE_TIMING__===saveTiming)window.__LUNY_ACTIVE_SAVE_TIMING__=null;
    throw err;
  }
  ...
  try{
    ...
    const preparedSave=await buildFreshSafeOrderPayloadForSave();
    const payload=preparedSave.payload;
    const formalAssets=preparedSave.formalAssets;
    markLunySaveTiming_(saveTiming,"formal_assets_ready",{
      durationMs:lunyRoundMs_(lunyTimingNow_()-formalExportStartedMs),
    ...
    const previewAsset=await makePreviewThumbBlob_(220,0.62);
    markLunySaveTiming_(saveTiming,"preview_blob_ready",{
    ...
    const uploadRetryDelays=[0,1200,2500];
    ...
    for(let attempt=1;attempt<=3;attempt++){
      try{
        ...
        startUploadCountdown(attempt===1?25:30);
        const uploadAttemptStartedMs=lunyTimingNow_();
        markLunySaveTiming_(saveTiming,"cloud_upload_attempt_start",{attempt});
        await uploadDesignAssetsToCloud(
          designId,
          formalAssets,
          previewAsset,
          uploadedAssets
        );
        assertUploadedDesignAssetsReady(uploadedAssets);
        markLunySaveTiming_(saveTiming,"cloud_upload_attempt_complete",{
          attempt,
          durationMs:lunyRoundMs_(lunyTimingNow_()-uploadAttemptStartedMs)
        });
        uploadErr=null;
        uploadComplete=true;
        break;
        ...
        uploadErr=err;
        clearUploadCountdown();
        markLunySaveTiming_(saveTiming,"cloud_upload_attempt_error",{
          attempt,
          ...lunyErrorTimingInfo_(err)
        });
        console.warn(`[LUNY] 第 ${attempt} 次圖檔上傳失敗`,err);
      }
    }

    if(!uploadComplete){
      throw uploadErr||new Error("圖檔上傳失敗，請稍後再試");
    }

    const assets=buildCloudAssetMeta(uploadedAssets);
    markLunySaveTiming_(saveTiming,"cloud_assets_ready",{
      previewSizeBytes:Number(assets.previewSizeBytes||0),
      printSizeBytes:Number(assets.printSizeBytes||0),
      cutSizeBytes:Number(assets.cutSizeBytes||0),
      customerSourceSizeBytes:Number(assets.customerSourceSizeBytes||0)
    });
    const bodyObj={
      type:"design",
      v:5,
      ...
      productCode:payload.productCode||window.currentProductName||"",
      productType:getLunyProductType(),
      quote:payload.quote||{},
      page:payload.page||{href:location.href,path:location.pathname,title:document.title},
      assets,
      assetStatus:"done",
      ...
      stripLargeImageDataForSheet(bodyObj),
      {
        ...
        onRetry:function(attempt,waitMs,lastJson,lastErr){
          ...
          setSaveProgress(
            85,
            isResponseUncertain
              ? `伺服器回應確認中；將以同一設計編號進行第 ${attempt} 次安全對帳`
              : `伺服器忙碌；正在進行第 ${attempt} 次資料寫入重試`
          );
        }
      }
    );
    ...
    const finalPreviewDataUrl=makePreviewThumb(220,0.62);
    ...
    const savedItem={
      ...
      createdAt:new Date().toISOString(),
      quote:payload.quote||{},
      previewThumb:finalPreviewDataUrl,
      assets:assets,
      assetStatus:"done",
      cloudStorage:{enabled:true,bucket:"luny-design-assets-414ec27a"}
    };
    ...
    saveConfirmed=true;
    rememberLastEditProductUrl();
    markLunySaveTiming_(saveTiming,"local_checkout_list_ready",{
    ...
    if(isLunyBleedRiskCancelError(err)){
      finishLunySaveTiming_(saveTiming,"cancelled",{reason:"bleed-risk",...lunyErrorTimingInfo_(err)});
      markLunyBleedSaveCancelled();
      return null;
    }

    console.error("[saveDesignToGAS] failed:",err);
    finishLunySaveTiming_(saveTiming,"error",{
      activeDesignId,
      designMetadataWriteStarted,
      backendDesignConfirmed,
      ...lunyErrorTimingInfo_(err)
    });
    const responseUncertain=
      designMetadataWriteStarted&&
      isRetryableGasError_(err);
    if(responseUncertain){
      setSaveStatus("圖檔已上傳，但伺服器回應仍無法確認。請按一次「儲存設計」安全對帳；系統會沿用同一設計編號，不會重複建立。");
      setSaveButtonText("重新確認儲存");
    }else if(backendDesignConfirmed){
      setSaveStatus("設計資料已由伺服器確認，但本頁清單尚未更新完成。請按一次「儲存設計」以同一設計編號重新同步。");
      setSaveButtonText("重新同步設計");
    }else{
  ...
  const bodyObj={
    ...
    checkoutTotal:checkoutPayload.total||0,
    total:checkoutPayload.total||0,
    groupId:checkoutPayload.groupId||checkoutPayload.cartKey||"",
    cartKey:checkoutPayload.cartKey||checkoutPayload.groupId||"",
    orderSessionId:checkoutPayload.orderSessionId||"",
    productType:checkoutPayload.productType||getLunyProductType(),
    designIds,
    designIdsCount:designIds.length,
    itemsCount:(checkoutPayload.items||[]).length,
    items:checkoutPayload.items||[],
    checkoutPayload:{...checkoutPayload,requestId:checkoutRequestId},
  ...
  const json=await postJsonToGASWithRetry(
    ...
    stripLargeImageDataForSheet(bodyObj),
    {
      ...
      onRetry:function(attempt,waitMs){
        if(typeof setSaveStatus==="function"){
          setSaveStatus(`結帳資料忙碌，${Math.ceil(waitMs/1000)} 秒後自動重試…`);
        }
      }
    }
  );
  ...
  if(!json||!json.ok){
    throw new Error((json&&json.error)?json.error:"checkout_started 寫入失敗");
  }
  ...
  function getCurrentQuote(){
    ...
    return {
      ...
      edgeColor: document.getElementById("edgeColor")?.value || document.getElementById("bgColor")?.value || "#ffffff",
      edgeText: (typeof getEdgeTextValue === "function" ? getEdgeTextValue(document.querySelector('input[name="edgeOption"]:checked')?.value || "off", document.getElementById("edgeColor")?.value || document.getElementById("bgColor")?.value || "#ffffff") : ((document.querySelector('input[name="edgeOption"]:checked')?.value || "off") === "on" ? "加白邊" : "滿版底色")),
...
/* 專用付款商品的 1SHOP 數量由購物車同步程式負責。
 * 訂單流程不得在一般商品／編輯頁全域改寫 input[name="Quantity"]，
 * 避免舊的結帳總額干擾商品頁報價與平均單價初始化。
 */
...
/* Phase 5.5 authoritative quote builder. The base product price remains the
   existing quote; file preparation is an explicit per-design surcharge. */
function buildOrderPayload(){
  const edgeOption=document.querySelector('input[name="edgeOption"]:checked')?.value||"off";
  const edgeColor=document.getElementById("edgeColor")?.value||document.getElementById("bgColor")?.value||"#ffffff";
  const basePrice=parseInt(document.getElementById("price")?.textContent||"0",10)||0;
  const preflight=typeof window.LUNY_getPreflightResult==="function"
    ? window.LUNY_getPreflightResult()
    : {version:"unavailable",status:"BLOCKED_REUPLOAD_REQUIRED",level:"red",title:"印前檔案檢查未載入",issues:["系統無法確認出血、安全距離與解析度。"],recommendations:["請重新整理頁面後再試。"],canProceed:false,filePrep:{selected:false,fee:0,status:"not_selected"}};
  const filePrep=Object.assign({
    selected:false,
    sku:"FILE_PREP_BASIC",
    label:"印刷檔案基礎整理",
    fee:0,
  ...
  const filePrepFee=filePrep.selected?(parseInt(filePrep.fee||filePrep.unitFee||"300",10)||300):0;
  const price=basePrice+filePrepFee;
  const persistedPreflight={
    version:String(preflight.version||""),
    status:String(preflight.status||""),
    level:String(preflight.level||""),
    title:String(preflight.title||""),
    canProceed:preflight.canProceed===true,
    issues:Array.isArray(preflight.issues)?preflight.issues.slice(0,8):[],
    recommendations:Array.isArray(preflight.recommendations)?preflight.recommendations.slice(0,8):[],
    resolution:preflight.resolution||{},
    checkedAt:preflight.checkedAt||new Date().toISOString()
  };

  return{
    productCode:window.currentProductName||"",
    quote:{
      ...
      filePrep,
      summary:(typeof currentSummary==="string")?currentSummary:"",
      shapeText:getShapeTextValue(document.getElementById("shape")?.value||""),
      materialText:getMaterialTextValue(document.getElementById("material")?.value||""),
      laminateText:getLaminateTextValue(document.getElementById("laminate")?.value||""),
      urgentText:getUrgentTextValue(document.getElementById("urgent")?.value||"")
    },
    page:{href:location.href,path:location.pathname,title:document.title}
  };
}
window.LUNY_FILE_PREP_BASIC={sku:"FILE_PREP_BASIC",label:"印刷檔案基礎整理",unitFee:300};
