...
/**
 * LUNY 1SHOP cart absolute reconciliation v10
...
(function installLunyOneShopCartV10(){
  ...
  window.__LUNY_ONESHOP_CART_ABSOLUTE_V10__ = "2026-08-09.1";
  ...
  function publishCartSyncState(status, bridge, message){
    var params = readParams();
    var state = {
      version:10,
      status:text(status),
  ...
  async function reconcileBeforeNativeAdd(originalButton){
    ...
    reconcileFlight = (async function(){
      ...
      try{
        ...
        if(!bridge) throw new Error("找不到本次有效的確認頁付款資料，請返回確認頁重新操作。");
        ...
        publishCartSyncState("error", bridge, String(error && error.message || error));
  ...
  function startAutomaticAdd(){
    ...
    if(automaticAddStartedKey === key) return automaticAddFlight;
    if(automaticAddFlight) return automaticAddFlight;
    automaticAddStartedKey = key;
    ...
    automaticAddFlight = (async function(){
      ...
      var succeeded = await reconcileBeforeNativeAdd(button);
      return succeeded;
    })().finally(function(){
