 ...
 * Version: 2026-09-07.20.6.2
 * Load at the original checkout-confirm script position without async/defer.
 */
window.__LUNY_CHECKOUT_CONFIRM_CART_READY_FIX_V206__ = "2026-09-07.20.6.2-catalog-spec";
 ...
 * 6. v20.5：依 GAS 的 1SHOP 核對結果區分正式訂單與未確認 Token 衝突
 */
...
const CHECKOUT_TOKEN_BOUND_PREFIX = "LUNY_CHECKOUT_RECONCILIATION_V2::";
...
const LUNY_V17_VERSION = "2026-08-26.20.5";
...
function LUNY_V17_tokenBoundStorageKey(token){
  return CHECKOUT_TOKEN_BOUND_PREFIX + String(token || "").trim();
}

function LUNY_V17_isTokenBoundPrewriteError(error){
  if(!error) return false;
  const code = String(error.code || "").trim().toUpperCase();
  const message = String(error.message || "");
  return (
    code === "CHECKOUT_TOKEN_ALREADY_BOUND" ||
    code === "CHECKOUT_TOKEN_ALREADY_BELONGS_TO_ORDER" ||
    code === "CHECKOUT_TOKEN_CONFLICT" ||
    code === "CHECKOUT_ALREADY_ORDERED" ||
    code === "ONESHOP_ORDER_EXISTS" ||
    /checkoutToken already belongs to an order/i.test(message)
  );
}

function LUNY_V17_markTokenBound(token, error){
  const normalizedToken = String(token || "").trim();
  if(!normalizedToken) return null;

  const now = Date.now();
  const oneShopOrderExists = !!(
    error &&
    error.code === "ONESHOP_ORDER_EXISTS" &&
    error.oneShopVerified === true &&
    error.orderExists === true
  );
  const marker = {
    v:1,
    token:normalizedToken,
    orderState:oneShopOrderExists
      ? "ONESHOP_ORDER_EXISTS"
      : "ONESHOP_ORDER_NOT_CONFIRMED",
    code:String(error && error.code || "CHECKOUT_TOKEN_ALREADY_BOUND"),
    message:String(error && error.message || "").slice(0, 300),
    orderNo:String(error && error.orderNo || ""),
    paymentStatus:String(error && error.paymentStatus || ""),
    oneShopVerified:oneShopOrderExists,
    markedAt:now,
    expiresAt:now + LUNY_V17_TTL_MS
  };

  safeStorageSet(
    localStorage,
    LUNY_V17_tokenBoundStorageKey(normalizedToken),
...
function LUNY_V17_getTokenBoundMarker(token){
  const normalizedToken = String(token || "").trim();
  if(!normalizedToken) return null;

  const key = LUNY_V17_tokenBoundStorageKey(normalizedToken);
  let marker = null;
  try{ marker = safeParseJson(localStorage.getItem(key), null); }catch(e){ return null; }
  if(
    !marker ||
    String(marker.token || "") !== normalizedToken ||
    Number(marker.expiresAt || 0) <= Date.now()
  ){
...
function LUNY_V17_getCurrentTokenBoundMarker(){
  let token = "";
  try{ token = localStorage.getItem(CHECKOUT_TOKEN_KEY) || ""; }catch(e){}
  return LUNY_V17_getTokenBoundMarker(token);
}

function LUNY_V17_renderTokenBoundState(marker){
  if(!marker || !marker.token) return false;
  ...
  window.__LUNY_CHECKOUT_UI_LOCKED__ = true;
  ...
  const oneShopOrderExists = !!(
    marker.orderState === "ONESHOP_ORDER_EXISTS" &&
    marker.oneShopVerified === true &&
    marker.orderNo
  );
  ...
  getCheckoutActionButtons().forEach(function(button){
    ...
    button.textContent = oneShopOrderExists
      ? "1SHOP 訂單已存在｜停止重送"
      : "對帳編號待核對｜停止重送";
  });
  ...
  if(status){
    ...
    status.textContent = oneShopOrderExists
      ? (
        "1SHOP 已確認存在正式訂單：" + marker.orderNo +
        "｜付款狀態：" + (marker.paymentStatus || "未提供") +
        "｜對帳編號：" + marker.token
      )
      : (
        "這個對帳編號已被後端關聯，但尚未確認 1SHOP 正式訂單。" +
        "系統已停止重送，請以 1SHOP 訂單查詢結果為準。對帳編號：" + marker.token
      );
  }
  return true;
}
...
function getCatalogMaterialText(value){
  const map = {
    pearlescent:"防水合成紙",
    shtte:"模造貼紙",
    transparent:"透明貼紙（加白墨）"
  };
  return map[value] || getMaterialText(value);
}
...
function updateCheckoutButtonState(){
  ...
  const tokenBoundMarker = LUNY_V17_getCurrentTokenBoundMarker();
  if(tokenBoundMarker){
    LUNY_V17_renderTokenBoundState(tokenBoundMarker);
    return;
  }
      ...
      return `
        <section class="checkout-product-group">
          ...
          ${group.items.map((row, groupIndex) => {
            ...
            const materialText = q.materialText || (
              group.type === "CATALOG"
                ? getCatalogMaterialText(q.material)
                : getMaterialText(q.material)
            );
...
function beginCheckoutLaunchMonitor(){
  ...
  checkoutLaunchMonitorTimer = setInterval(function(){
    ...
    if(location.search && location.search.length > 1){
      routeParts.push(location.search.slice(1));
    }
    if(location.hash && location.hash.length > 1){
      routeParts.push(location.hash.slice(1).replace(/^!/, ""));
    }

    routeParts.join("&").split("&").forEach(function(pair){
      if(!pair) return;
      ...
      if(key) routeParams[key] = value;
    });
    let bridge = null;

    try{
      bridge = JSON.parse(localStorage.getItem("LUNY_ONESHOP_CHECKOUT_V17") || "null");
      if(bridge && Number(bridge.expiresAt || 0) <= Date.now()) bridge = null;
    }catch(e){
      bridge = null;
    }

    const expectedRewriteKey = String(
      window.__LUNY_EXPECTED_CART_REWRITE_KEY__ ||
      routeParams.cartRewriteKey ||
      routeParams.rewriteKey ||
      (bridge && bridge.cartRewriteKey) ||
      ""
    ).trim();
    const expectedCheckoutToken = String(
      routeParams.checkoutToken ||
      routeParams.token ||
      (bridge && bridge.checkoutToken) ||
      ""
    ).trim();
    const expectedTotal = Number(
      routeParams.checkoutTotal ||
      routeParams.luny_qty ||
      (bridge && bridge.total) ||
      0
    );
    ...
    const matchingCartSyncState = !!(
      expectedRewriteKey &&
      /^LUNY-[A-Z0-9-]{12,160}$/i.test(expectedCheckoutToken) &&
      Number.isInteger(expectedTotal) &&
      expectedTotal > 0 &&
      cartSyncState &&
      String(cartSyncState.version || "") === "10.1" &&
      String(cartSyncState.key || "") === expectedRewriteKey &&
      String(cartSyncState.checkoutToken || "") === expectedCheckoutToken &&
      Number(cartSyncState.total) === expectedTotal &&
      (!bridge || (
        String(bridge.cartRewriteKey || "") === expectedRewriteKey &&
        String(bridge.checkoutToken || "") === expectedCheckoutToken &&
        Number(bridge.total) === expectedTotal
      ))
    );

    if(matchingCartSyncState && cartSyncState.status === "success"){
    ...
    if(matchingCartSyncState && cartSyncState.status === "error"){
      failCheckoutLaunch(
        "自動加入正式購物車失敗：" +
...
function LUNY_V17_makePrewriteError(json, fallbackMessage){
  ...
  error.orderExists = json && json.orderExists === true;
  error.oneShopVerified = json && json.oneShopVerified === true;
  error.orderNo = String(json && json.orderNo || "");
  error.paymentStatus = String(json && json.paymentStatus || "");
  error.lookupCode = String(json && json.lookupCode || "");
  return error;
}
...
async function LUNY_V17_ensurePrewrite(pack, options){
  ...
  flight.promise = (async function(){
    try{
      ...
      if(LUNY_V17_isTokenBoundPrewriteError(error)){
        LUNY_V17_markTokenBound(pack.checkoutPayload.checkoutToken, error);
      }else if(!flight.superseded && (!error || error.code !== "LUNY_PREWRITE_ABORTED")){
...
async function LUNY_V17_runEntryPrewrite_(){
  ...
  try{
    ...
    if(LUNY_V17_isTokenBoundPrewriteError(error)){
      let token = String(error && error.checkoutToken || "");
      if(!token){
        try{ token = String(localStorage.getItem(CHECKOUT_TOKEN_KEY) || ""); }catch(e){}
      }
      const marker = LUNY_V17_markTokenBound(token, error);
      LUNY_V17_ENTRY_ATTEMPTS = 0;
      cancelScheduledEntryPrewrite();
      if(marker) LUNY_V17_renderTokenBoundState(marker);
      console.warn("[LUNY] checkout token already belongs to an order", error);
      return;
    }
...
async function retryPendingCheckoutStarted(){
  ...
  const pendingToken = String(pending.checkoutPayload.checkoutToken || "");
  const tokenBoundMarker = LUNY_V17_getTokenBoundMarker(pendingToken);
  if(tokenBoundMarker){
    safeStorageRemove(localStorage, CHECKOUT_STARTED_RETRY_KEY);
    LUNY_V17_renderTokenBoundState(tokenBoundMarker);
    return;
  ...
  try{
    ...
    if(LUNY_V17_isTokenBoundPrewriteError(error)){
      const marker = LUNY_V17_markTokenBound(pendingToken, error);
      safeStorageRemove(localStorage, CHECKOUT_STARTED_RETRY_KEY);
      if(marker) LUNY_V17_renderTokenBoundState(marker);
      console.warn("[LUNY] checkout_started retry stopped: token already bound", error);
    }else{
...
async function goToProductCheckout(){
  ...
  try{
    ...
    if(LUNY_V17_isTokenBoundPrewriteError(err)){
      let token = "";
      try{ token = localStorage.getItem(CHECKOUT_TOKEN_KEY) || ""; }catch(e){}

      // 解除一般 loading 樣式，但立即套用不可重送狀態；返回修改仍可使用。
      setCheckoutUILocked(false);
      const marker = LUNY_V17_markTokenBound(token, err);
      if(marker) LUNY_V17_renderTokenBoundState(marker);
      ...
      alert(
        oneShopOrderExists
          ? (
            "1SHOP 已確認存在正式訂單，系統已停止重複送出。\n" +
            "訂單編號：" + marker.orderNo + "\n" +
            "付款狀態：" + (marker.paymentStatus || "未提供")
          )
          : (
            "這個對帳編號已被後端關聯，但不代表 1SHOP 訂單已成立。\n" +
            "系統已停止重複送出，請先確認 1SHOP 訂單查詢結果。\n" +
            "對帳編號：" + token
          )
      );
      return;
