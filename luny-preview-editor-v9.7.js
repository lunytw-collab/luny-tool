/* LUNY 預覽主程式整合版 v7.9.26-eyedropper-fill-safe-to-bleed
 ...
 * v7.9.26：滴管取色套用範圍改為安全範圍外到灰色出血線；取樣範圍文字改為「取樣」，避免與套用範圍混淆。
 */
...
function applyMirrorBleedFromCutLine(ctx,canvas,cm2px){
  ...
  // 滴管顏色的「改色範圍」：從安全範圍線外側，一直到灰色出血虛線。
  // 也就是綠線外～灰線內全部套用，避免只補到裁切線附近。
  if(pickedFillColor){
    ctx.save();
...
/* LUNY v7.9.26：移除模式切換 UI；保留畫布上滴管取色工具；滴管顏色套用於綠線外～灰線。 */
(function(){
  function hideModeUI(){
