/* LUNY v16.7.7-backend-preview-match：前台、結帳縮圖與後台預覽共用同一套合成與導引線。 */
...
function applySelectedEdgeColor(ctx,canvas,cm2px){
  ...
  addMirrorBleedShapePath(ctx,shape.value,cx,cy,cutW,cutH,cm2px);
  ctx.fill('evenodd');
  ctx.restore();
}
...
function addLunyRiskShapePath(ctx,shapeValue,cx,cy,w,h,cm2px){if(shapeValue==='custom'&&lunyCustomAddPath(ctx,w,h,cm2px))return;const s=shapeValue;const rpx=0.1*cm2px;if(s==='circle'){const r=Math.min(w,h)/2;ctx.arc(cx,cy,r,0,Math.PI*2);}else if(s==='roundrect'){const x=cx-w/2,y=cy-h/2;roundedRectPath(ctx,x,y,w,h,rpx);ctx.closePath();}else if(s==='ellipse'){ctx.ellipse(cx,cy,w/2,h/2,0,0,Math.PI*2);}else if(s==='arch'){const x=cx-w/2,y=cy-h/2;archPath(ctx,x,y,w,h,cm2px);ctx.closePath();}}function drawProductPreviewCrop(ctx,canvas,cm2px){
  ...
  const caution=!!(
    ...
    preflight.flattenedRisk&&preflight.flattenedRisk.caution
  );
...
function lunyClassifyPreflight(){
  ...
  if(flattenedRisk.danger){
    return lunyMakePreflightResult(
      'BLOCKED_FILE_PREP_ELIGIBLE','red','原圖中的重要圖文太靠近裁切線',
      ['文字、Logo 或細節已和主圖壓成同一張圖片，系統無法只移動單一物件。'],
      ['可回原始設計檔將重要內容移到綠線內，再重新上傳。','也可加購印刷檔案基礎整理，由我們將整張圖等比例縮小、置中並補出血。','若需要拆字、改字、移動單一物件、重排或重繪，需另行報價。'],
      {
        resolution:resolutionSummary,
        flattenedRisk,
        filePrepEligible:true,
        filePrepScope:['SAFE_MARGIN_SCALE','CENTER_AND_SCALE','SOLID_COLOR_BLEED'],
        filePrepDescription:'含整張圖等比例縮小、置中、安全距離調整與單色出血；不含拆字、改字、移動單一物件、重排或重繪。整理完成後提供預覽確認，製作天數從確認後起算。'
      }
    );
  }
...
window.lunyConfirmBleedBeforeSave=function(){
  const result=lunyUpdatePreflight();
...
(function(){
  ...
  function renderMatchedPreviewCanvas(maxSide){
    ...
    // 邊緣色已由 drawAll/applySelectedEdgeColor 依正式印刷範圍畫入；此處只畫一次導引線。
    drawEdgeRing(ctx,width,height,cm2px,null,true,true);
    return out;
  }
  ...
  function decoratePreviewThumbCanvas(canvas){
    ...
    // 底圖已包含正式範圍的邊緣色，避免縮圖再用「出血線至安全線」覆蓋一次。
    drawEdgeRing(ctx,canvas.width,canvas.height,cm2px,null,true,true);
    return canvas;
  }
  ...
  function renderOverlay(){
    ...
    // 前台 overlay 只負責導引線；邊緣色由主畫布按正式輸出範圍繪製。
    drawEdgeRing(ctx,overlay.width,overlay.height,cm2px,null,true,true);
  }
