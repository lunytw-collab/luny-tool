/*
  LUNY Catalog Patch v8-5 UI Only
  GitHub filename:
  luny-catalog-patch-v8-5-ui-only.js

  Required load order in 1SHOP:
  1) 原本標籤貼紙模板
  2) luny-catalog-pricing-v6.js
  3) luny-catalog-patch-v8-5-ui-only.js
...
(function LunyCatalogFromLabelTemplateV8(){
  ...
  // ✅ 圖鑑貼紙價格表已拆到 GitHub：luny-catalog-pricing-v6.js
  // 本頁只透過 window.LUNY_CATALOG_PRICING.getPrice(...) 取得價格。



  const CATALOG_PRICING_SRC = "https://cdn.jsdelivr.net/gh/lunytw-collab/luny-tool/luny-catalog-pricing-v6.js?v=20260818-1";
  ...
  function forceCatalogQuantityOptions(){
    const qty = $("quantity");
    if(!qty) return;
    const values = ["20","30","40","50","60","70","80","90","100","150","200","300","400","500"];
  ...
  function validateCatalogQuote(){
    ...
    if(!window.LUNY_CATALOG_PRICING){ alert("圖鑑報價資料尚未載入，請確認 luny-catalog-pricing-v6.js 已正確引用。"); return false; }
    ...
    if(!window.LUNY_CATALOG_PRICING || typeof window.LUNY_CATALOG_PRICING.getPrice !== "function"){
      ...
      ensureCatalogPricingLoaded(function(ok){
        if(ok){
          ...
          if(hint2) hint2.textContent = "圖鑑報價資料載入失敗，請確認 luny-catalog-pricing-v6.js 已上傳 GitHub，且引用順序在 patch 前面。";
        }
      });
      return null;
    }
