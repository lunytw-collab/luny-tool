/* LUNY v16.7.5 preflight：局部疑似重要內容改為黃色自助確認，避免誤放綠燈與增加人工審稿。 */
...
var LUNY_PREFLIGHT_VERSION='16.7.5';
...
function lunyClassifyPreflight(){
  ...
  if(flattenedRisk.danger){
    return lunyMakePreflightResult(
      'BLOCKED_REUPLOAD_REQUIRED','red','原圖中的重要圖文太靠近裁切線',
      ['文字、Logo 或細節已和主圖壓成同一張圖片，系統無法只移動單一物件。'],
      ['回原始設計檔將重要內容移到綠線內，再重新上傳。','若需要重排、改字或重繪，請聯繫客服另行報價。'],
      {resolution:resolutionSummary,flattenedRisk}
    );
  }
  ...
  if(!lunyPhotoCoversBleed()){
    ...
    return lunyMakePreflightResult(
      'BLOCKED_AUTO_FIXABLE','red','圖片尚未鋪滿出血範圍',
      ['圖片沒有完整延伸到灰色出血線，裁切後可能露出白邊。'],
      ['按「置中並放大到出血線」後確認裁切結果。','或改為保留白邊，或使用單色補出血。'],
      {resolution:resolutionSummary,flattenedRisk,autoFix:'FIT_TO_BLEED'}
    );
  }
...
function lunyGetPreflightResult(){
  ...
  result.filePrep={
    ...
    scope:selected?['TRIM_WHITE_MARGIN','CENTER_AND_SCALE','SOLID_COLOR_BLEED']:[]
  };
