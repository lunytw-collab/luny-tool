...
or a validated same-tab checkout handoff. If all browser handoffs are missing, the
backend may recover one unique, recent, amount-matched unbound pending order.
Global latest-token guessing is forbidden.
v8 keeps the safe binding flow, adds server-side pending recovery, and restores
the full grouped order-detail card.
*/
(function installLunyPhase1CompletionPage(){
  "use strict";

  if (window.__LUNY_PHASE1_COMPLETION_PAGE__) return;
  window.__LUNY_PHASE1_COMPLETION_PAGE__ = "2026-07-24.8.0";
  ...
  async function attemptBind(trigger){
    ...
    if (!token){
      request.source = "phase1_server_pending_recovery_v8";
      request.clientValidationCode = "MISSING_CHECKOUT_TOKEN";
      request.fallbackMode = true;
      request.fallbackReason = "missing_checkout_token";
    }else if (!payload){
    ...
    saveStatus(token || "", "binding", {
      ...
      message: !token
        ? "完成頁未取得 token，正在由後端依訂單金額、時間與未綁定狀態安全找回。"
        : (!payload
