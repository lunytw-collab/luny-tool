/* LUNY v16.7.7-preflight-confirm-save-v12-accept-white-unchanged：「接受目前白邊」只記錄接受，不補色、不加框、不改動正式輸出。 */
/* 基於 v11-danger-zone-warning；保留局部重要細節壓到安全線外的黃色警示。 */
/* 基於 v10-ready-save-gate；保留可製作狀態與正式產檔使用同一個 canProceed 結果。 */
/* 基於 v9-material-clean-source；保留材質效果強制使用無刀線、無尺寸標示的統一成品圖。 */
...
var LUNY_PREFLIGHT_VERSION='16.7.7';
...
var __lunyWhiteMarginAccepted=false;
var __lunyWhiteMarginAcceptanceKey='';
...
function lunyFlattenedPhotoDangerAssessment(){
  ...
  const cutW=W-2*b,cutH=H-2*b;
  const safeW=Math.max(1,cutW-2*gap),safeH=Math.max(1,cutH-2*gap);
  const sectorCount=24,sectorHits=Array(sectorCount).fill(0),sectorSamples=Array(sectorCount).fill(0);
  ...
  for(let t=0;t<=1.0001;t+=0.125){
    ...
    for(const point of pts){
      ...
      if(photoPointHasCompositeContent(point.x,point.y)){
        hits+=1;
        sectorHits[sector]+=1;
      }
    }
  ...
  // 局部 Logo、文字或 QR Code 往往只占危險環的一小段；舊門檻 hits>=160
  // 會漏掉肉眼明顯壓線、但面積不大的重要細節。photoPointHasCompositeContent()
  // 已排除純色滿版背景，因此這裡以局部密度與連續三區密度判定黃色確認。
  const localizedCaution=!danger&&samples>=1000&&hits>=20&&ratio>=0.006&&maxSectorRatio>=0.12&&maxWindowRatio>=0.06;
  return{
    danger,caution:localizedCaution,
    hits,samples,ratio,maxSectorRatio,maxWindowRatio,hotSectorCount,
    detection:broadDanger?'broad':(moderateDenseDanger?'moderate':(localizedDanger?'localized-danger':(localizedCaution?'localized-caution':'none')))
  };
}
...
function lunyWhiteMarginAcceptedForCurrentState(){
  return __lunyWhiteMarginAccepted&&
    __lunyWhiteMarginAcceptanceKey===lunyPreflightStateKey();
}

function lunyAcceptCurrentWhiteMargin(){
  __lunyWhiteMarginAccepted=true;
  __lunyWhiteMarginAcceptanceKey=lunyPreflightStateKey();
...
function lunyResetWhiteMarginAcceptance(){
  __lunyWhiteMarginAccepted=false;
  __lunyWhiteMarginAcceptanceKey='';
}
...
function lunyClassifyPreflight(){
  ...
  if(resolutionLevel==='danger'){
    return lunyMakePreflightResult(
      'BLOCKED_REUPLOAD_REQUIRED','red','圖片解析度不足，不能直接送印',
  ...
  if(placed.any){
    ...
    return lunyMakePreflightResult(
      ...
      [`${names.join('、')}碰到紅線內側 2.5mm 的危險區域。`],
      ['按「移回安全區」讓系統將新增內容移到綠線內。','或自行縮小並拖曳到綠線內，再重新檢查。'],
  ...
  if(flattenedRisk.danger){
    const accepted=lunyWarningAcceptedFor('flattened-safety');
    return lunyMakePreflightResult(
      accepted?'WARNING_ACCEPTED':'WARNING_ACCEPTANCE_REQUIRED',
      'yellow',
      '請確認原圖中的重要內容與裁切線距離',
      ['系統偵測到安全區外有較多圖像細節，但單張合成圖片可能包含滿版底色、紋理或裝飾外框，無法可靠判斷是否為重要文字或 Logo。'],
      ['請確認文字、Logo、QR Code、電話等重要內容完整位於綠色安全線內。','背景、底色、紋理或裝飾外框可延伸至裁切線及出血區。','若重要內容確實越過綠線，請縮小圖片或回原始設計檔調整後重新上傳。'],
      {
        resolution:resolutionSummary,
        flattenedRisk,
        warningKind:'flattened-safety',
        warningAcceptanceLabel:'我已確認文字、Logo、QR Code 等重要內容位於綠色安全線內，仍使用此檔製作。',
        canProceed:accepted
      }
    );
  }

  const whiteMarginAccepted=lunyWhiteMarginAcceptedForCurrentState();
  if(!lunyPhotoCoversBleed()&&!whiteMarginAccepted){
    ...
    if(eligible){
      return lunyMakePreflightResult(
        ...
        ['先按「裁掉白邊並放大」查看結果。','也可改為保留白邊，或使用單色補出血。','不方便自行調整時，可加購印刷檔案基礎整理。'],
    ...
    return lunyMakePreflightResult(
      ...
      ['可按「置中並放大到出血線」後確認裁切結果。','也可改為保留白邊、使用單色補出血，或加購印刷檔案基礎整理。'],
      {
        resolution:resolutionSummary,
        flattenedRisk,
  ...
  if(flattenedRisk.caution){
    ...
    return lunyMakePreflightResult(
      ...
      'yellow',
      '局部文字或細節接近安全距離',
      ['系統偵測到原圖中有局部細節靠近綠色安全線，但無法判斷它是文字、Logo 或背景裝飾。'],
      ['確認文字、Logo、QR Code 等重要內容都完整位於綠色安全線內。','若重要內容越過綠線，請縮小圖片或回原始設計檔調整後重新上傳。'],
      {
        resolution:resolutionSummary,
        flattenedRisk,
        warningKind:'flattened-safety',
        warningAcceptanceLabel:'我已確認文字、Logo、QR Code 等重要內容都在綠色安全線內。',
        canProceed:accepted
      }
    );
  }
  ...
  if(resolutionLevel==='warning'||resolutionLevel==='unknown'){
    const accepted=lunyWarningAcceptedFor('resolution');
    return lunyMakePreflightResult(
      accepted?'WARNING_ACCEPTED':'WARNING_ACCEPTANCE_REQUIRED',
      'yellow',
      resolutionLevel==='warning'?'圖片可製作，但請確認清晰度':'圖片可製作，解析度結果尚未取得',
      [resolutionLevel==='warning'?'細節、小字或 QR Code 可能不如螢幕顯示清楚。':'系統目前無法取得完整解析度判定。'],
      ['放大預覽確認文字、Logo 與 QR Code 清楚可辨識。','有更清晰的原始檔時，建議改用原始檔。'],
      {
        resolution:resolutionSummary,
        flattenedRisk,
        warningKind:'resolution',
        warningAcceptanceLabel:'我已確認預覽中的文字、Logo 與 QR Code 清楚，仍使用此檔製作。',
        canProceed:accepted
      }
    );
  }

  if(whiteMarginAccepted){
    return lunyMakePreflightResult(
      'READY','green','已接受目前白邊，可直接製作',[],
      ['不會新增白邊、補色或改動上傳圖片。','成品邊緣將依目前預覽保留白邊。'],
      {resolution:resolutionSummary,flattenedRisk,canProceed:true,whiteMarginAccepted:true}
    );
  }
  ...
  return lunyMakePreflightResult(
    ...
    {resolution:resolutionSummary,flattenedRisk,canProceed:true}
  );
}
...
function lunyRenderPreflight(result){
  ...
  const colors=result.level==='green'
    ?{border:'#86c89a',background:'#f1faf4',text:'#175b31',chip:'#d9f2e1'}
    :(result.level==='yellow'
      ?{border:'#e7c36d',background:'#fff9e8',text:'#75520a',chip:'#f9e7ad'}
      :{border:'#e59aa3',background:'#fff4f5',text:'#8f1d2c',chip:'#ffdce0'});
  ...
  if(isBleedFix&&(result.status==='BLOCKED_FILE_PREP_ELIGIBLE'||result.status==='BLOCKED_AUTO_FIXABLE')){
    actions.push(lunyPreflightButton('接受目前白邊','white',false));
    actions.push(lunyPreflightButton('套用建議邊緣色','color',false));
  }
  if(result.edgeChoice)actions.push(lunyPreflightButton('重新選擇補邊方式','edge-reset',false));
  if(result.level==='red')actions.push(lunyPreflightButton('重新上傳圖片','upload',false));
  let decision='';
  if(result.status==='WARNING_ACCEPTANCE_REQUIRED'||result.status==='WARNING_ACCEPTED'){
    const accepted=result.status==='WARNING_ACCEPTED';
    const acceptanceLabel=result.warningAcceptanceLabel||'我已確認目前預覽內容，仍使用此檔製作。';
    decision=`<label style="display:flex;gap:8px;align-items:flex-start;margin-top:12px;font-weight:700;"><input id="lunyPreflightAcceptWarning" type="checkbox" ${accepted?'checked':''} style="margin-top:4px;"><span>${acceptanceLabel}</span></label>`;
  }
  ...
  const versionHeading=panel.querySelector('div[style*="font-size:12px"]');
  if(versionHeading){
    const versionTag=document.createElement('span');
    versionTag.textContent=` v${result.version}`;
    versionTag.style.cssText='font-weight:700;opacity:.72;';
    versionHeading.appendChild(versionTag);
  }
  panel.querySelectorAll('[data-luny-preflight-action]').forEach(button=>{
    button.addEventListener('click',function(){
      ...
      if(action==='white')lunyAcceptCurrentWhiteMargin();
      if(action==='color')lunyUseSuggestedEdgeColor();
      if(action==='edge-reset')lunyResetEdgeChoice();
      if(action==='upload')imgInput.click();
    });
  });
  const accept=panel.querySelector('#lunyPreflightAcceptWarning');
  if(accept)accept.addEventListener('change',function(){
    __lunyPreflightWarningAccepted=!!accept.checked;
    __lunyPreflightWarningAcceptanceKey=accept.checked
      ?lunyWarningAcceptanceKey(result.warningKind,result.stateKey)
      :'';
    // 勾選即同步授權同一輪正式產檔，避免儲存流程在重新檢查時遺失人工確認。
    if(accept.checked){
      approveBleedRiskTemporarily();
    }else{
      window.lunyResetBleedRiskDecision();
...
window.LUNY_acceptCurrentWhiteMargin=lunyAcceptCurrentWhiteMargin;
window.LUNY_resetWhiteMarginAcceptance=function(){lunyResetWhiteMarginAcceptance();lunyUpdatePreflight();};
window.LUNY_PREFLIGHT_VERSION=LUNY_PREFLIGHT_VERSION;
window.lunyConfirmBleedBeforeSave=function(){
  // 儲存按鈕可能與解析度檢查同時執行；先以目前設計狀態重新分類，
  // 再更新面板，避免僅因非設計變更的重繪而取消已勾選的人工確認。
...
document.addEventListener('luny:resolutionChecked',function(){
  ...
  // flattened-safety 是客戶對同一份設計與位置的人工確認，不能被非同步解析度事件清掉。
...
window.getPrintAndCutBlobs=async function(){
  // 「加入結帳清單」前已由 lunyConfirmBleedBeforeSave() 完成正式檔案檢查。
  // 頁面層的邊緣處理（接受目前白邊／套用建議邊緣色／調整到出血）
  // 可能把原始結果正規化為 canProceed=true；產檔必須承接同一結果，
  // 不可再以舊版 getBleedRiskStatus() 對同一設計做第二套判定。
  let currentPreflight=null;
  try{
    currentPreflight=typeof window.LUNY_getPreflightResult==='function'
      ?window.LUNY_getPreflightResult()
      :null;
  }catch(e){
    currentPreflight=null;
  }
  const preflightAllowsProduction=!!(currentPreflight&&currentPreflight.canProceed===true);
  if(!preflightAllowsProduction&&!validateDesignBeforeExport('printcut',{manual:false,silent:true})){
