...
(function () {
  // v39-materials：新增低殘膠銅板、低殘膠珠光、亮銀龍與消光銀龍。
  // 低殘膠材質完全沿用對應材質報價；兩款銀龍固定亮膜並共用市場基準價。
  // 材質倍率統一回市場基準邏輯：
  // 銅板上膜基準價先對齊市場基準，再由這裡產生各材質價格。
  // 冷凍珠光：依市場基準約 1.77 倍。
  // 一般防水珠光：依你提供的 3x3 市場基準價格，500 張以上約為 1.20 倍。
  var MATERIAL_MULTIPLIER = {
    ...
    silver_laminated: 1.51,

    kraft: 1.15,
    shtte: 0.9
  };

  // 不再針對 35 / 56 / 88 模壓低冷凍珠光倍率，避免小尺寸珠光比市場基準便宜太多。
  ...
  var MARKET_REFERENCE_MULTIPLIER = {
    artpaper_laminated: 1,
    artpaper_none: 0.9,
  ...
  // 1000張起含1000張，價格對齊市場基準，個位數無條件捨去。
  var MARKET_REFERENCE_MATCH_FROM_1000 = {
    56: {
      1000: 773,
      2000: 904,
      3000: 1036,
  ...
  // v37.2：一般件新增 15,000～50,000 張級距。
  // 有市場基準實測資料的代表模數直接使用實測總價；
  // 其他中間模數依原表 9,000→10,000 的增量延伸。
  var LARGE_QUANTITY_LEVELS = [15000, 20000, 30000, 40000, 50000];
  var MARKET_REFERENCE_LARGE_QTY_EXACT = {
    "2": {15000:25181,20000:33359,30000:49718,40000:66077,50000:82436},
    "M2_SQUARE_18X18": {15000:59341,20000:78908,30000:118040,40000:157173,50000:196305},
    "M2_LARGE_20X15": {15000:33199,20000:44051,30000:65756,40000:87461,50000:109166},
    "M4_LARGE_15X15": {15000:25181,20000:33359,30000:49718,40000:66077,50000:82436},
    "6": {15000:20369,20000:26944,30000:40095,40000:53247,50000:66398},
    "8": {15000:11655,20000:15325,30000:22667,40000:30008,50000:37350},
    "12": {15000:9517,20000:12473,30000:18390,40000:24306,50000:30222},
    "24": {15000:5694,20000:7378,30000:10746,40000:14113,50000:17482},
    "35": {15000:4042,20000:5175,30000:7442,40000:9709,50000:11975},
    "56": {15000:2612,20000:3269,30000:4582,40000:5896,50000:7209},
    "88": {15000:1819,20000:2212,30000:2997,40000:3782,50000:4567}
  };

  function floorPriceToTen(value) {
    return Math.floor((Number(value) || 0) / 10) * 10;
  }

  function extendPriceMapTo50k(price, module) {
    var output = {};
    Object.keys(price || {}).forEach(function (qty) {
      output[qty] = price[qty];
    });

    var exact = MARKET_REFERENCE_LARGE_QTY_EXACT[String(module)] || null;
    var price9000 = Number(output[9000] || 0);
    var price10000 = Number(output[10000] || 0);
    var perThousand = Math.max(0, price10000 - price9000);

    LARGE_QUANTITY_LEVELS.forEach(function (qty) {
      var raw = exact && exact[qty]
        ? exact[qty]
        : price10000 + perThousand * ((qty - 10000) / 1000);
      output[qty] = floorPriceToTen(raw);
    });

    return output;
  }

  function extendPricingTableTo50k(table) {
    return (table || []).map(function (item) {
      return {
        module: item.module,
        price: extendPriceMapTo50k(item.price, item.module)
      };
    });
  }
  ...
  window.LUNY_BASE_PRICE_V34 = extendPricingTableTo50k(window.LUNY_BASE_PRICE_V34);

  // 先覆蓋基準價，再產生各材質價格。
  // 這樣銅板、冷凍珠光、一般防水珠光、透明、牛皮、模造都會吃到同一份新基準價。
  window.LUNY_BASE_PRICE_V34 = applyMarketReferenceMatchToBasePrice(
    window.LUNY_BASE_PRICE_V34,
    MARKET_REFERENCE_MATCH_FROM_1000
  );

  var MARKET_REFERENCE_ARTPAPER_LAMINATED_TABLE = [
    ...
    { module: 6, price: { 100:774,300:1037,500:1300,1000:1957,2000:3272,3000:4587,4000:5902,5000:7218,6000:8533,7000:9848,8000:11163,9000:12478,10000:13793 } },
    { module: 7, price: { 100:763,300:1005,500:1127,1000:1613,2000:2584,3000:3556,4000:4526,5000:5497,6000:6469,7000:7440,8000:8410,9000:9382,10000:10354 } },
    { module: 8, price: { 100:714,300:862,500:1008,1000:1375,2000:2109,3000:2845,4000:3578,5000:4311,6000:5047,7000:5781,8000:6514,9000:7249,10000:7983 } },
    { module: 9, price: { 100:711,300:852,500:990,1000:1340,2000:2038,3000:2738,4000:3436,5000:4133,6000:4833,7000:5532,8000:6229,9000:6928,10000:7626 } },
    { module: 10, price: { 100:708,300:841,500:972,1000:1304,2000:1966,3000:2631,4000:3293,5000:3955,6000:4619,7000:5282,8000:5944,9000:6608,10000:7270 } },
    { module: 12, price: { 100:700,300:820,500:937,1000:1234,2000:1824,3000:2417,4000:3008,5000:3599,6000:4191,7000:4783,8000:5374,9000:5967,10000:6558 } },
    { module: 15, price: { 100:693,300:797,500:900,1000:1158,2000:1674,3000:2190,4000:2706,5000:3222,6000:3738,7000:4255,8000:4770,9000:5288,10000:5803 } },
    { module: 16, price: { 100:691,300:790,500:888,1000:1133,2000:1624,3000:2115,4000:2606,5000:3096,6000:3587,7000:4078,8000:4569,9000:5061,10000:5552 } },
    { module: 20, price: { 100:681,300:759,500:838,1000:1032,2000:1423,3000:1813,4000:2203,5000:2593,6000:2983,7000:3374,8000:3764,9000:4155,10000:4545 } },
    { module: 22, price: { 100:679,300:751,500:824,1000:1005,2000:1370,3000:1733,4000:2096,5000:2460,6000:2822,7000:3186,8000:3550,9000:3914,10000:4278 } },
    { module: 24, price: { 100:676,300:743,500:810,1000:978,2000:1316,3000:1653,4000:1989,5000:2326,6000:2662,7000:2999,8000:3337,9000:3673,10000:4010 } },
    { module: 35, price: { 100:665,300:710,500:755,1000:868,2000:1095,3000:1322,4000:1548,5000:1776,6000:2003,7000:2229,8000:2456,9000:2682,10000:2909 } },
    { module: 56, price: { 100:655,300:680,500:708,1000:773,2000:904,3000:1036,4000:1168,5000:1298,6000:1430,7000:1561,8000:1692,9000:1823,10000:1955 } },
    { module: 88, price: { 100:649,300:665,500:680,1000:720,2000:799,3000:877,4000:955,5000:1033,6000:1112,7000:1190,8000:1269,9000:1347,10000:1426 } }
  ];

  MARKET_REFERENCE_ARTPAPER_LAMINATED_TABLE = extendPricingTableTo50k(MARKET_REFERENCE_ARTPAPER_LAMINATED_TABLE);
  ...
  function applyFloorByMarketReferenceArtpaper(lunyTable, referenceMultiplier) {
    return lunyTable.map(function (item) {
      var referenceItem = findModule(MARKET_REFERENCE_ARTPAPER_LAMINATED_TABLE, item.module);
      ...
      Object.keys(item.price).forEach(function (qty) {
        ...
        if (!referenceItem.price[qty]) return;

        var stickerPrice = Math.round(referenceItem.price[qty] * referenceMultiplier);
  ...
  function applyMarketReferenceMatchToBasePrice(baseTable, matchMap) {
  ...
  function buildPricingTableFromMarketReference(multiplier) {
    return MARKET_REFERENCE_ARTPAPER_LAMINATED_TABLE.map(function (item) {
      var newPrice = {};
      Object.keys(item.price).forEach(function (qty) {
        newPrice[qty] = Math.round(item.price[qty] * multiplier);
      });
  ...
  // v38.1：100 / 300 張依 2026-09-06 重新提供的市場基準校正。
  // - 88～12 模：若原價低於市場價，售價為市場價減 100 元；其餘回到市場價。
  // - 11～3 模：一律回到市場價，並將個位數無條件捨去。
  function getSmallQuantityRuleModule(module) {
    if (!isNaN(Number(module))) return Number(module);
    ...
    return aliasMap[String(module)] || 0;
  }

  // 上一版（v38）實際顯示價格的市場參考值。
  // 新規則中的「目前價格」須以 v38 顯示結果比較，不能回頭使用更早期的原始價表。
  var PREVIOUS_V38_SMALL_QTY_REFERENCE = {
    3: { 100: 795, 300: 1100 },
    4: { 100: 784, 300: 1069 },
    6: { 100: 774, 300: 1037 },
    7: { 100: 739, 300: 934 },
    ...
    56: { 100: 450, 300: 580 },
    88: { 100: 400, 300: 500 },
    M3_MEDIUM_14X12: { 100: 795, 300: 1100 },
    M4_LARGE_15X15: { 100: 806, 300: 1132 },
    M2_LARGE_20X15: { 100: 860, 300: 1293 },
    M2_SQUARE_18X18: { 100: 1033, 300: 1815 }
  };

  function applyPreviousV38SmallQuantityRules(pricingMap) {
    var materialReferenceMultiplier = {
      artpaper: { "上膜": 1, "無": 0.9 },
      pearlescent: { "亮膜": 1.77, "霧膜": 1.77, "無": 1.77 * 0.9 },
      normalPearlescent: { "亮膜": 1.32, "霧膜": 1.32, "無": 1.32 * 0.9 },
      waterproofPearlescent: { "亮膜": 1.32, "霧膜": 1.32, "無": 1.32 * 0.9 },
      "一般防水珠光": { "亮膜": 1.32, "霧膜": 1.32, "無": 1.32 * 0.9 },
      transparent: { "亮膜": 1.69, "霧膜": 1.69 },
      glossSilver: { "亮膜": 1.51 },
      matteSilver: { "亮膜": 1.51 },
      kraft: { "無": 1.15 },
      shtte: { "無": 0.9 }
    ...
    Object.keys(materialReferenceMultiplier).forEach(function (materialKey) {
      if (!pricingMap[materialKey]) return;

      Object.keys(materialReferenceMultiplier[materialKey]).forEach(function (finishKey) {
        var table = pricingMap[materialKey][finishKey];
        if (!Array.isArray(table)) return;

        var multiplier = materialReferenceMultiplier[materialKey][finishKey];
        table.forEach(function (item) {
          var ruleModule = getSmallQuantityRuleModule(item.module);
          var reference = PREVIOUS_V38_SMALL_QTY_REFERENCE[item.module];
          if (ruleModule < 3 || ruleModule > 88 || !reference) return;

          ["100", "300"].forEach(function (qty) {
            if (typeof item.price[qty] === "undefined" || typeof reference[qty] === "undefined") return;
            var currentPrice = Number(item.price[qty]) || 0;
            var previousMarketPrice = roundNicePrice(Math.round(Number(reference[qty]) * multiplier));

            if (ruleModule <= 12) {
              item.price[qty] = floorToTen(previousMarketPrice);
              return;
            }

            item.price[qty] = currentPrice < previousMarketPrice
              ? Math.max(0, previousMarketPrice - 100)
              : previousMarketPrice;
          });
        });
      });
  ...
  function applySmallQuantityMarketRules(pricingMap) {
    var materialReferenceMultiplier = {
      artpaper: { "上膜": 1, "無": 0.9 },
      pearlescent: { "亮膜": 1.77, "霧膜": 1.77, "無": 1.77 * 0.9 },
      normalPearlescent: { "亮膜": 1.32, "霧膜": 1.32, "無": 1.32 * 0.9 },
    ...
    Object.keys(materialReferenceMultiplier).forEach(function (materialKey) {
      ...
      Object.keys(materialReferenceMultiplier[materialKey]).forEach(function (finishKey) {
        ...
        var referenceMultiplier = materialReferenceMultiplier[materialKey][finishKey];

        table.forEach(function (item) {
          var ruleModule = getSmallQuantityRuleModule(item.module);
          if (ruleModule < 3 || ruleModule > 88) return;
          ...
          ["100", "300"].forEach(function (qty) {
            ...
            if (typeof referenceItem.price[qty] === "undefined") return;

            var currentPrice = Number(item.price[qty]) || 0;
            var marketPrice = Math.round(
              Number(referenceItem.price[qty]) * referenceMultiplier
            );

            if (ruleModule <= 11) {
              item.price[qty] = floorToTen(marketPrice);
              return;
            }

            item.price[qty] = currentPrice < marketPrice
              ? Math.max(0, marketPrice - 100)
              : marketPrice;
          });
        });
      });
  ...
  var artpaperLaminated = applyFloorByMarketReferenceArtpaper(
    buildPricingTable(MATERIAL_MULTIPLIER.artpaper_laminated),
    MARKET_REFERENCE_MULTIPLIER.artpaper_laminated
  );

  var artpaperNone = applyFloorByMarketReferenceArtpaper(
    buildPricingTable(MATERIAL_MULTIPLIER.artpaper_none),
    MARKET_REFERENCE_MULTIPLIER.artpaper_none
  );

  var pearlescentLaminated = applyFloorByMarketReferenceArtpaper(
    buildPricingTableByModule(
      MATERIAL_MULTIPLIER.pearlescent_laminated,
      PEARLESCENT_MODULE_MULTIPLIER,
      1
    ),
    MARKET_REFERENCE_MULTIPLIER.pearlescent_laminated
  );
  ...
  var transparentLaminated = applyFloorByMarketReferenceArtpaper(
    buildPricingTable(MATERIAL_MULTIPLIER.transparent_laminated),
    MARKET_REFERENCE_MULTIPLIER.transparent_laminated
  );

  // 貼紙先生同規格薄型亮銀龍相對銅板上膜約為 1.51 倍。
  // 健豪未標示番數，因此番數只作內部市場比價依據，不寫入前台或訂單規格。
  var silverLaminated = buildPricingTableFromMarketReference(
    MATERIAL_MULTIPLIER.silver_laminated
  );

  var kraftNone = applyFloorByMarketReferenceArtpaper(
    buildPricingTable(MATERIAL_MULTIPLIER.kraft),
    MARKET_REFERENCE_MULTIPLIER.kraft
  );

  var shtteNone = applyFloorByMarketReferenceArtpaper(
    buildPricingTable(MATERIAL_MULTIPLIER.shtte),
    MARKET_REFERENCE_MULTIPLIER.shtte
  );
  ...
  function cloneMaterialPricing(materialPricing) {
    var output = {};
    Object.keys(materialPricing || {}).forEach(function (finishKey) {
      output[finishKey] = (materialPricing[finishKey] || []).map(function (item) {
        var price = {};
        Object.keys(item.price || {}).forEach(function (qty) {
          price[qty] = item.price[qty];
        });
        return { module: item.module, price: price };
      });
    });
    return output;
  }

  pricingMap = applyLegacyUnder500PriceOverrides(pricingMap);
  pricingMap = applyPreviousV38SmallQuantityRules(pricingMap);
  pricingMap = applySmallQuantityMarketRules(pricingMap);

  // 完全比照對應材質，避免後續校價造成低殘膠版本與原材質不同步。
  pricingMap.lowResidueArtpaper = cloneMaterialPricing(pricingMap.artpaper);
  pricingMap.lowResiduePearlescent = cloneMaterialPricing(pricingMap.normalPearlescent);

  window.LUNY_PRICING_TABLE = pricingMap;
})();
