/* LUNY 主程式 v7.9.12：與頁面儲存前置檢查配合；確認才儲存，取消不儲存，取消後可再次判定 */
...
function isLunySaveLikeButton(el){
  if(!el) return false;
  const target=el.closest&&el.closest('button,a,input[type="button"],input[type="submit"],[role="button"]');
  if(!target) return false;
  if(target.id==='downloadPreview'||target.id==='downloadOriginal') return false;
  const hay=[
    target.id||'',
    target.name||'',
    target.className||'',
  ...
  return /saveDesign|save-design|saveOrder|submitOrder|uploadDesign|checkout|cart|儲存設計|儲存圖片|送出設計|上傳設計|加入購物車|立即結帳|送出訂單/.test(hay);
}
function isLunySaveLikeButton(el){
  if(!el) return false;
  const target=el.closest&&el.closest('button,a,input[type="button"],input[type="submit"],[role="button"]');
...
function runLunySaveGate(e,phase){
  const target=e.target&&e.target.closest&&e.target.closest('button,a,input[type="button"],input[type="submit"],[role="button"]');
  if(!isLunySaveLikeButton(target)) return true;

  if(isBleedRiskRecentlyCancelled()){
    return stopLunySaveEvent(e);
  }
  ...
  const ok=validateDesignBeforeExport('save',{manual:true});
  if(!ok){
    return stopLunySaveEvent(e);
  }
...
function bindBleedRiskBeforeSaveButtons(){
  if(document.__lunyBleedRiskSaveGateBound) return;
  document.__lunyBleedRiskSaveGateBound=true;

  // 只在最早的 pointerdown 做 confirm。
  // 後續 mousedown/touchstart/click 只負責依「已確認 / 已取消」狀態放行或阻擋，不再重複開視窗。
  ['pointerdown','mousedown','touchstart','click'].forEach(function(type){
    document.addEventListener(type,function(e){
      ...
      // 沒有 pointer 事件的舊裝置，才在 mousedown/touchstart/click 補一次。
      if(!window.PointerEvent && (type==='mousedown'||type==='touchstart'||type==='click')){
        return runLunySaveGate(e,type);
      }

      // 有 PointerEvent 的環境，非 pointerdown 不再跳窗，避免第二次 confirm。
      return true;
    },true);
  });
  ...
  document.addEventListener('submit',function(e){
    ...
    if(!/saveDesign|save-design|saveOrder|submitOrder|uploadDesign|checkout|cart|儲存設計|儲存圖片|送出設計|上傳設計|加入購物車|立即結帳|送出訂單/.test(hay)) return true;

    if(isBleedRiskRecentlyCancelled()){
      return stopLunySaveEvent(e);
    }

    if(!validateDesignBeforeExport('submit',{manual:true})){
      return stopLunySaveEvent(e);
    }
    approveBleedRiskTemporarily();
    return true;
