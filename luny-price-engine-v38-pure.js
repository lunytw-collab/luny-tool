(function () {
  ...
  /* LUNY price engine v38
     更新：新增純試算函式 getPriceForSpec / getAllQuantityPricesForSpec，不操作 DOM，供數量卡片直接顯示全價格、降低切換材質/上膜/急件卡頓。
     原 v37：修正方形 value 為 square 時，被誤判成特殊形狀的問題。
  ...
  */
  ...
  // 急件產能限制：最多 120 張大紙，但切割時間上限控制在 5000 張貼紙。
  ...
  function getShapeForPricing(shapeValue) {
    if (shapeValue === "roundrect") return "rounded";
    if (shapeValue === "ellipse") return "circle";
    if (shapeValue === "custom") return "custom";
    return shapeValue;
  }

  function getMaterialTableForPricing(material, laminate) {
    let materialTable = pricingTable?.[material];

    if (!materialTable) return null;
    ...
    return Array.isArray(materialTable) ? materialTable : null;
  }

  function getAvailableQuantityLevelsForSpec(spec) {
    const shapeValue = spec?.shape || spec?.shapeValue || "circle";
    const widthCm = Number(spec?.widthCm) || 0;
    const heightCm = Number(spec?.heightCm) || 0;
    const urgent = spec?.urgent || "normal";
    ...
    return QUANTITY_LEVELS.filter(function(q){
      if (specLimitResult.limited && q > specLimitResult.maxQty) return false;
      if (urgent === "rush" && isRushUnavailableByModule(mappedModule, q)) return false;
      if (urgent === "superrush" && isSuperRushUnavailableByModule(mappedModule, q)) return false;
      return true;
    });
  }

  function getPriceForSpec(spec) {
    const material = spec?.material || "";
    const laminate = spec?.laminate || "";
    const quantity = parseInt(spec?.quantity || "0", 10) || 0;
    const urgent = spec?.urgent || "normal";
    const shapeValue = spec?.shape || spec?.shapeValue || "circle";
    const shapeForPricing = getShapeForPricing(shapeValue);
    ...
    if (!Number.isFinite(widthCm) || !Number.isFinite(heightCm)) {
      return Object.assign(resultBase, { reason: "尺寸未填寫完整" });
    }
    ...
    if (widthCm < 1 || heightCm < 1 || longSide > 37 || shortSide > 26) {
      return Object.assign(resultBase, { reason: "尺寸超出限制" });
    }

    if (!QUANTITY_LEVELS.includes(quantity)) {
      return Object.assign(resultBase, { reason: "數量不在可選級距內" });
    }

    const estimatedModule = estimateModule(longSide, shortSide);
    const mappedModule = mapModuleByRules(estimatedModule);
    const pricingModule = getCorrectedPricingModule(widthCm, heightCm, mappedModule);
    const specQuantityLimit = getSpecQuantityLimitResult(shapeValue, widthCm, heightCm, mappedModule);

    if (specQuantityLimit.limited && quantity > specQuantityLimit.maxQty) {
      return Object.assign(resultBase, {
        reason: specQuantityLimit.message || "此規格超出可製作數量",
        estimatedModule,
        mappedModule,
        pricingModule,
        specQuantityLimit
    ...
    if (urgent === "rush" && isRushUnavailableByModule(mappedModule, quantity)) {
      return Object.assign(resultBase, {
        reason: getRushUnavailableMessage(mappedModule),
        estimatedModule,
        mappedModule,
        pricingModule,
        specQuantityLimit,
    ...
    if (urgent === "superrush" && isSuperRushUnavailableByModule(mappedModule, quantity)) {
      return Object.assign(resultBase, {
        reason: getSuperRushUnavailableMessage(mappedModule),
        estimatedModule,
        mappedModule,
        pricingModule,
        specQuantityLimit,
    ...
    const materialTable = getMaterialTableForPricing(material, laminate);
    ...
    const baseTotal = Number(closest.price[quantity] || 0);
    ...
    const total = applyExtraFees(baseTotal, shapeForPricing, urgent, shapeValue, widthCm, heightCm, quantity);
    const unitPrice = quantity > 0 ? Math.round((total / quantity) * 10) / 10 : 0;
    const summaryText = [
    ...
    return Object.assign(resultBase, {
      ...
      customShapeUsesProductionPartner: shouldUseProductionPartner(quantity, urgent),
      customShapeExtraFee: getCustomShapeExtraFee(shapeForPricing, shapeValue, widthCm, heightCm, quantity, urgent),
  ...
  function getCurrentSpecFromDom(overrides) {
    const spec = {
      material: getEl("material")?.value || "",
      laminate: getEl("laminate")?.value || "",
      quantity: parseInt(getEl("quantity")?.value || "0", 10),
      urgent: getEl("urgent")?.value || "normal",
      shape: getEl("shape")?.value || "circle",
      widthCm: parseFloat(getEl("widthCm")?.value),
  ...
  function getPriceForCurrentSpec(overrides) {
    return getPriceForSpec(getCurrentSpecFromDom(overrides));
  }

  function getAllQuantityPricesForSpec(spec) {
    const baseSpec = Object.assign({}, spec || {});
    const levels = Array.isArray(baseSpec.quantities) && baseSpec.quantities.length
      ? baseSpec.quantities.map(q => parseInt(q, 10)).filter(q => QUANTITY_LEVELS.includes(q))
      : QUANTITY_LEVELS.slice();

    const results = levels.map(function(q){
      return getPriceForSpec(Object.assign({}, baseSpec, { quantity: q }));
    });

    const priceMap = {};
    results.forEach(function(item){
      priceMap[item.quantity] = item;
    });
    ...
    return {
      ...
      results,
      priceMap,
      availableLevels: results.filter(item => item.available).map(item => item.quantity)
    };
  }

  function getAllQuantityPricesForCurrentSpec(overrides) {
    return getAllQuantityPricesForSpec(getCurrentSpecFromDom(overrides));
  }
  ...
  window.LUNY_PRICE_ENGINE = {
    ...
    calculatePrice,
    getPriceForSpec,
    getPriceForCurrentSpec,
    getAllQuantityPricesForSpec,
    getAllQuantityPricesForCurrentSpec,
    getAvailableQuantityLevelsForSpec,
    getCurrentSpecFromDom,
    estimateModule,
    findClosestModule,
  ...
  window.LUNY_getPriceForSpec = getPriceForSpec;
  window.LUNY_getPriceForCurrentSpec = getPriceForCurrentSpec;
  window.LUNY_getAllQuantityPricesForSpec = getAllQuantityPricesForSpec;
  window.LUNY_getAllQuantityPricesForCurrentSpec = getAllQuantityPricesForCurrentSpec;
