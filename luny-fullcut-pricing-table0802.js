...
(function () {
  ...
  function discountRows(rows, discount) {
    return rows.map(row => ({
      module: row.module,
      price: Object.fromEntries(
        Object.entries(row.price).map(([quantity, price]) => [
          quantity,
          Math.max(0, Number(price) - discount)
        ])
      )
    }));
  }
  ...
  window.LUNY_PRICING_TABLE = {
    ...
    coated_paper: {
      "亮膜": discountRows(FULLCUT_PRICE_ROWS, 30),
      "霧膜": discountRows(FULLCUT_PRICE_ROWS, 30)
    },
    pvc: {
      "亮膜": cloneRows(FULLCUT_PRICE_ROWS),
      "霧膜": cloneRows(FULLCUT_PRICE_ROWS)
