...
const LUNY_DIRECT_UPLOAD_ONLY=window.LUNY_DIRECT_UPLOAD_ONLY===true;
...
function validateDesignBeforeExport(action,opts){
  ...
  // 水晶轉印貼依客戶 PNG 原檔製作，不套用標籤貼紙的出血、危險區與白邊判斷。
  if(LUNY_DIRECT_UPLOAD_ONLY){
...
function lunyFitDirectMainImage(){
  if(!LUNY_DIRECT_UPLOAD_ONLY||!img)return;
  const b=BLEED_CM*CM2PX;
  const availableW=Math.max(1,cG.width-2*b);
  const availableH=Math.max(1,cG.height-2*b);
  scale=Math.min(availableW/Math.max(1,img.width),availableH/Math.max(1,img.height));
  offsetX=0;offsetY=0;angle=0;activeTarget=null;handles=null;
}
function drawPreview(){
  ...
  resizePreviewCanvas();
  lunyFitDirectMainImage();
  ctxG.imageSmoothingEnabled=true;
  ctxG.imageSmoothingQuality=__lunyInteractiveDraw?'medium':'high';
  const showGuides=!LUNY_DIRECT_UPLOAD_ONLY&&shouldShowSafetyGuides();
  handles=drawAll(ctxG,cG,CM2PX,{includeGuides:showGuides,includeSelection:!LUNY_DIRECT_UPLOAD_ONLY&&shape.value!=='custom',showQRTestMark:showQRTest,showMinText:showTestText,isPreview:true,useFullImage:false});
  if(!LUNY_DIRECT_UPLOAD_ONLY&&!showGuides){drawProductWhiteEdgePreview(ctxG,cG,CM2PX);drawProductPreviewCrop(ctxG,cG,CM2PX);}
  if(!LUNY_DIRECT_UPLOAD_ONLY){drawImportantSafetyRiskMask(ctxG,cG,CM2PX);updateBleedRiskUI();}
  lunyCustomUpdateControlUI();
  if(!LUNY_DIRECT_UPLOAD_ONLY)lunyUpdateWhiteEdgeBadge();
...
cG.addEventListener('pointerdown',(e)=>{
  if(LUNY_DIRECT_UPLOAD_ONLY||shape.value==='custom'){activeTarget=null;handles=null;mode=null;return;}
...
cG.addEventListener('pointermove',(e)=>{
  if(LUNY_DIRECT_UPLOAD_ONLY||shape.value==='custom')return;
  ...
  function installPreviewThumbPatch(){
    ...
    const patched = function(maxSize, quality){
      ...
      if(LUNY_DIRECT_UPLOAD_ONLY){
        ...
        const ratio = Math.min(1,safeMaxSize/sw,safeMaxSize/sh);
        const out = document.createElement('canvas');
...
(function(){
  ...
  function applyCanvasPresentation(){
    ...
    canvas.style.setProperty('background-image','linear-gradient(45deg,#e7eaee 25%,transparent 25%),linear-gradient(-45deg,#e7eaee 25%,transparent 25%),linear-gradient(45deg,transparent 75%,#e7eaee 75%),linear-gradient(-45deg,transparent 75%,#e7eaee 75%)','important');
    canvas.style.setProperty('background-size','20px 20px','important');
    canvas.style.setProperty('background-position','0 0,0 10px,10px -10px,-10px 0','important');
    canvas.style.border='1px solid #d7dce2';
    canvas.style.borderRadius='14px';
    canvas.style.boxSizing='border-box';
    canvas.style.imageRendering='auto';
    ensurePreviewQualityNotice(canvas);
    var notice=document.getElementById('lunyPreviewQualityNotice');
    if(notice) notice.innerHTML='<strong>棋盤格代表透明區域，不會印刷</strong><br>PNG 內有底色的區域會照原圖顯示；請確認不要把白色底誤認為透明。';
  }
...
/* LUNY 水晶轉印貼：僅接受真正的 PNG，並保留 alpha 透明資訊。 */
(function enforceCrystalPngUpload(){
  const PNG_SIGNATURE=[137,80,78,71,13,10,26,10];
  window.LUNY_CRYSTAL_PNG_ONLY=true;
  ...
  function install(){
    ...
    if(!input||input.__lunyCrystalPngOnlyBound)return;
    input.__lunyCrystalPngOnlyBound=true;
    input.accept='.png,image/png';

    input.addEventListener('change',async function(event){
      if(input.__lunyCrystalPngRedispatch){
        input.__lunyCrystalPngRedispatch=false;
        return;
      }
      ...
      if(meta)meta.textContent=file.name+'｜正在確認 PNG 格式…';
      const valid=await isPngFile(file);
      if(!valid){
        input.value='';
        if(meta)meta.textContent='格式不符：水晶轉印貼僅支援 PNG';
        alert('水晶轉印貼僅支援 PNG 圖檔，請重新選擇檔案。');
        return;
      }

      input.__lunyCrystalPngRedispatch=true;
