 ...
 * 檔名：luny-label-conversion-patch-20260921.js
 *
 * 1shop 試算頁最底部加一行（不要整頁貼 HTML）：
 * <script src="https://cdn.jsdelivr.net/gh/lunytw-collab/luny-tool@main/luny-label-conversion-patch-20260921.js?v=20260921a"></script>
 *
 * 本檔只做：
 * - 文案人話化（上傳／信任／步驟）
 * - 材質卡順序：透明、牛皮 → 再銀龍系列
...
(function () {
  ...
  if (window.__LUNY_LABEL_CONVERSION_PATCH_20260921__) return;
  window.__LUNY_LABEL_CONVERSION_PATCH_20260921__ = true;
  ...
  function injectStyle() {
    if ($("lunyConversionPatchStyle20260921")) return;
    var style = document.createElement("style");
    style.id = "lunyConversionPatchStyle20260921";
    style.textContent =
  ...
  function reorderSpecialMaterials() {
    ...
    if (moved) group.setAttribute("data-luny-material-order", "transparent-kraft-silver");
  ...
  function bindStepScroll() {
    if (window.__LUNY_CONV_STEP_SCROLL__) return;
    window.__LUNY_CONV_STEP_SCROLL__ = true;
  ...
  function observe() {
    if (window.__LUNY_CONV_PATCH_OBSERVER__) return;
    window.__LUNY_CONV_PATCH_OBSERVER__ = true;
    var scheduled = false;
