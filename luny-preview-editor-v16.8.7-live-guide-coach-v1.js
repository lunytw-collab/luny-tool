...
function lunyCustomDrawGuides(ctx,cx,cy,dims,b,gap,cm2px){
  ...
  const editorGuides=ctx.canvas===cG;
  const previewW=colorMode&&!editorGuides?cutW:bleedW,previewH=colorMode&&!editorGuides?cutH:bleedH;
  ...
  if(!colorMode||editorGuides)stroke(bleedW,bleedH,'#888888',4,[8,8]);stroke(cutW,cutH,'#D3162D',4,[]);stroke(safeW,safeH,'#32CD32',4,[8,8]);return true;
}
...
function drawGuides(ctx,shapeValue,cx,cy,dims,b,gap,cm2px){
  ...
  if(!colorMode||editorGuides)strokeShape(bleedW,bleedH,'#888888',4,[8,8]);
...
function validateDesignBeforeExport(action,opts){
  ...
  if(['printcut','external-save'].includes(action)&&!lunyGuideRequireConsent())return false;
...
}function shouldShowSafetyGuides(){return true;}
...
/* Lives inside the editor closure. No artwork or pricing mutation. */
var lunyGuideConsentKey='';
function lunyGuideCurrentKey(){
  return lunyWhiteEdgeStateKey()+'|'+String(textFill)+'|'+String(document.getElementById('edgeColor')?.value||'');
}
function lunyGuideHasConsent(){return !!img&&lunyGuideConsentKey===lunyGuideCurrentKey();}
function lunyGuideRequireConsent(){
  if(lunyGuideHasConsent())return true;
  document.dispatchEvent(new CustomEvent('luny:guideConsentNeeded'));
  return false;
}
window.LUNY_GUIDE_COACH={
  consent:function(yes){lunyGuideConsentKey=yes&&img?lunyGuideCurrentKey():'';return lunyGuideHasConsent();},
  snapshot:function(){
    const W=cG.width,H=cG.height,b=BLEED_CM*CM2PX,gap=GAP_CM*CM2PX;
    const edge=String(getEdgeOption()||'off').toLowerCase();
    const white=['on','white','whiteedge','white-edge','border','edge'].includes(edge);
    const fill=!!backgroundColor||['color','edgecolor','fullcolor'].includes(edge);
    const points=sampleShapeBoundaryPoints(Math.max(1,W-4),Math.max(1,H-4),100);
    const info=img?getPhotoPixelInfo():null;
    const missing=(!white&&!fill&&img&&info)?points.filter(p=>{const q=previewPointToPhotoPixel(p.x,p.y);return !q||getPhotoPixelRGBA(info,q.x,q.y)[3]<32;}):[];
    const risk=window.__LUNY_PREFLIGHT_LAST_RESULT__||{};
    return {ready:!!img,key:lunyGuideCurrentKey(),confirmed:lunyGuideHasConsent(),white,fill,unknown:!!img&&!info,
      danger:!!(window.__lunyDangerZoneHit||risk.flattenedRisk?.danger),missing, W,H,b,gap,
      safePoint:sampleShapeBoundaryPoints(Math.max(1,W-2*b-2*gap),Math.max(1,H-2*b-2*gap),80).filter(p=>p.x>=0&&p.x<=W&&p.y>=0&&p.y<=H).reduce((a,p)=>p.y<a.y?p:a,{x:W/2,y:H}),
      bleedPoint:points.filter(p=>p.x>=0&&p.x<=W&&p.y>=0&&p.y<=H).reduce((a,p)=>p.y>a.y?p:a,{x:W/2,y:0})};
  },
  paint:function(canvas,s,active){
    const ctx=canvas.getContext('2d');ctx.clearRect(0,0,canvas.width,canvas.height);
    if(!s.ready)return;
    const ratio=canvas.width/s.W;ctx.save();ctx.scale(ratio,ratio);
    if(s.danger){ctx.fillStyle='rgba(245,158,11,.16)';ctx.beginPath();addExportShapePath(ctx,shape.value,s.W/2,s.H/2,s.W-2*s.b,s.H-2*s.b,CM2PX);addExportShapePath(ctx,shape.value,s.W/2,s.H/2,Math.max(1,s.W-2*s.b-2*s.gap),Math.max(1,s.H-2*s.b-2*s.gap),CM2PX);ctx.fill('evenodd');}
    if(s.missing.length){ctx.save();ctx.beginPath();addExportShapePath(ctx,shape.value,s.W/2,s.H/2,s.W,s.H,CM2PX);addExportShapePath(ctx,shape.value,s.W/2,s.H/2,s.W-2*s.b,s.H-2*s.b,CM2PX);ctx.clip('evenodd');ctx.strokeStyle='rgba(217,119,6,.7)';ctx.lineWidth=2/ratio;for(const p of s.missing){const r=10/ratio;ctx.beginPath();ctx.moveTo(p.x-r,p.y+r);ctx.lineTo(p.x+r,p.y-r);ctx.stroke();}ctx.restore();}
    ctx.restore();
    if(!active)return;
    // Leaders point to actual native boundaries, including custom cut paths.
    const draw=(point,top,text,color)=>{const x=point.x*ratio,y=point.y*ratio;const w=Math.min(canvas.width-16,290),h=30,left=8,yy=top?8:canvas.height-h-8;ctx.strokeStyle=color;ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(left+w/2,top?yy+h:yy);ctx.lineTo(x,y);ctx.stroke();ctx.beginPath();ctx.arc(x,y,4,0,Math.PI*2);ctx.fillStyle=color;ctx.fill();ctx.fillStyle='rgba(255,255,255,.96)';ctx.fillRect(left,yy,w,h);ctx.strokeRect(left,yy,w,h);ctx.fillStyle=color;ctx.font='600 12px sans-serif';ctx.textAlign='left';ctx.textBaseline='middle';ctx.fillText(text,left+7,yy+h/2,w-14);};
    draw(s.safePoint,true,'文字・Logo・想保留的外框 → 綠線內','#237a38');
    draw(s.bleedPoint,false,'背景圖片／底色 → 延伸到出血線','#555');
  }
};
...
function drawPreview(){
  ...
  document.dispatchEvent(new CustomEvent('luny:guideDraw'));
...
window.lunyConfirmBleedBeforeSave=function(){
  if(!lunyGuideRequireConsent())return false;
...
(function(){
  ...
  function renderOverlay(){
    const canvas = $('canvasGuides');
    if(!canvas)return;
    if(document.getElementById('lunyThreeGuides')||!isFullBleedMode()||(window.__lunyFullBleedShouldApplyColor&&!window.__lunyFullBleedShouldApplyColor())){
