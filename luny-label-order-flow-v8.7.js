/* LUNY label order flow v8.5.1-cloud-safety
   減壓版 V1.1：Cloud publicUrl 優先、三檔上傳完整檢查、缺檔不送 GAS / 不加入清單。
   低風險拆檔版：儲存/結帳/訂單回寫流程。
...
async function saveDesignToGAS(){
  ...
  async function runSaveAttempt(attempt){
    ...
    const uploadedAssets=await uploadDesignAssetsToCloud(designId,payload,previewDataUrl);
    ...
    setSaveStatus(attempt===1?"正在建立設計資料…":"正在重新建立設計資料…");
    ...
    return {payload,designId,json,uploadedAssets,assets,previewDataUrl};
  }
  ...
  try{
    ...
    const savedItem={
      ...
      assets:result.assets||buildCloudAssetMeta(result.uploadedAssets),
      assetStatus:"done",
